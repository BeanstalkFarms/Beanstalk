...
function findTokenDepositIdsForAccounts(jsonFilePath, accounts) {
  // Load the JSON file
  const jsonData = JSON.parse(fs.readFileSync(jsonFilePath, "utf8"));

  // Array to hold the encoded data for each account
  let encodedResults = [];
  ...
  for (let account of accounts) {
    ...
    if (!jsonData.hasOwnProperty(account)) {
      encodedResults.push("0x"); // indicate to caller that there are no deposits for this account
      continue;
    }
    ...
    // Add the encoded data for this account to the result array
    encodedResults.push(encodedData);
  }

  // abi encode the array into a bytes array
  const encodedResultsArray = ethers.utils.defaultAbiCoder.encode(["bytes[]"], [encodedResults]);
...
const accounts = args.slice(1); // Now accepts an array of accounts

// Run the function and output the results
const encodedTokenDepositIdsArray = findTokenDepositIdsForAccounts(jsonFilePath, accounts);
if (encodedTokenDepositIdsArray) {
  console.log(encodedTokenDepositIdsArray);
}
