...
describe("newField", function () {
  before(async function () {
    ...
    this.bean = await initalizeUsersForToken(BEAN, [user, user2], to6("10000"));
  ...
  describe("Sow", async function () {
    describe("all soil", async function () {
      ...
      it("updates user's balance", async function () {
        expect(await this.bean.balanceOf(user.address)).to.eq(to6("9900"));
        expect(await beanstalk.plot(user.address, 0)).to.eq(to6("101"));
      });

      it("updates total balance", async function () {
        expect(await this.bean.balanceOf(beanstalk.address)).to.eq("0");
      ...
      it("emits Sow event", async function () {
        await expect(this.result)
          ...
          .withArgs(user.address, "0", to6("100"), to6("101"));
      });
    });
    ...
    describe("some soil from internal", async function () {
      beforeEach(async function () {
        ...
        await beanstalk
          .connect(user)
          .transferToken(this.bean.address, user.address, to6("100"), EXTERNAL, INTERNAL);
        this.result = await beanstalk.connect(user).sow(
            to6("100"), 
            0,
            INTERNAL
    ...
    describe("some soil from internal tolerant", async function () {
      beforeEach(async function () {
        ...
        await beanstalk
          .connect(user)
          .transferToken(this.bean.address, user.address, to6("50"), EXTERNAL, INTERNAL);
        this.result = await beanstalk.connect(user).sow(
            to6("100"), 
            0, 
            INTERNAL_TOLERANT
      ...
      it("updates user's balance", async function () {
        expect(await this.bean.balanceOf(user.address)).to.eq(to6("9950"));
      ...
      it("emits Sow event", async function () {
        await expect(this.result)
          .to.emit(mockBeanstalk, "Sow")
          .withArgs(user.address, "0", to6("50"), to6("50.5"));
      });
    });
    ...
    describe("second plot", async function () {
      ...
      it("emits Sow event", async function () {
        await expect(this.result)
          .to.emit(mockBeanstalk, "Sow")
          .withArgs(user.address, to6("101"), to6("100"), to6("101"));
      });
    });
  });
  ...
  describe("Harvest", async function () {
    ...
    describe("Full", async function () {
      ...
      it("updates user's balance", async function () {
        expect(await this.bean.balanceOf(user.address)).to.eq(to6("10001"));
      ...
      it("emits Sow event", async function () {
        await expect(this.result)
          ...
          .withArgs(user.address, ["0"], to6("101"));
      });
    });
    ...
    describe("Partial", async function () {
      ...
      it("emits Sow event", async function () {
        await expect(this.result)
          .to.emit(mockBeanstalk, "Harvest")
          .withArgs(user.address, ["0"], to6("50"));
      });
    });
