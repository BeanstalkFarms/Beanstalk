...
contract OracleTest is TestHelper {
    ...
    function testUniswapOracleImplementation() public {
        ...
        uint256 price = OracleFacet(address(bs)).getUsdTokenPrice(LibConstant.WBTC);
        assertEq(price, 0.00002e8, "price using encode type 0x01");

        setupUniswapWBTCOracleImplementation();

        price = OracleFacet(address(bs)).getTokenUsdPrice(LibConstant.WBTC);
    ...
    function test_uniswap_external() public {
        ...
        uint256 tokenUsdPriceFromExternal = OracleFacet(LibConstant.BEANSTALK)
            .getTokenUsdPriceFromExternal(LibConstant.WBTC, 0);
        assertApproxEqRel(
        ...
        uint256 usdTokenPriceFromExternal = OracleFacet(LibConstant.BEANSTALK)
            .getUsdTokenPriceFromExternal(LibConstant.WBTC, 0);
    ...
    function testExternalOracleImplementation() public {
        ...
        uint256 tokenPriceWBTC = OracleFacet(LibConstant.BEANSTALK).getTokenUsdPrice(
            LibConstant.WBTC
        ); // should be 50000e6
        assertEq(tokenPriceWBTC, 50000e6, "getTokenUsdPrice wbtc");

        // also exercise getMillionUsdPrice
        uint256 tokenPriceWBTCMillion = OracleFacet(LibConstant.BEANSTALK).getMillionUsdPrice(
            LibConstant.WBTC,
            0
        );
    ...
    function testGetOracleImplementationForToken() public {
        ...
        IMockFBeanstalk.Implementation memory oracleImplementation = bs
            .getOracleImplementationForToken(LibConstant.WBTC);
    ...
    function testGetTokenPrice() public {
        ...
        // token price is number of dollars per token, i.e. 50000 USD for 1 LibConstant.WBTC
        ...
        uint256 tokenPriceWBTC = OracleFacet(address(bs)).getTokenUsdPrice(LibConstant.WBTC); // should be 50000e6
        assertEq(tokenPriceWBTC, 50000e6, "getTokenUsdPrice wbtc");

        // number of tokens received per dollar
        uint256 usdPriceWBTC = OracleFacet(address(bs)).getUsdTokenPrice(LibConstant.WBTC); // $1 = 0.00002 wbtc, wbtc is 8 decimals,
    ...
    function test_getUsdTokenPrice_whenExternalToken_priceIsInvalid() public {
        ...
        // LibConstant.WBTC price is 50000
        uint256 priceWBTC = OracleFacet(address(bs)).getUsdTokenPrice(LibConstant.WBTC);
    ...
    function testForkMainnetWBTCOracle() public {
        ...
        uint256 priceWBTCmillion = OracleFacet(LibConstant.BEANSTALK).getMillionUsdPrice(
            LibConstant.WBTC,
            0
        );
        ...
        uint256 priceWBTC = OracleFacet(LibConstant.BEANSTALK).getUsdTokenPrice(LibConstant.WBTC);
    ...
    function testForkMainnetAAVEOracle() public {
        ...
        uint256 priceAAVE = OracleFacet(LibConstant.BEANSTALK).getUsdTokenPrice(AAVE);
    ...
    function testForkMainnetWSTETHOracle() public {
        ...
        uint256 priceWSTETH = OracleFacet(LibConstant.BEANSTALK).getUsdTokenPrice(
            LibConstant.WSTETH
        );
    ...
    function testForkMainnetWBTCDeltaB() public {
        ...
        vm.prank(WBTC_WHALE);
        IERC20(LibConstant.WBTC).transfer(LibConstant.BEAN_WBTC_WELL, 2e8); // 2 wbtc
        deal(address(LibConstant.BEAN), LibConstant.BEAN_WBTC_WELL, 117989199462, true); // approx 2 btc worth of beans
        IWell(LibConstant.BEAN_WBTC_WELL).sync(users[0], 0);

        // mock init state so that the bean token is defined
        IMockFBeanstalk(LibConstant.BEANSTALK).mockInitState();

        int256 deltaB = IMockFBeanstalk(LibConstant.BEANSTALK).poolCurrentDeltaBMock(
            LibConstant.BEAN_WBTC_WELL
        );
        assertEq(deltaB, 0);
    }
