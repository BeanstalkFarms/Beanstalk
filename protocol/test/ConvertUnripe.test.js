...
describe("Unripe Convert", function () {
  ...
  describe("calclates beans to peg", async function () {
    it("p > 1", async function () {
      await this.well
        .connect(user)
        .addLiquidity([toBean("0"), to18("0.2")], "0", user.address, ethers.constants.MaxUint256);
    ...
    it("p < 1", async function () {
      await this.well
        .connect(user)
        .addLiquidity([toBean("2000"), to18("0")], "0", user.address, ethers.constants.MaxUint256);
      expect(await this.convertGet.getMaxAmountIn(UNRIPE_BEAN, UNRIPE_LP)).to.be.equal("0");
    });
  });
  ...
  describe("calclates lp to peg", async function () {
    ...
    it("p < 1", async function () {
      await this.well
        .connect(user)
        .addLiquidity([toBean("2000"), to18("0")], "0", user.address, ethers.constants.MaxUint256);
      expect(await this.convertGet.getMaxAmountIn(UNRIPE_LP, UNRIPE_BEAN)).to.be.equal(
        to6("31.606981")
      );
    });
  });
    ...
    describe("basic", function () {
      ...
      beforeEach(async function () {
        ...
        this.result = await this.convert
          .connect(user)
          .convert(ConvertEncoder.convertUnripeBeansToLP(to6("1000"), "0"), ["0"], [to6("2000")]);
      });
      ...
      it("properly updates total values", async function () {
        // updates unripeBean.
        expect(await this.siloGetters.getTotalDeposited(this.unripeBean.address)).to.eq(
          to6("1000")
        );
        expect(await this.siloGetters.getTotalDepositedBdv(this.unripeBean.address)).to.eq(
          to6("100")
        );
        ...
        expect(await this.siloGetters.getGerminatingTotalDeposited(this.unripeLP.address)).to.eq(
          "4711829"
        );
        let bdv = await this.siloGetters.bdv(this.unripeLP.address, "4711829");
        await console.log("bdv", bdv.toString());
        expect(await this.siloGetters.getGerminatingTotalDepositedBdv(this.unripeLP.address)).to.eq(
          bdv
        );
      ...
      it("properly updates user values -test", async function () {
        ...
        expect(await this.siloGetters.balanceOfStalk(userAddress)).to.eq(
          toStalk("100.04").add(toStalk("0.04"))
        );
        expect(await this.siloGetters.balanceOfGerminatingStalk(userAddress)).to.eq(
          bdv.mul(to6("0.01"))
        );
      });
      ...
      it("properly updates user deposits", async function () {
        expect(
          (await this.siloGetters.getDeposit(userAddress, this.unripeBean.address, 0))[0]
        ).to.eq(to6("1000"));
      ...
      it("emits events", async function () {
        ...
        await expect(this.result)
          ...
          .withArgs(
            ...
            2656387,
            "4711829",
            await this.siloGetters.bdv(this.unripeLP.address, "4711829")
          );
      });
    });
  ...
  describe("convert lp to beans", async function () {
    ...
    describe("revert", async function () {
      it("not enough Beans", async function () {
        await this.well
          .connect(user)
          .addLiquidity([toBean("200"), "0"], "0", user.address, ethers.constants.MaxUint256);
    ...
    describe("below max", function () {
      ...
      it("properly updates total values", async function () {
        ...
        expect(await this.siloGetters.getGerminatingTotalDeposited(this.unripeBean.address)).to.eq(
          "0"
        );
        expect(
          await this.siloGetters.getGerminatingTotalDepositedBdv(this.unripeBean.address)
        ).to.eq("0");
        expect(await this.siloGetters.getGerminatingTotalDeposited(this.unripeLP.address)).to.eq(
          to6("0")
        );
  ...
  describe("convert unripe beans to beans", async function () {
    ...
    describe("basic urBEAN-->BEAN convert", function () {
      ...
      it("getters", async function () {
        ...
        expect(await this.unripe.getPenalizedUnderlying(UNRIPE_BEAN, to6("1"))).to.be.equal(
          to6("0.00101")
        );
      ...
      it("properly updates user deposits", async function () {
        ...
        expect((await this.siloGetters.getDeposit(userAddress, this.bean.address, 0))[0]).to.eq(
          to6("0.1")
        );
      });
      ...
      it("emits events", async function () {
        await expect(this.result)
          .to.emit(this.silo, "RemoveDeposits")
          .withArgs(userAddress, this.unripeBean.address, [0], [to6("100")], to6("100"), [
            to6("10")
          ]);
        await expect(this.result)
          .to.emit(this.silo, "AddDeposit")
