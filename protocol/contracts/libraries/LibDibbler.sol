...
library LibDibbler {
    ...
    function sow(uint256 amount, uint256 yield, address account) internal returns (uint256) {
        ...
        if (s.season.abovePeg) {
            ...
            amount = scaleSoilDown(amount, yield, maxYield);
            pods = beansToPods(amount, maxYield);
        } else {
            pods = beansToPods(amount, yield);
        }
    ...
    function scaleSoilUp(
        ...
        uint256 maxYield,
        uint256 yield
    ) internal pure returns (uint256 scaledSoil) {
        return soil.mulDiv(
    ...
    function scaleSoilDown(
        ...
        uint256 yield, 
        uint256 maxYield
    ) internal view returns (uint256 scaledSoil) {
        return soil.mulDiv(
            yield.add(100e6),
            maxYield.add(100e6),
