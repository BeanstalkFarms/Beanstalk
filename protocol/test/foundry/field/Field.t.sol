...
contract FieldTest is FieldFacet, TestHelper {
    ...
    function testMorningAuctionValues(uint256 blockNo, uint32 _weather) public {
        ...
        vm.roll(blockNo);
        uint256 __weather = uint256(season.weather().t).mulDiv(ScaleValues[blockNo - 1], 1e6, LibPRBMath.Rounding.Up);
    ...
    function test_remainingPods_belowPeg() public prank(brean) {
        ...
        assertEq(
            totalSoilSown, initalBreanBal - C.bean().balanceOf(brean), "total bean used does not equal total soil sown"
        );
    }
