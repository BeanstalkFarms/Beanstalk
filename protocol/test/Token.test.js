...
describe('Token', function () {
    ...
    describe('Balance functions', async function () {
        beforeEach(async function () {
            await beanstalk.connect(this.user).transferToken(this.token.address, this.user.address, '100', EXTERNAL, INTERNAL);
            await beanstalk.connect(this.user).transferToken(this.token.address, this.user2.address, '200', EXTERNAL, INTERNAL);
            await beanstalk.connect(this.user).transferToken(this.token2.address, this.user.address, '250', EXTERNAL, INTERNAL);
        })

        it('Tracks internal balance', async function () {
        expect(await beanstalk.getInternalBalance(this.user.address, this.token.address)).to.equal('100')
        expect(await beanstalk.getInternalBalance(this.user2.address, this.token.address)).to.equal('200')
        const internalBalances = await beanstalk.getInternalBalances(this.user.address, [this.token.address, this.token2.address])
        expect(internalBalances[0]).to.equal('100')
        expect(internalBalances[1]).to.equal('250')
        })

        it('Tracks external balance', async function () {
        expect(await beanstalk.getExternalBalance(this.user.address, this.token.address)).to.equal('700')
        expect(await beanstalk.getExternalBalance(this.user2.address, this.token.address)).to.equal('1000')
        const externalBalances = await beanstalk.getExternalBalances(this.user.address, [this.token.address, this.token2.address])
        ...
        const balances = await beanstalk.getBalances(this.user.address, [this.token.address, this.token2.address])
        expect(balances[0]).to.equal('800')
        expect(balances[1]).to.equal('1000')
        })

        it ('Tracks all balances', async function () {
        checkAllBalance(await beanstalk.getAllBalance(this.user.address, this.token.address), '100', '700', '800')
        checkAllBalance(await beanstalk.getAllBalance(this.user2.address, this.token.address), '200', '1000', '1200')
        const allBalances = await beanstalk.getAllBalances(this.user.address, [this.token.address, this.token2.address])
    ...
    describe('transfer', async function () {
        describe('External to external', async function () {
            it('basic', async function () {
                await beanstalk.connect(this.user).transferToken(this.token.address, this.recipient.address, '200', EXTERNAL, EXTERNAL);
        ...
        describe('External to internal', async function () {
            it('basic', async function () {
                this.result = await beanstalk.connect(this.user).transferToken(this.token.address, this.recipient.address, '200', EXTERNAL, INTERNAL);
                checkAllBalance(await beanstalk.getAllBalance(this.user.address, this.token.address), '0', '800', '800')
        ...
        describe('internal to internal', async function () {
            ...
            it('internal', async function () {
                await beanstalk.connect(this.user).transferToken(this.token.address, this.user.address, '200', EXTERNAL, INTERNAL);
                this.result = await beanstalk.connect(this.user).transferToken(this.token.address, this.recipient.address, '200', INTERNAL, INTERNAL);
                checkAllBalance(await beanstalk.getAllBalance(this.user.address, this.token.address), '0', '800', '800')
                checkAllBalance(await beanstalk.getAllBalance(this.recipient.address, this.token.address), '200', '0', '200')
            ...
            it('internal tolerant', async function () {
                await beanstalk.connect(this.user).transferToken(this.token.address, this.user.address, '200', EXTERNAL, INTERNAL);
                this.result = await beanstalk.connect(this.user).transferToken(this.token.address, this.recipient.address, '300', INTERNAL_EXTERNAL, INTERNAL);
            ...
            it('internal + external', async function () {
                await beanstalk.connect(this.user).transferToken(this.token.address, this.user.address, '200', EXTERNAL, INTERNAL);
                this.result = await beanstalk.connect(this.user).transferToken(this.token.address, this.recipient.address, '250', INTERNAL_TOLERANT, INTERNAL);
                checkAllBalance(await beanstalk.getAllBalance(this.user.address, this.token.address), '0', '800', '800')
                checkAllBalance(await beanstalk.getAllBalance(this.recipient.address, this.token.address), '200', '0', '200')
                await expect(this.result).to.emit(beanstalk, 'InternalBalanceChanged').withArgs(this.user.address, this.token.address, '-200')
                await expect(this.result).to.emit(beanstalk, 'InternalBalanceChanged').withArgs(this.recipient.address, this.token.address, '200')
            ...
            it('0 internal tolerant', async function () {
                this.result = await beanstalk.connect(this.user).transferToken(this.token.address, this.recipient.address, '0', INTERNAL_TOLERANT, INTERNAL);
        ...
        describe('internal to external', async function () {
            it('basic', async function () {
                await beanstalk.connect(this.user).transferToken(this.token.address, this.user.address, '200', EXTERNAL, INTERNAL);
                this.result = await beanstalk.connect(this.user).transferToken(this.token.address, this.recipient.address, '200', INTERNAL, EXTERNAL);
                checkAllBalance(await beanstalk.getAllBalance(this.user.address, this.token.address), '0', '800', '800')
                checkAllBalance(await beanstalk.getAllBalance(this.recipient.address, this.token.address), '0', '200', '200')
                await expect(this.result).to.emit(beanstalk, 'InternalBalanceChanged').withArgs(this.user.address, this.token.address, '-200')
            })
    ...
    describe('transfer internal from', async function () {
      ...
      describe("spender = msg.sender", async function () {
        it('to external', async function () {
            this.result = await beanstalk.connect(this.user).transferInternalTokenFrom(this.token.address, this.user.address, this.recipient.address, '200', EXTERNAL);
            checkAllBalance(await beanstalk.getAllBalance(this.user.address, this.token.address), '0', '800', '800')
            checkAllBalance(await beanstalk.getAllBalance(this.recipient.address, this.token.address), '0', '200', '200')
            await expect(this.result).to.emit(beanstalk, 'InternalBalanceChanged').withArgs(this.user.address, this.token.address, '-200')
        })
        ...
        it('to internal', async function () {
          this.result = await beanstalk.connect(this.user).transferInternalTokenFrom(this.token.address, this.user.address, this.recipient.address, '200', INTERNAL);
          checkAllBalance(await beanstalk.getAllBalance(this.user.address, this.token.address), '0', '800', '800')
          checkAllBalance(await beanstalk.getAllBalance(this.recipient.address, this.token.address), '200', '0', '200')
          await expect(this.result).to.emit(beanstalk, 'InternalBalanceChanged').withArgs(this.user.address, this.token.address, '-200')
        })
      ...
      describe('spender != msg.sender', async function () {
        describe('without approval', async function () {
          it('reverts if approval = 0', async function () {
            await expect(beanstalk.connect(this.user2).transferInternalTokenFrom(this.token.address, this.user.address, this.recipient.address, '200', INTERNAL)).to.be.revertedWith('Token: insufficient allowance')
          })

          it('reverts if approval < amount', async function () {
            await beanstalk.connect(this.user).approveToken(this.user2.address, this.token.address, '100')
            await expect(beanstalk.connect(this.user2).transferInternalTokenFrom(this.token.address, this.user.address, this.recipient.address, '200', INTERNAL)).to.be.revertedWith('Token: insufficient allowance')
          })
        })
        ...
        describe('with approval', async function () {
          it('max approval', async function () {
            await beanstalk.connect(this.user).approveToken(this.user2.address, this.token.address, ethers.constants.MaxUint256)
            this.result = await beanstalk.connect(this.user2).transferInternalTokenFrom(this.token.address, this.user.address, this.recipient.address, '200', INTERNAL)
            checkAllBalance(await beanstalk.getAllBalance(this.user.address, this.token.address), '0', '800', '800')
            checkAllBalance(await beanstalk.getAllBalance(this.recipient.address, this.token.address), '200', '0', '200')
            await expect(this.result).to.emit(beanstalk, 'InternalBalanceChanged').withArgs(this.user.address, this.token.address, '-200')
          })
    ...
    describe("weth", async function () {
        ...
        it('deposit WETH to external, not all, farm', async function () {
            const ethBefore = await ethers.provider.getBalance(this.user.address)
            const wrapEth = await beanstalk.interface.encodeFunctionData("wrapEth", [to18('1'), EXTERNAL]);
    ...
    describe("Token Approval", async function () {
        describe("approve allowance", async function () {
          beforeEach(async function () {
            this.result = await beanstalk.connect(this.user).approveToken(this.user2.address, this.token.address, '100');
          })
    
          it('properly updates users token allowance', async function () {
            expect(await beanstalk.tokenAllowance(this.user.address, this.user2.address, this.token.address)).to.be.equal('100')
          })
    
          it('emits TokenApproval event', async function () {
            await expect(beanstalk.connect(this.user).approveToken(this.user2.address, this.token.address, '100')).to.emit(beanstalk, 'TokenApproval').withArgs(this.user.address ,this.user2.address, this.token.address, '100');
          });
        })
        ...
        describe("increase and decrease allowance", async function () {
          ...
          it('properly increase users token allowance', async function () {
            await beanstalk.connect(this.user).increaseTokenAllowance(this.user2.address, this.token.address, '100');
            expect(await beanstalk.tokenAllowance(this.user.address, this.user2.address, this.token.address)).to.be.equal('200')
          })
    
          it('properly decrease users token allowance', async function () {
            await beanstalk.connect(this.user).decreaseTokenAllowance(this.user2.address, this.token.address, '25')
            expect(await beanstalk.tokenAllowance(this.user.address, this.user2.address, this.token.address)).to.be.equal('75')
          })
    
          it('decrease users token allowance below zero', async function () {
            await expect(beanstalk.connect(this.user).decreaseTokenAllowance(this.user2.address, this.token.address, '101')).to.revertedWith('Silo: decreased allowance below zero');
          })
    
          it('emits TokenApproval event on increase', async function () {
            const result = await beanstalk.connect(this.user).increaseTokenAllowance(this.user2.address, this.token.address, '25');
            await expect(result).to.emit(beanstalk, 'TokenApproval').withArgs(this.user.address ,this.user2.address, this.token.address, '125');
          });
    
          it('emits TokenApproval event on decrease', async function () {
            const result = await beanstalk.connect(this.user).decreaseTokenAllowance(this.user2.address, this.token.address, '25');
            await expect(result).to.emit(beanstalk, 'TokenApproval').withArgs(this.user.address ,this.user2.address, this.token.address, '75');
          });
        })
        ...
        describe("Approve Token Permit", async function () {
          describe('reverts', function () {
            it('reverts if tokenPermitDomainSeparator is invalid', async function () {
              expect(await beanstalk.connect(this.user).tokenPermitDomainSeparator()).to.be.equal("0xd74031d32a83121ee5d7c0c14f2aac23c5603bf832f855c2e075ba6d0b20612e");
            });
          });
          ...
          describe("single token permit", async function() {
            describe('reverts', function () {
              it('reverts if permit expired', async function () {
                ...
                await expect(beanstalk.connect(this.user).permitToken(
                  signature.owner, 
                  signature.spender, 
                  signature.token, 
                  signature.value, 
            ...
            describe("approve permit", async function() {
              beforeEach(async function () {
                ...
                const nonce = await beanstalk.connect(this.user).tokenPermitNonces(this.user.address);
                const signature = await signTokenPermit(this.user, this.user.address, this.user2.address, this.token.address, '1000', nonce);
                this.result = await beanstalk.connect(this.user).permitToken(
                  signature.owner, 
                  signature.spender, 
                  signature.token, 
                  signature.value, 
              ...
              it('properly updates user token allowance', async function () {
                expect(await beanstalk.tokenAllowance(this.user.address, this.user2.address, this.token.address)).to.be.equal('1000')
              });
      
              it('emits TokenApproval event', async function () {
                await expect(this.result).to.emit(beanstalk, 'TokenApproval').withArgs(this.user.address ,this.user2.address, this.token.address, '1000');
              });
            });
          });
        });
