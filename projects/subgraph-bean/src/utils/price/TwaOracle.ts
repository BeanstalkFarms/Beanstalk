...
export function getTWAPrices(poolAddress: string, type: TWAType, timestamp: BigInt): BigInt[] {
  ...
  if (type == TWAType.UNISWAP) {
    ...
    twaPrices = [
      ...
      newPriceCumulative[0].minus(twaOracle.priceCumulativeLast[0]).div(timeElapsed).times(BI_10.pow(6)).div(ONE_BI.leftShift(112)),
      newPriceCumulative[1].minus(twaOracle.priceCumulativeLast[1]).div(timeElapsed).times(BI_10.pow(6)).div(ONE_BI.leftShift(112))
    ];
  } else {
    // Curve
    newPriceCumulative = curveCumulativePrices(Address.fromString(poolAddress), timestamp);
    twaPrices = [
      newPriceCumulative[0].minus(twaOracle.priceCumulativeLast[0]).div(timeElapsed),
      newPriceCumulative[1].minus(twaOracle.priceCumulativeLast[1]).div(timeElapsed)
    ];
  }
