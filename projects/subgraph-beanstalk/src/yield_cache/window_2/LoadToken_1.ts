...
function loadTokenCache(): void {
  for (let i = 0; i < TOKEN_YIELD_7_DAY_12_000.length; i++) {
    let tokenYield = loadTokenYield(
      Address.fromString(TOKEN_YIELD_7_DAY_12_000[i][0]),
      <i32>parseInt(TOKEN_YIELD_7_DAY_12_000[i][1]),
      <i32>parseInt(TOKEN_YIELD_7_DAY_12_000[i][5])
    );
    tokenYield.beanAPY = BigDecimal.fromString(TOKEN_YIELD_7_DAY_12_000[i][2]);
    tokenYield.stalkAPY = BigDecimal.fromString(TOKEN_YIELD_7_DAY_12_000[i][3]);
    tokenYield.createdAt = BigInt.fromString(TOKEN_YIELD_7_DAY_12_000[i][4]);
    tokenYield.save();
  }
}
