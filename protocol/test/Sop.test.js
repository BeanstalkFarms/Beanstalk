...
describe('Sop', function () {
  ...
  describe('1 sop', async function () {
    ...
    it('sops p > 1', async function () {
      ...
      expect(await this.weth.balanceOf(beanstalk.address)).to.be.equal(to18('51.191151829696906017'))
    ...
    it('tracks user plenty before update', async function () {
      expect(await beanstalk.connect(user).balanceOfPlenty(user.address)).to.be.equal('25595575914848452999')
    })
    ...
    it('tracks user2 plenty', async function () {
      expect(await beanstalk.connect(user).balanceOfPlenty(user2.address)).to.be.equal(to18('25.595575914848452999'))
    })
  ...
  describe('multiple sop', async function () {
    ...
    it('tracks user plenty before update', async function () {
      expect(await beanstalk.connect(user).balanceOfPlenty(user.address)).to.be.equal('38544532214605630101')
    })
    ...
    it('tracks user2 plenty', async function () {
      expect(await beanstalk.connect(user).balanceOfPlenty(user2.address)).to.be.equal('38547120970363278477')
    })
