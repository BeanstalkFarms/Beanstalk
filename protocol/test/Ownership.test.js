...
describe('Ownership', function () {
  ...
  describe('ownership', async function () {
    describe('transfer', async function () {
      it('reverts if not owner', async function () {
        await expect(beanstalk.connect(user2).transferOwnership(user2.address)).to.be.revertedWith('LibDiamond: Must be contract owner')
      })
    ...
    describe('claim', async function () {
      it('reverts if not candidate', async function () {
        await expect(beanstalk.connect(user2).claimOwnership()).to.be.revertedWith('Ownership: Not candidate')
      })
      ...
      it('claims ownership', async function () {
        ...
        await expect(this.result).to.emit(beanstalk, 'OwnershipTransferred').withArgs(ownerAddress, user2.address)
      })
    })
  })
  ...
  describe('pause', async function () {
    it('reverts if not owner', async function () {
      await expect(beanstalk.connect(user2).pause()).to.be.revertedWith('LibDiamond: Must be contract or owner')
    })
  ...
  describe('unpause', async function () {
    it('reverts if not owner', async function () {
      await expect(beanstalk.connect(user2).unpause()).to.be.revertedWith('LibDiamond: Must be contract or owner')
    })
