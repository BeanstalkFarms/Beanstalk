...
contract BasinDeployer is Utils {
    ...
    function deployWells(bool mock, bool verbose) internal {
        ...
        wells.push(deployBeanCp2Well([LibConstant.BEAN_ETH_WELL, LibConstant.WETH], _pump));
        if (verbose) console.log("Bean Eth well deployed at:", wells[0]);
        vm.label(LibConstant.BEAN_ETH_WELL, "BEAN/ETH Well");

        // deploy bean wsteth well:
        wells.push(deployBeanCp2Well([LibConstant.BEAN_WSTETH_WELL, LibConstant.WSTETH], _pump));
        if (verbose) console.log("Bean wstEth well deployed at:", wells[1]);
        vm.label(LibConstant.BEAN_WSTETH_WELL, "BEAN/WSTETH Well");
    }
    ...
    function deployExtraWells(bool mock, bool verbose) internal {
        ...
        wells.push(deployBeanCp2Well([BEAN_USDC_WELL, LibConstant.USDC], _pump));
        if (verbose) console.log("Bean USDC well deployed at:", wells[0]);
        vm.label(BEAN_USDC_WELL, "BEAN/USDC Well");

        // deploy Bean USDT well:
        wells.push(deployBeanCp2Well([LibConstant.BEAN_USDT_WELL, LibConstant.USDT], _pump));
        if (verbose) console.log("Bean USDT well deployed at:", wells[1]);
        vm.label(LibConstant.BEAN_USDT_WELL, "BEAN/USDT Well");
    }
