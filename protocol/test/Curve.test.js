...
describe.skip("Curve", function () {
  ...
  describe("Add Liquidity", async function () {
    describe("To external", async function () {
      ...
      it("adds lp tokens to external balance", async function () {
        expect(
          await beanstalk.getExternalBalance(user.address, this.beanMetapool.address)
        ).to.be.equal(to18("2000"));
      });

      it("adds lp tokens to internal balance", async function () {
        expect(
          await beanstalk.getInternalBalance(user.address, this.beanMetapool.address)
        ).to.be.equal(to18("0"));
      });
    });
  ...
  describe("Exchange", async function () {
    describe("To external", async function () {
      ...
      it("adds lp tokens to internal balance", async function () {
        expect(
          await beanstalk.getInternalBalance(user2.address, this.threeCurve.address)
        ).to.be.equal("0");
      });

      it("adds lp tokens to external balance", async function () {
        expect(
          await beanstalk.getExternalBalance(user2.address, this.threeCurve.address)
  ...
  describe("Remove Liqudity", async function () {
    describe("To external", async function () {
      ...
      it("adds lp tokens to external balance", async function () {
        expect(
          await beanstalk.getExternalBalance(user.address, this.threeCurve.address)
        ).to.be.equal(to18("250"));
        expect(await beanstalk.getExternalBalance(user.address, bean.address)).to.be.equal(
          to6("9250")
        );
      });
    });
