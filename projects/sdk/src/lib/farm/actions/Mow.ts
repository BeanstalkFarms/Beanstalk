  ...
  async run(_amountInStep: ethers.BigNumber, context: RunContext) {
    ...
    if (tokensToMow.length === 1) {
      return {
        ...
        decode: (data: string) =>
          Mow.sdk.contracts.beanstalk.interface.decodeFunctionData("mow", data),
        decodeResult: (result: string) =>
          Mow.sdk.contracts.beanstalk.interface.decodeFunctionResult("mow", result)
      };
    } else {
      return {
        name: "mowMultiple",
        ...
        decode: (data: string) =>
          Mow.sdk.contracts.beanstalk.interface.decodeFunctionData("mowMultiple", data),
        decodeResult: (result: string) =>
          Mow.sdk.contracts.beanstalk.interface.decodeFunctionResult("mowMultiple", result)
      };
    }
  }
}
