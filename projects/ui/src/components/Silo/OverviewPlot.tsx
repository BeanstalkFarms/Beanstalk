...
export type OverviewPlotProps = {
  ...
  stats: (dataPoint: BaseDataPoint | undefined) => React.ReactElement;
  empty: boolean;
  loading: boolean;
  label: string;
...
const OverviewPlot: FC<OverviewPlotProps> = ({
  ...
  const [displayDataPoint, setDisplayDataPoint] = useState<
    BaseDataPoint | undefined
  >();
  ...
  const formatValueStacked = (value: number) =>
    `${displayStalk(BigNumber(value, 10), 2)}`;
        ...
        {ready ? (
          <>
            {useStackedChart ? (
              ...
              <LineChart
                series={filteredSeries}
                onCursor={handleCursorLineChart}
              />
            )}
          </>
        ) : (
