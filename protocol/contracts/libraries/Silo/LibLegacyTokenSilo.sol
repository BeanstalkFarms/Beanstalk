...
library LibLegacyTokenSilo {
    ...
    function balanceOfGrownStalkUpToStemsDeployment(
        ...
        return stalkReward(s.a[account].s.seeds, stemStartSeason.sub(lastUpdate));
    }
    ...
    function _mowAndMigrate(
        ...
        LibSilo.mintActiveStalk(account, balanceOfGrownStalkUpToStemsDeployment(account));
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            for (uint256 j = 0; j < seasons[i].length; j++) {
                ...
                migrateData.totalGrownStalk = migrateData.totalGrownStalk.add(
                    stalkGrownSinceStemStartSeason
                );
    ...
    function _mowAndMigrateMerkleCheck(
        ...
        // the amount of stalk that has grown between the silo v3 deployment season and the enroot fix season
        if (seedsDiff > 0) {
