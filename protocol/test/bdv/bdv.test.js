...
const { BEAN, THREE_POOL, BEAN_3_CURVE, UNRIPE_LP, UNRIPE_BEAN, ZERO_ADDRESS, WETH, BEAN_WSTETH_WELL, WSTETH } = require('../utils/constants');
const { to18, to6 } = require('../utils/helpers.js');
const { deployMockPump, getWellContractFactory, deployMockWellWithMockPump } = require('../../utils/well.js');
...
describe('BDV', function () {
  ...
  it("reverts if not correct", async function () {
    await expect(beanstalk.bdv(ZERO_ADDRESS, to18('2000'))).to.be.revertedWith('Silo: Token not whitelisted')
  })
});