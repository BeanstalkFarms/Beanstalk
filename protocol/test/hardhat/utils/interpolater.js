...
function interpolatePoints(xs, ys) {
  ...
  for (let i = 0; i < length; i++) {
    ...
    let exponentDeg0 = math.pow(
      math.bignumber(10),
      math.bignumber(math.number(ys[i]).calculateShifts(startingExponent))
    );
    let exponentDeg1 = math.pow(
      math.bignumber(10),
      math.bignumber(math.number(c1s[i]).calculateShifts(startingExponent))
    );

    coefficients[i * 4] = math.format(math.floor(math.abs(math.multiply(ys[i], exponentDeg0))), {
      notation: "fixed"
    });
    coefficients[i * 4 + 1] = math.format(
      math.floor(math.abs(math.multiply(c1s[i], exponentDeg1))),
      { notation: "fixed" }
    );
    ...
    if (i < dxs.length) {
      ...
      let exponentDeg2 = math.pow(
        math.bignumber(10),
        math.bignumber(math.number(c2s[i]).calculateShifts(startingExponent))
      );
      let exponentDeg3 = math.pow(
        math.bignumber(10),
        math.bignumber(math.number(c3s[i]).calculateShifts(startingExponent))
      );
      coefficients[i * 4 + 2] = math.format(
        math.floor(math.abs(math.multiply(c2s[i], exponentDeg2))),
        { notation: "fixed" }
      );
      coefficients[i * 4 + 3] = math.format(
        math.floor(math.abs(math.multiply(c3s[i], exponentDeg3))),
        { notation: "fixed" }
      );
    } else {
      signs[i * 4 + 2] = false;
  ...
  const hexFunc =
    "0x" + hexLen + hexBrkpts.join("") + hexCoefs.join("") + hexExps.join("") + hexSigns.join("");
