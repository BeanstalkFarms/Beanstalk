...
describe("Depot", function () {
  before(async function () {
    ...
    await this.beanstalk
      .connect(user)
      .transferToken(BEAN, user.address, to6("1"), EXTERNAL, INTERNAL);
  });
  ...
  describe("Deposit Transfer reverts with wrong sender", async function () {
    it("transferDeposit", async function () {
      await expect(
        this.depot
          .connect(user2)
          .transferDeposit(user.address, PIPELINE, this.siloToken.address, 0, to6("1"))
    ...
    it("transferDeposits", async function () {
      await expect(
        this.depot
          .connect(user2)
          .transferDeposits(user.address, PIPELINE, this.siloToken.address, [to6("0")], [to6("1")])
      ).to.be.revertedWith("invalid sender");
    });
  });
