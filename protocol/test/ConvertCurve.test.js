...
describe("Curve Convert", function () {
  ...
  describe.skip("calculates beans to peg LEGACY", async function () {
    it("p > 1", async function () {
      ...
      expect(
        await this.convertGet.getMaxAmountIn(this.bean.address, this.beanMetapool.address)
      ).to.be.equal(ethers.utils.parseUnits("200", 6));
    });
  ...
  describe("calculates beans to peg", async function () {
    it("p > 1", async function () {
      await expect(
        this.convertGet.getMaxAmountIn(this.bean.address, this.beanMetapool.address)
  ...
  describe("calculates lp to peg", async function () {
    it("p > 1", async function () {
      ...
      expect(
        await this.convertGet.getMaxAmountIn(this.beanMetapool.address, this.bean.address)
      ).to.be.equal("199185758314813528598");
    });
  ...
  describe.skip("convert beans to lp LEGACY", async function () {
    describe("revert", async function () {
      ...
      it("Not whitelisted pool", async function () {
        const convertData = ConvertEncoder.convertBeansToCurveLP(
          toBean("200"),
          to18("190"),
          this.fakeMetapool.address
        );
        await expect(this.convert.connect(owner).convert(convertData, [], [])).to.be.revertedWith(
          "Convert: Not a whitelisted Curve pool."
        );
      });
    });
    ...
    describe("below max", async function () {
      ...
      it("it gets amount out", async function () {
        expect(await this.convertGet.getAmountOut(BEAN, BEAN_3_CURVE, toBean("100"))).to.be.equal(
          "100634476734756985505"
        );
      });
      ...
      describe("it converts", async function () {
        ...
        it("properly updates total values", async function () {
          ...
          expect(await this.siloGetters.getTotalDepositedBdv(this.bean.address)).to.eq(
            toBean("100")
          );
          expect(await this.siloGetters.getTotalDeposited(this.beanMetapool.address)).to.eq(
            "100634476734756985505"
          );
          expect(await this.siloGetters.getTotalDepositedBdv(this.beanMetapool.address)).to.eq(
            toBean("100")
          );
        ...
        it("properly updates user deposits", async function () {
          ...
          const stemMetapool = await this.siloGetters.seasonToStem(this.beanMetapool.address, "12");
          expect(
            (await this.siloGetters.getDeposit(userAddress, this.bean.address, stemBean))[0]
          ).to.eq(toBean("100"));
        ...
        it("emits events", async function () {
          ...
          await expect(this.result)
            ...
            .withArgs(userAddress, this.bean.address, [stemBean], [toBean("100")], toBean("100"), [
              toBean("100")
            ]);
          await expect(this.result)
    ...
    describe("above max", function () {
      ...
      it("it gets amount out", async function () {
        expect(await this.convertGet.getAmountOut(BEAN, BEAN_3_CURVE, toBean("200"))).to.be.equal(
          "200832430692705624354"
        );
      });
      ...
      describe("it converts", async function () {
        ...
        it("emits events", async function () {
          ...
          await expect(this.result)
            .to.emit(this.silo, "RemoveDeposits")
            .withArgs(userAddress, this.bean.address, [stemBean], [toBean("200")], toBean("200"), [
              toBean("200")
            ]);
          const stemMetapool = await this.siloGetters.seasonToStem(this.beanMetapool.address, "12");
          await expect(this.result)
    ...
    describe("multiple crates", function () {
      ...
      describe("it converts", async function () {
        beforeEach(async function () {
          ...
          this.result = await this.convert
            ...
            .convert(
              ...
              [stemBean10, stemBean14],
              [toBean("100"), toBean("100")]
            );
        });
        ...
        it("properly updates user deposits", async function () {
          ...
          const stemBean14 = await this.siloGetters.seasonToStem(this.bean.address, "14");
          expect(
            (await this.siloGetters.getDeposit(userAddress, this.bean.address, stemBean10))[0]
          ).to.eq(toBean("0"));
          expect(
            (await this.siloGetters.getDeposit(userAddress, this.bean.address, stemBean14))[0]
          ).to.eq(toBean("0"));
          ...
          const deposit = await this.siloGetters.getDeposit(
            ...
            this.beanMetapool.address,
            12
          ); //someone else please do the math and verify that 12 is the expected season here
          expect(deposit[0]).to.eq("200832430692705624354");
          expect(deposit[1]).to.eq(toBean("200"));
        });
  ...
  describe("convert beans to lp", async function () {
    describe("below max", async function () {
      ...
      it("it gets amount out", async function () {
        await expect(
          this.convertGet.getAmountOut(BEAN, BEAN_3_CURVE, toBean("100"))
        ).to.be.revertedWith("Convert: Tokens not supported");
      });
  ...
  describe("convert lp to beans", async function () {
    describe("revert", async function () {
      ...
      it("not enough Beans", async function () {
        await this.beanMetapool
          .connect(user)
          .add_liquidity([toBean("200"), to18("0")], to18("150"));
      ...
      it("Not whitelisted pool", async function () {
        const convertData = ConvertEncoder.convertCurveLPToBeans(
          to18("100"),
          toBean("99"),
          this.fakeMetapool.address
        );
    ...
    describe("below max", function () {
      beforeEach(async function () {
        ...
        await this.beanMetapool
          ...
          .add_liquidity([toBean("200"), to18("0")], to18("150"));
        await this.silo.connect(user).deposit(this.beanMetapool.address, to18("1000"), EXTERNAL);
        this.stem = await this.siloGetters.seasonToStem(
          this.beanMetapool.address,
      ...
      it("it gets amount out", async function () {
        expect(await this.convertGet.getAmountOut(BEAN_3_CURVE, BEAN, to18("100"))).to.be.equal(
          "100618167"
        );
      });
    ...
    describe("above max", function () {
      ...
      it("it gets amount out", async function () {
        expect(
          await this.convertGet.getAmountOut(BEAN_3_CURVE, BEAN, "199185758314813528598")
    ...
    describe("multiple crates", function () {
      beforeEach(async function () {
        ...
        await this.season.siloSunrise(0);
        await this.silo.connect(user).deposit(this.beanMetapool.address, to18("500"), EXTERNAL);
        this.stem2 = await this.siloGetters.seasonToStem(
          this.beanMetapool.address,
          await this.seasonGetters.season()
        );
      ...
      describe("it converts", async function () {
        ...
        it("properly updates user deposits", async function () {
          expect(
            (await this.siloGetters.getDeposit(userAddress, this.bean.address, this.beanStem))[0]
          ).to.eq("100618167");
          const deposit = await this.siloGetters.getDeposit(
            userAddress,
            this.beanMetapool.address,
