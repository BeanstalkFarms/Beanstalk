...
testIfRpcSet('SeedGauge Init Test', function () {
  ...
  describe('init state', async function () {
    ...
    it('lockedBeans', async function () {
      ...
      expect(await beanstalk.getLockedBeans()).to.be.within(to6('25900000.000000'), to6('26000000.000000'));
    })

    it('usd Liquidity', async function () {
      // ~13.2m usd liquidity in Bean:Eth
      expect(await beanstalk.getTwaLiquidityForWell(BEAN_ETH_WELL)).to.be.within(to18('13100000'), to18('13300000'));
      // ~13.2m usd liquidity in Bean:Eth
      expect(await beanstalk.getTotalUsdLiquidity()).to.be.within(to18('13100000'), to18('13300000'));
    })
  ...
  describe('bean3crv dewhitelisted', async function () {
    ...
    it('reverts on deposit', async function () {
      await expect(beanstalk.connect(user).deposit(BEAN_3_CURVE, to18('100'), INTERNAL))
    ...
    it('allows legacy bean3crv to be mown', async function () {
      initalStalk = await beanstalk.balanceOfStalk(
      ...
      newStalk = await beanstalk.balanceOfStalk(
        '0x1B7eA7D42c476A1E2808f23e18D850C5A4692DF7'
      )
  ...
  describe('silo v3.1 migration', async function () {
    ...
    it('correctly updates a deposit for user', async function () {
      ...
      this.result = await beanstalk.connect(account).transferDeposit(
        account.address,
        user.address,
        BEAN,
        12648000000,
