...
describe('Fundraiser', function () {
  before(async function () {
    ...
    await bean.mint(user2.address, '1000000000')
    await this.token.mint(user.address, '1000000000')
    await this.token.mint(user2.address, '1000000000')
  ...
  describe('create fundraiser', function () {
    ...
    describe('create a fundraiser', async function () {
      ...
      it('mints beans to protocol', async function () {
        await expect(await bean.balanceOf(this.fundraiser.address)).to.be.equal('1000')
      })
    })
    ...
    describe('create 2 fundraisers', async function () {
      ...
      it('mints beans to protocol', async function () {
        await expect(await bean.balanceOf(this.fundraiser.address)).to.be.equal('2000')
      })
    })
  })
  ...
  describe('fund fundraiser', async function () {
    ...
    describe('over fund', async function () {
      ...
      it('burns beans from protocol', async function () {
        await expect(await bean.balanceOf(this.fundraiser.address)).to.be.equal('0')
      })
    })
    ...
    describe('partial fund', async function () {
      ...
      it('emits a FundFundraiser event', async function () {
        await expect(this.result).to.emit(this.fundraiser, 'FundFundraiser').withArgs(user.address, 0, '500')
      })

      it('updates user balances', async function () {
        expect(await beanstalk.plot(user.address, 0)).to.be.equal('500')
      })

      it('emits a Sow event', async function () {
        await expect(this.result).to.emit(beanstalk, 'Sow').withArgs(user.address, 0, '500', '500')
      })

      it('burns beans from protocol', async function () {
        await expect(await bean.balanceOf(this.fundraiser.address)).to.be.equal('500')
      })
    })
    ...
    describe('fully fund', async function () {
      ...
      it('emits a FundFundraiser event', async function () {
        await expect(this.result).to.emit(this.fundraiser, 'FundFundraiser').withArgs(user.address, 0, '1000')
      })

      it('sows a plot', async function () {
        expect(await beanstalk.plot(user.address, 0)).to.be.equal('1000')
      })

      it('emits a Sow event', async function () {
        await expect(this.result).to.emit(beanstalk, 'Sow').withArgs(user.address, 0, '1000', '1000')
      })
