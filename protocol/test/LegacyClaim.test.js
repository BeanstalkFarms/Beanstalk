...
describe.skip("Legacy Claim", async function () {
  before(async function () {
    ...
    await upgradeWithNewFacets({
      ...
      facetNames: ["LegacyClaimWithdrawalFacet"],
  ...
  describe("claim", async function () {
    ...
    before(async function () {
      ...
      for (var i = 0; i < this.withdraws.length; i++) {
        ...
        this.deltaTokens[i] = (await thisToken.balanceOf(withdraw.farmer.id)).sub(
          this.userTokensBefore[i]
        );
      }
    });
    ...
    it("properly removes the withdrawal", async function () {
      for (var i = 0; i < this.withdraws.length; i++) {
        const withdraw = this.withdraws[i];
        expect(
          await this.legacyClaims[i].getWithdrawal(
            withdraw.farmer.id,
            withdraw.token,
            withdraw.withdrawSeason
          )
        ).to.eq("0");
      }
    });
    ...
    it("emits a RemoveWithdrawal event", async function () {
      for (var i = 0; i < this.withdraws.length; i++) {
        ...
        await expect(this.results[i])
          .to.emit(this.silo, "RemoveWithdrawal")
          .withArgs(
            ethers.utils.getAddress(withdraw.farmer.id),
            ethers.utils.getAddress(withdraw.token),
            withdraw.withdrawSeason,
            withdraw.amount
          );
      }
