...
library LibConvert {
    ...
    function beansToPeg(uint ethBeanPool, uint beansBeanPool) internal view returns (uint256 beans) {
        ...
        uint256 newBeans = LibMarket.sqrt(ethBeanPool.mul(beansBeanPool).mul(usdcUSDCPool).div(ethUSDCPool));
    ...
    function lpToPeg() internal view returns (uint256 lp) {
        ...
        uint c = LibMarket.sqrt(y*b*1e18/(x*e)).mul(1e9);
        if (c <= 1e18) return 0;
