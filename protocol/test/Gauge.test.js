...
describe("Gauge", function () {
  ...
  describe("Bean to maxLP ratio", function () {
    ...
    describe("L2SR > excessively high L2SR % + P > 1", async function () {
      it("increases Bean to maxLP ratio", async function () {
        ...
        await expect(this.result).to.emit(this.season, "BeanToMaxLpGpPerBdvRatioChange").withArgs(
          3, // season
          108, // caseId
    ...
    it("Bean to maxLP ratio properly scales", async function () {
      ...
      // 0.51 * (1 - 0.5) + 0.5 = 75.5
      expect(await this.seasonGetters.getBeanToMaxLpGpPerBdvRatioScaled()).to.be.equal(
        to18("75.5")
      );
    });
  ...
  describe("L2SR calculation", async function () {
    describe("getter", function () {
      it("outputs correct liquidity values:", async function () {
        expect(await this.seasonGetters.getTwaLiquidityForWell(BEAN_ETH_WELL)).to.be.equal(
          to18("1000000")
        );
        expect(await this.seasonGetters.getTotalUsdLiquidity()).to.be.equal(to18("1000000"));
        expect(await this.seasonGetters.getWeightedTwaLiquidityForWell(BEAN_ETH_WELL)).to.be.equal(
          to18("1000000")
        );
        expect(await this.seasonGetters.getTotalWeightedUsdLiquidity()).to.be.equal(
          to18("1000000")
        );
      });
      ...
      it("initial state", async function () {
        ...
        expect(await this.seasonGetters.getLiquidityToSupplyRatio()).to.be.equal(to18("1"));
      });
      ...
      it("returns 0 if no liquidity", async function () {
        ...
        expect(await this.seasonGetters.getLiquidityToSupplyRatio()).to.be.equal(0);
      });
    ...
    describe("with unripe", function () {
      before(async function () {
        ...
        await this.unripe.connect(owner).addUnderlying(UNRIPE_LP, to18("31.62277663"));
      ...
      it("getters", async function () {
        ...
        expect(await this.unripe.getLockedBeansUnderlyingUnripeBeanEth()).to.be.eq(
          to6("436.332105")
        );
        expect(await this.unripe.getLockedBeans()).to.be.eq(to6("872.66421"));
        expect(await this.seasonGetters.getLiquidityToSupplyRatio()).to.be.eq(
          to18("1.000873426417975035")
        );
      });
      ...
      it("< 1m unripe lockedBeans calculation:", async function () {
        ...
        const getLockedBeansUnderlyingUnripeBean =
          await this.unripe.getLockedBeansUnderlyingUnripeBean();
        const getLockedBeansUnderlyingUrLP =
          await this.unripe.getLockedBeansUnderlyingUnripeBeanEth();
        ...
        expect(await this.unripe.getLockedBeansUnderlyingUnripeBean()).to.be.eq(
  ...
  describe("GaugePoints", async function () {
    ...
    it("update seeds values", async function () {
      ...
      expect(await this.seasonGetters.getBeanEthGaugePointsPerBdv()).to.be.eq(
        to18("15.811392351684831136")
      );
      expect(await this.seasonGetters.getBeanGaugePointsPerBdv()).to.be.eq(
        to18("11.858544263763623352")
      );
    ...
    it("Cannot exceed the maximum gauge points", async function () {
      expect(await this.gaugePoint.defaultGaugePointFunction(to18("1000"), 50e6, 49e6)).to.be.eq(
        to18("1000")
      );

      expect(await this.gaugePoint.defaultGaugePointFunction(to18("1001"), 50e6, 49e6)).to.be.eq(
        to18("1000")
      );
    });
  });
  ...
  describe("excessive price", async function () {
    beforeEach(async function () {
      ...
      await this.season.mockSetUsdTokenPrice(BEAN_ETH_WELL, to18("0.001"));
    });
