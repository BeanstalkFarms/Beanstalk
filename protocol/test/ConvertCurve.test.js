  ...
  describe('convert beans to lp', async function () {
    ...
    describe('revert', async function () {      
      it('not enough LP', async function () {
        await this.silo.connect(user).depositBeans(toBean('200'));
        await this.beanMetapool.connect(user).add_liquidity([toBean('0'), to18('200')], to18('150'));
    ...
    describe('below max', function () {
      ...
      it('properly updates total values', async function () {
        expect(await this.silo.totalDepositedBeans()).to.eq(toBean('100'));
        expect(await this.silov2.getTotalDeposited(this.beanMetapool.address)).to.eq('100634476734756985505');
      ...
      it('properly updates user deposits', async function () {
        expect(await this.silo.beanDeposit(userAddress, 2)).to.eq(toBean('100'));
        const deposit = await this.silov2.getDeposit(userAddress, this.beanMetapool.address, 2);
      ...
      it('emits events', async function () {
        await expect(this.result).to.emit(this.silo, 'BeanRemove')
          .withArgs(userAddress, [2], [toBean('100')], toBean('100'));
        await expect(this.result).to.emit(this.silov2, 'Deposit')
    ...
    describe('above max', function () {
      beforeEach(async function () {
        await this.silo.connect(user).depositBeans(toBean('300'));
        await this.beanMetapool.connect(user).add_liquidity([toBean('0'), to18('200')], to18('150'));
      ...
      it('properly updates total values', async function () {
        expect(await this.silo.totalDepositedBeans()).to.eq(toBean('100'));
        expect(await this.silov2.getTotalDeposited(this.beanMetapool.address)).to.eq('200832430692705624354');
      ...
      it('emits events', async function () {
        await expect(this.result).to.emit(this.silo, 'BeanRemove')
          .withArgs(userAddress, [2], [toBean('200')], toBean('200'));
        await expect(this.result).to.emit(this.silov2, 'Deposit')
    ...
    describe('after one season', function () {
      ...
      it('properly updates total values', async function () {
        expect(await this.silo.totalDepositedBeans()).to.eq(toBean('0'));
        expect(await this.silov2.getTotalDeposited(this.beanMetapool.address)).to.eq('200832430692705624354');
      ...
      it('properly updates user deposits', async function () {
        expect(await this.silo.beanDeposit(userAddress, 2)).to.eq(toBean('0'));
        const deposit = await this.silov2.getDeposit(userAddress, this.beanMetapool.address, 3);
        expect(deposit[0]).to.eq('200832430692705624354');
        expect(deposit[1]).to.eq(toBean('200'));
      });
    ...
    describe('multiple crates', function () {
      beforeEach(async function () {
        await this.silo.connect(user).depositBeans(toBean('100'));
        await this.season.siloSunrise(0);
        await this.season.siloSunrise(0);
        await this.season.siloSunrise(0);
        await this.season.siloSunrise(0);
      ...
      it('properly updates user deposits', async function () {
        expect(await this.silo.beanDeposit(userAddress, 2)).to.eq(toBean('0'));
        expect(await this.silo.beanDeposit(userAddress, 6)).to.eq(toBean('0'));
        const deposit = await this.silov2.getDeposit(userAddress, this.beanMetapool.address, 5);
        expect(deposit[0]).to.eq('200832430692705624354');
        expect(deposit[1]).to.eq(toBean('200'));
      });
      ...
      it('emits events', async function () {
        await expect(this.result).to.emit(this.silo, 'BeanRemove')
          .withArgs(userAddress, [2, 6], [toBean('100'), toBean('100')], toBean('200'));
        await expect(this.result).to.emit(this.silov2, 'Deposit')
  ...
  describe('convert lp to beans', async function () {
    ...
    describe('revert', async function () {      
      it('not enough Beans', async function () {
        await this.beanMetapool.connect(user).add_liquidity([toBean('200'), to18('0')], to18('150'));
        await this.silov2.connect(user).deposit(this.beanMetapool.address, to18('1000'));
        console.log(`---------------------------------------------------`);
    ...
    describe('below max', function () {
      ...
      it('properly updates total values', async function () {
        expect(await this.silo.totalDepositedBeans()).to.eq('100618167');
        expect(await this.silov2.getTotalDeposited(this.beanMetapool.address)).to.eq(to18('900'));
      ...
      it('properly updates user deposits', async function () {
        expect(await this.silo.beanDeposit(userAddress, 2)).to.eq('100618167');
        const deposit = await this.silov2.getDeposit(userAddress, this.beanMetapool.address, 2);
    ...
    describe('above max', function () {
      ...
      it('properly updates total values', async function () {
        expect(await this.silo.totalDepositedBeans()).to.eq('199999999');
        expect(await this.silov2.getTotalDeposited(this.beanMetapool.address)).to.eq('800832430692705624354');
      ...
      it('properly updates user deposits', async function () {
        expect(await this.silo.beanDeposit(userAddress, 2)).to.eq('199999999');
        const deposit = await this.silov2.getDeposit(userAddress, this.beanMetapool.address, 2);
      ...
      it('emits events', async function () {
        ...
        await expect(this.result).to.emit(this.silo, 'BeanDeposit')
          .withArgs(userAddress, 2, '199999999');
      });
    });
    ...
    describe('multiple crates', function () {
      beforeEach(async function () {
        await this.beanMetapool.connect(user).add_liquidity([toBean('200'), to18('0')], to18('150'));
        await this.silov2.connect(user).deposit(this.beanMetapool.address, to18('500'));
        await this.season.siloSunrise(0);
        await this.silov2.connect(user).deposit(this.beanMetapool.address, to18('500'));
      ...
      it('properly updates user deposits', async function () {
        expect(await this.silo.beanDeposit(userAddress, 3)).to.eq('100618167');
        const deposit = await this.silov2.getDeposit(userAddress, this.beanMetapool.address, 2);
      ...
      it('emits events', async function () {
        ...
        await expect(this.result).to.emit(this.silo, 'BeanDeposit')
          .withArgs(userAddress, 3, '100618167');
      });
    });
  });
});
