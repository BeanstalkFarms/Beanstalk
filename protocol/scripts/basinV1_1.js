...
const { toX } = require("../test/hardhat/utils/helpers");
...
const BEAN_WSTETH_WELL_DEPLOY_SALT =
  "0x8c5a1440b12f0eca90b905ed8a1d6ff0595c4192e23963e595e223f4780d10af";
const BEAN_WSTETH_WELL_NAME = "BEAN:WSTETH Constant Product 2 Well";
const BEAN_WSTETH_WELL_SYMBOL = "BEANWSTETHCP2w";
const BEAN_WSTETH_PUMP_DATA =
  "";
...
async function deployBasinV1_1(
  mock = true,
  accounts = undefined,
  verbose = true,
  justDeploy = false,
...
async function deployBasinV1_1Upgrade(
  ...
  c.constantProduct2 = await deployWellContractAtNonce(
    "ConstantProduct2",
    CONSTANT_PRODUCT_2_DEPLOY_NONCE,
    [],
    account,
    verbose,
    (version = "1.1")
  ...
  if (mockPump) {
    ...
    c.multiFlowPump = await deployWellContractAtNonce(
      "MultiFlowPump",
      MULTI_FLOW_PUMP_DEPLOY_NONCE,
      [],
      account,
      verbose,
      (version = "1.1")
  ...
  const initData = c.wellImplementation.interface.encodeFunctionData("init", [
    BEAN_WSTETH_WELL_NAME,
    BEAN_WSTETH_WELL_SYMBOL
  ]);
  ...
  const wellTxn = await c.aquifer
    .connect(account)
    .boreWell(c.wellImplementation.address, immutableData, initData, BEAN_WSTETH_WELL_DEPLOY_SALT);
  ...
  const stEthEthChainlinkAggregator = await ethers.getContractAt(
    "MockChainlinkAggregator",
    STETH_ETH_CHAINLINK_PRICE_AGGREGATOR
  );
  ...
  let txn = await c.well
    .connect(account)
    .addLiquidity(amounts, lpAmountOut, account.address, ethers.constants.MaxUint256);
  await txn.wait();
  txn = await c.well
    .connect(account)
    .addLiquidity([toBN("0"), toBN("0")], "0", account.address, ethers.constants.MaxUint256);
  await txn.wait();
