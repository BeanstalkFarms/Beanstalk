...
export default function useRevitalized() {
  ...
  return useMemo(() => {
    ...
    const expectedGrownBDV = (addr: string) =>
      balances[addr]?.deposited.crates.reduce(
        (ss, c) =>
          ss.plus(
            currentSeason
              .minus(c.season)
              .times(c.amount.times(beanstalkSilo.balances[addr]?.bdvPerToken))
          ),
        ZERO_BN
      ) || ZERO_BN;
    const actualGrownBDV = (addr: string) =>
      balances[addr]?.deposited.crates.reduce(
        (ss, c) => ss.plus(currentSeason.minus(c.season).times(c.bdv)),
        ZERO_BN
      ) || ZERO_BN;
    ...
    const deltaGrown2 = MaxBN(
      expectedGrownBDV(urBeanCrv3.address).minus(
        actualGrownBDV(urBeanCrv3.address)
      ),
      ZERO_BN
    );
    ...
    const stalk = delta1
      .plus(delta2)
      .times(BEAN_TO_STALK)
      .plus(deltaGrown1.times(BEAN_TO_SEEDS).div('10000'))
      .plus(deltaGrown2.times(LP_TO_SEEDS).div('10000'));
  ...
  }, [balances, beanstalkSilo, currentSeason, getChainToken]);
}
