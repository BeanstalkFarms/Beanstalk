...
describe("Whitelist", function () {
  ...
  describe("whitelist", async function () {
      ...
      await expect(
        this.whitelist
          ...
          .whitelistToken(
            ...
            "10000",
            "1",
            this.gaugePoint.interface.getSighash(
              "defaultGaugePointFunction(uint256,uint256,uint256)"
            ),
    ...
    it("whitelists token", async function () {
      ...
      await expect(this.result)
        .to.emit(this.whitelist, "WhitelistToken")
        .withArgs(
          this.well.address,
          this.bdv.interface.getSighash("wellBdv"),
          1,
    ...
    it("reverts on whitelisting same token again", async function () {
      this.resultFirst = await this.whitelist
        .connect(owner)
        .whitelistTokenWithEncodeType(
          this.well.address,
          this.bdv.interface.getSighash("wellBdv"),
    ...
    it("reverts on updating stalk per bdv per season for token that is not whitelisted", async function () {
      await expect(
        this.whitelist.connect(owner).updateStalkPerBdvPerSeasonForToken(this.well.address, 1)
  ...
  describe("update stalk per bdv per season for token", async function () {
    it("reverts if not owner", async function () {
      await expect(
        this.whitelist.connect(user2).updateStalkPerBdvPerSeasonForToken(this.well.address, 1)
      ).to.be.revertedWith("LibDiamond: Must be contract or owner");
    });
    ...
    it("updates stalk per bdv per season", async function () {
      ...
      await expect(this.result)
        .to.emit(this.whitelist, "UpdatedStalkPerBdvPerSeason")
  ...
  describe("dewhitelist", async function () {
    it("reverts if not owner", async function () {
      await expect(
        this.whitelist.connect(user2).dewhitelistToken(this.well.address)
      ).to.be.revertedWith("LibDiamond: Must be contract or owner");
    });
