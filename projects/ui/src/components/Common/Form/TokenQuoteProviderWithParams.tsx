...
export type TokenQuoteProviderWithParamsCustomProps<T> = {
  ...
  'state' | 'tokenOut' | 'handleQuote' | 'quoteSettings' | 'displayQuote'
>;

type TokenQuoteWithParamsProviderProps<T> =
  TokenQuoteProviderWithParamsCustomProps<T> & Partial<TokenInputProps>;

const DefaultQuoteDisplay = (
...
export default function TokenQuoteProviderWithParams<T>({
  ...
  const [result, quoting, getAmountOut] = useQuoteWithParams<T>(
    tokenOut,
    handleQuote,
    quoteSettings
  );
  ...
  const displayQuote =
    _displayQuote === undefined ? DefaultQuoteDisplay : _displayQuote;
  ...
  useEffect(() => {
    if (state.token.symbol !== tokenOut.symbol) {
      console.debug(
        `[TokenQuoteProvider] Inputs changed. Refreshing amount out: ${state.amount} ${state.token?.symbol} => X ${tokenOut.symbol}`
      );
      getAmountOut(
        state.token, // tokenIn
  ...
  }, [params, state.token, state.amount, getAmountOut, tokenOut]);
