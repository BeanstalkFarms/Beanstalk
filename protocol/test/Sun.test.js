  ...
  [...Array(numberTests).keys()].map(i => i + startTest).forEach(function(v) {
    ...
    describe(testStr.concat((v)), function () {
      ...
      it('decrements the withdraw buffer', async function () {
              await this.season.resetState()
              expect(await this.season.getWithdrawBuffer()).to.eq(25)
              expect(await this.season.getCurrentSeason()).to.eq('1')
              await this.season.weekSunrise()
              expect(await this.season.getWithdrawBuffer()).to.eq(24)
              expect(await this.season.getCurrentSeason()).to.eq('168')
              await this.season.weekSunrise()
              expect(await this.season.getWithdrawBuffer()).to.eq(23)
              await this.season.weekSunrise()
              expect(await this.season.getWithdrawBuffer()).to.eq(22)
              await this.season.decrementSunrise(14)
              expect(await this.season.getWithdrawBuffer()).to.eq(8)
              await this.season.decrementSunrise(100)
              expect(await this.season.getWithdrawBuffer()).to.eq(4)
      })
    })
  })
})
