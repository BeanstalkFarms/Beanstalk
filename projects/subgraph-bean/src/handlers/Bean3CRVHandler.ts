...
export function handleAddLiquidity(event: AddLiquidity): void {
  handleLiquidityChange(
    event.address.toHexString(),
    event.params.token_amounts[0] !== ZERO_BI && event.params.token_amounts[1] !== ZERO_BI,
    event.block
  );
}
...
export function handleRemoveLiquidityOne(event: RemoveLiquidityOne): void {
  handleLiquidityChange(event.address.toHexString(), false, event.block);
}

function handleLiquidityChange(poolAddress: string, isBoth: boolean, block: ethereum.Block): void {
  // TODO: refactor this part out from liquidity and swap
  let deltaB;
  let newPoolPrice;
  let reserveBalances;
  let deltaLiquidityUSD;
  if (poolAddress === BEAN_3CRV_V1.toHexString()) {
    ...
    deltaB = curveDeltaBUsingVPrice(Address.fromString(poolAddress), reserveBalances[0]);
