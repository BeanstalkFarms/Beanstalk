...
contract MetadataImage {
    ...
    function border() internal pure returns (string memory) {
        return
            '<rect x="8" y="8" width="240" height="335" rx="6" stroke="#9BCAA0" stroke-width="2" fill="none"/>';
    }
    ...
    function plotLogic(int96 stalkPerBDV) internal pure returns (string memory) {
        ...
        uint256[NUM_PLOTS] memory order = [
            uint256(20),
            19,
            13,
            18,
        ...
        for (uint256 i = 0; i < NUM_PLOTS; ++i) {
            ...
            if (plotNo < numPlotsToFill) {
                _plot = abi.encodePacked(
                    _plot,
                    useAsset("fullLeafPlot", XYPLOT[i][0], XYPLOT[i][1])
                );
                ...
                if (numPlotsToFill == 1) {
                    ...
                    _plot = abi.encodePacked(
                        _plot,
                        useAsset("partialLeafPlot", XYPLOT[i][0], XYPLOT[i][1])
                    );
                }
            } else {
    ...
    function useAssetTransformFill(
        string memory assetName,
        int256 x,
        int256 y,
        string memory color
