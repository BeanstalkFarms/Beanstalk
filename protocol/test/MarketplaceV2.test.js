...
const { ppval_listing, ppval_order, findSortedIndex, interpolate, parseIntervals, getFunctionDegree } = require('./utils/interpolater.js')
...
describe('MarketplaceV2', function () {
    ...
    describe("order evaluation", async function () {
      ...
      describe("within an interval", async function () {
        ...
        it("first interval", async function () {
          ...
          expect(await this.marketplace.connect(user).priceOrderFill(this.f, placeInLine, 0, amount)).to.be.equal(amountBeans);
        })
  ...
  describe("Pod Listings", async function () {
    ...
    describe("Create Dynamic", async function () {
      ...
      it('Fails if already expired', async function () {
        await this.field.incrementTotalHarvestableE('2000');
        await expect(this.marketplace.connect(user).createDPodListing('0', '0', '500', '100000', '0', INTERNAL, this.function)).to.be.revertedWith('Marketplace: Expired.');
      })

      it('Fails if amount is 0', async function () {
        await expect(this.marketplace.connect(user2).createDPodListing('1000', '0', '0', '100000', '0', INTERNAL, this.function)).to.be.revertedWith('Marketplace: Invalid Plot/Amount.');
      })

      it('Fails if start + amount too large', async function () {
        await expect(this.marketplace.connect(user2).createDPodListing('1000', '500', '1000', '100000', '0', INTERNAL, this.function)).to.be.revertedWith('Marketplace: Invalid Plot/Amount.');
      })

      describe("List full plot", async function () {
        beforeEach(async function () {
          this.result = await this.marketplace.connect(user).createDPodListing('0', '0', '1000', '500000', '0', EXTERNAL, this.function);
        })
        ...
        it('Emits event', async function () {
          await expect(this.result).to.emit(this.marketplace, 'DynamicPodListingCreated').withArgs(userAddress, 0, 0, '1000', 500000, 0, 0, this.function[1], this.function[2], this.function[3]);
        })
      })
      ...
      describe("List partial plot", async function () {
        ...
        it('Emits event', async function () {
          await expect(this.result).to.emit(this.marketplace, 'DynamicPodListingCreated').withArgs(userAddress, 0, 0, '500', 500000, 0, 0, this.function[1], this.function[2], this.function[3]);
        })
      })
      ...
      describe("List partial plot from middle", async function () {
        ...
        it('Emits event', async function () {
          await expect(this.result).to.emit(this.marketplace, 'DynamicPodListingCreated').withArgs(userAddress, 0, 500, '500', 500000, 2000, 1, this.function[1], this.function[2], this.function[3]);
        })
      })
      ...
      describe("Relist plot from middle", async function () {
        ...
        it('Emits event', async function () {
          await expect(this.result).to.emit(this.marketplace, 'PodListingCancelled').withArgs(userAddress, 0);
          await expect(this.result).to.emit(this.marketplace, 'DynamicPodListingCreated').withArgs(userAddress, 0, 500, '100', 500000, 2000, 1, this.function[1], this.function[2], this.function[3]);
        })
      })
    })
    ...
    describe("Fill", async function () {
      ...
      describe('revert', async function () {
        beforeEach(async function () {
          this.function = [CONSTANT, emptyFunction.values, emptyFunction.bases, emptyFunction.signs];
        ...
        it('Fill Listing not enough pods in listing', async function () {
          ...
          const l = [userAddress, '0', '0', '500', '500000', '0', INTERNAL, [CONSTANT, emptyFunction.values, emptyFunction.bases, emptyFunction.signs]]
    ...
    describe("Fill Dynamic", async function () {
      ...
      describe('revert', async function () {
        ...
        it('Fill Listing not enough pods in listing', async function () {
          ...
          await this.marketplace.connect(user).createDPodListing('0', '0', '500', '500000', '0', INTERNAL, this.function);
          await expect(this.marketplace.connect(user2).fillPodListing(l, 500, EXTERNAL)).to.be.revertedWith('Marketplace: Not enough pods in Listing');
        })
      })
    ...
    describe("Cancel Dynamic", async function () {
      ...
      it('Re-list plot cancels and re-lists', async function () {
        ...
        expect(await this.marketplace.podListing(0)).to.be.equal(await getHash(result));
        result = await this.marketplace.connect(user).createDPodListing('0', '0', '1000', '200000', '2000', INTERNAL, this.function);
        await expect(result).to.emit(this.marketplace, 'DynamicPodListingCreated').withArgs(userAddress, '0', 0, 1000, 200000, 2000, 1, this.function[1], this.function[2], this.function[3]);
        await expect(result).to.emit(this.marketplace, 'PodListingCancelled').withArgs(userAddress, '0');
        expect(await this.marketplace.podListing(0)).to.be.equal(await getHash(result));
      })
      ...
      it('Cancels Listing, Emits Listing Cancelled Event', async function () {
        result = await this.marketplace.connect(user).createDPodListing('0', '0', '1000', '500000', '2000', EXTERNAL, this.function);
        expect(await this.marketplace.podListing(0)).to.be.equal(await getHash(result));
        result = (await this.marketplace.connect(user).cancelPodListing('0'));
        expect(await this.marketplace.podListing(0)).to.be.equal(ZERO_HASH);
        expect(result).to.emit(this.marketplace, 'PodListingCancelled').withArgs(userAddress, '0');
  ...
  describe("Pod Order", async function () {
    ...
    describe("Create", async function () {
      ...
      describe('create order', async function () {
        ...
        it('Creates the order', async function () {
          ...
          expect(await this.marketplace.podOrder(userAddress, '100000', '1000', [CONSTANT, emptyFunction.values, emptyFunction.bases, emptyFunction.signs])).to.equal('500');
        })
    ...
    describe("Create Dynamic", async function () {
      ...
      describe('create order', async function () {
        ...
        it('emits an event', async function () {
          ...
          expect(this.result).to.emit(this.marketplace, 'DynamicPodOrderCreated').withArgs(userAddress, this.id, '500', 100000, '1000', this.function[1], this.function[2], this.function[3])
        })
      })
    ...
    describe("Fill Dynamic", async function() {
      ...
      describe("Full order with active listing", async function () {
        beforeEach(async function () {
          await this.marketplace.connect(user2).createDPodListing('1000', '500', '500', '10000', '5000', EXTERNAL, this.function);
          this.beanstalkBalance = await this.bean.balanceOf(this.marketplace.address)
          this.user2BeanBalance = await this.bean.balanceOf(user2Address)
          this.result = await this.marketplace.connect(user2).fillPodOrder(this.order, 1000, 0, 500, INTERNAL);
          this.beanstalkBalanceAfter = await this.bean.balanceOf(this.marketplace.address)
    ...
    describe("Cancel Dynamic", async function () {
      ...
      describe('Cancel owner', async function () {
        beforeEach(async function () {
          this.userBeanBalance = await this.bean.balanceOf(userAddress)
          this.beanstalkBeanBalance = await this.bean.balanceOf(this.marketplace.address)
          this.result = await this.marketplace.connect(user).cancelDPodOrder('100000', '1000', EXTERNAL, this.function);
          this.userBeanBalanceAfter = await this.bean.balanceOf(userAddress)
          this.beanstalkBeanBalanceAfter = await this.bean.balanceOf(this.marketplace.address)
        })
      ...
      describe('Cancel to wrapped', async function () {
        beforeEach(async function () {
          this.userBeanBalance = await this.bean.balanceOf(userAddress)
          this.beanstalkBeanBalance = await this.bean.balanceOf(this.marketplace.address)
          this.result = await this.marketplace.connect(user).cancelDPodOrder('100000', '1000', INTERNAL, this.function);
          this.userBeanBalanceAfter = await this.bean.balanceOf(userAddress)
          this.beanstalkBeanBalanceAfter = await this.bean.balanceOf(this.marketplace.address)
        })
