...
describe.skip("Silo V3: Stem deployment migrate everyone", function () {
  before(async function () {
    ...
    await upgradeWithNewFacets({
      ...
      facetNames: ["ConvertFacet", "WhitelistFacet", "MockSiloFacet", "MockSeasonFacet"],
  ...
  async function getDepositsForAccount(account, onChainBdv = false) {
    ...
    for (let i = 0; i < lpEvent.length; i++) {
      ...
      if (d.event == "AddDeposit") {
        if (!deposits[token][season])
          deposits[token][season] = {
            amount: toBN("0"),
            bdv: toBN("0")
          };
        ...
        for (let i = 0; i < seasons.length; i++) {
          ...
          if (`${amount}` !== "0") {
            let bdv = amount
              .mul(deposits[token][seasons[i]].bdv)
              .div(deposits[token][seasons[i]].amount);
            ...
            if (deposits[token][seasons[i]].amount.eq(toBN("0")))
              delete deposits[token][seasons[i]];
          }
        }
      }
    ...
    it("for all depositors", async function () {
      ...
      if (!fs.existsSync(__dirname + "/data/deposits.json")) {
        ...
        await fs.writeFileSync(
          __dirname + "/data/deposits.json",
          JSON.stringify(deposits, null, 4)
        );
      }
      ...
      if (!fs.existsSync(__dirname + "/data/stalk_balance_changed.json")) {
        ...
        await fs.writeFileSync(
          __dirname + "/data/stalk_balance_changed.json",
          JSON.stringify(stalkChanges, null, 4)
        );
      }
      ...
      if (!fs.existsSync(__dirname + "/data/seed_balance_changed.json")) {
        ...
        //write stalk_balance_changed to disk
        await fs.writeFileSync(
          __dirname + "/data/seed_balance_changed.json",
          JSON.stringify(seedChanges, null, 4)
        );
      }
      ...
      let seedStalkDiff = JSON.parse(
        await fs.readFileSync(__dirname + "/../scripts/silov3-merkle/data/seed-stalk-merkle.json")
      );

      //load stalk balance changed from disk
      let stalkChanges = JSON.parse(
        await fs.readFileSync(__dirname + "/data/stalk_balance_changed.json")
      );
      console.log("stalkChanges: ", stalkChanges);

      //load seed balance changed from disk
      let seedChanges = JSON.parse(
        await fs.readFileSync(__dirname + "/data/seed_balance_changed.json")
      );
      ...
      for (const depositorAddress in deposits) {
        ...
        const migrateResult = await this.migrate.mowAndMigrate(
          depositorAddress,
          tokens,
          seasons,
          amounts,
        ...
        for (let k = 0; k < migratedDeposits.length; k++) {
          ...
          const withdraw = await beanstalk
            .connect(depositorSigner)
            .withdrawDeposit(
              migratedDeposit.token,
              migratedDeposit.stem,
              migratedDeposit.amount,
              EXTERNAL
            );
        ...
        const finalStalkSum = stalkDeltaSum
          .add(stalkBalanceChangedSum)
          .add(migrateStalkBalanceChanged);
