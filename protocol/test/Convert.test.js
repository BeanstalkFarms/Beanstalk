...
describe("Convert", function () {
    ...
    describe("Withdraw 1 Crate", async function () {
      ...
      it("Emits event", async function () {
        await expect(this.result)
          ...
          .withArgs(userAddress, this.siloToken.address, [2000000], ["100"], "100", ["100"]);
  ...
  describe("Deposit For Convert", async function () {
    describe("Revert", async function () {
      it("Reverts if BDV is 0", async function () {
        await expect(
          this.convert.connect(user2).depositForConvertE(this.siloToken.address, "100", "0", "100")
        ).to.be.revertedWith("Convert: BDV or amount is 0.");
      });

      it("Reverts if amount is 0", async function () {
        await expect(
          this.convert.connect(user2).depositForConvertE(this.siloToken.address, "0", "100", "100")
        ).to.be.revertedWith("Convert: BDV or amount is 0.");
      });
    });
    ...
    describe("Deposit Tokens No Grown", async function () {
      beforeEach(async function () {
        this.result = await this.convert
          .connect(user2)
          .depositForConvertE(this.siloToken.address, "100", "100", "0");
      });

      it("Emits event", async function () {
        await expect(this.result)
          .to.emit(this.silo, "AddDeposit")
          .withArgs(user2Address, this.siloToken.address, to6("4"), "100", "100");
      });
      ...
      it("increment totals", async function () {
        ...
        expect(
          await this.siloGetters.getGerminatingTotalDeposited(this.siloToken.address)
        ).to.equal("100");
        expect(
    ...
    describe("Deposit not germinating", async function () {
      ...
      it("Emits event", async function () {
        await expect(this.result)
          .to.emit(this.silo, "AddDeposit")
          .withArgs(user2Address, this.siloToken.address, to6("3"), "100", "100");
      });
    ...
    describe("Deposit Tokens more grown", async function () {
      ...
      it("Emits event", async function () {
        ...
        await expect(this.result)
          .to.emit(this.silo, "AddDeposit")
          .withArgs(user2Address, this.siloToken.address, to6("1"), "100", "100");
      });
  ...
  describe("lambda convert", async function () {
    ...
    it("Emits events", async function () {
      ...
      await expect(this.result)
        .to.emit(this.silo, "AddDeposit")
        .withArgs(userAddress, this.siloToken.address, to6("1.5"), "200", "200");
    });
  });
});
