...
describe('Marketplace', function () {
  ...
  const resetState = async function () {
    ...
    await this.field.connect(user).sowBeansAndIndex('1000');
    await this.field.connect(user2).sowBeansAndIndex('1000');
    await this.marketplace.connect(user).listPlot('0', '1000', '0', '500000', '0');
    await this.marketplace.connect(user2).listPlot('1000', '500', '0', '100000', '0');
    await this.marketplace.connect(user).listPlot('2000', '1000', '0', '500000', '0');
    await this.marketplace.connect(user2).listPlot('3000', '1000', '0', '100000', '2000');
    await this.marketplace.connect(user).listPlot('4000', '1000', '0', '100000', '2000');
    await this.marketplace.connect(user2).listPlot('5000', '500', '0', '500000', '2000');
  ...
  describe("List Plot", async function () {
    ...
    it('Emits a List event', async function () {
      result = await this.marketplace.connect(user2).listPlot('5000', '1000', '0', '100000', '0');
      await expect(result).to.emit(this.marketplace, 'PodListingCreated').withArgs(user2Address, '5000', 1000, 0, 100000, 0);

    });

    it('Fails to List Unowned Plot', async function () {
      await expect(this.marketplace.connect(user).listPlot('5000', '1000', '0', '500000', '1000')).to.be.revertedWith('Marketplace: Invalid Plot/Amount.');
    });

    it('Lists partial Plot', async function () {
      const podListing = await this.marketplace.podListing(1000, user2Address);
      expect(podListing.price).to.equal(100000);
      expect(podListing.expiry.toString()).to.equal('0');
      expect(podListing.amount.toString()).to.equal('500');
    });


    it('Lists full Plot', async function () {
      const podListing = await this.marketplace.podListing(0, userAddress);
  ...
  describe("Buy Listing", async function () {
    ...
    it('Buy Full Listing, Plots Transfer, Balances Update', async function () {
      ...
      await this.marketplace.connect(user2).buyListing(userAddress, 0, 0, amountBeansBuyingWith);
      ...
      const podListingDeleted = await this.marketplace.podListing(0, userAddress);
    ...
    it('Buy Partial Listing, Plots Transfer, Balances Update', async function () {
      ...
      await this.marketplace.connect(user2).buyListing(userAddress, 2000, 0, amountBeansBuyingWith);
      ...
      const podListingDeleted = await this.marketplace.podListing(2000, userAddress);
      expect(podListingDeleted.price.toString()).to.equal('0');
      expect(podListingDeleted.amount.toString()).to.equal('0');

      const podListingNew = await this.marketplace.podListing(2500, userAddress);
    ...
    it('Buy Listing with ETH and beans from back of podListing', async function () {
      ...
      await this.marketplace.connect(user).buyBeansAndBuyListing(user2Address, 1000, 0, 0, 50, { value: 15 });

      expect((await this.field.plot(userAddress, 1500)).toString()).to.equal('500');

      expect((await this.field.plot(user2Address, 1000)).toString()).to.equal('500');

      expect((await this.field.plot(userAddress, 1000)).toString()).to.equal('0');

      expect((await this.field.plot(user2Address, 1500)).toString()).to.equal('0');
      ...
      const podListingDeleted = await this.marketplace.podListing(1000, user2Address);
      expect(podListingDeleted.price.toString()).to.equal('0');
      expect(podListingDeleted.amount.toString()).to.equal('0');
    ...
    it('Buy Partial Listing of Partial Plot With ETH and Beans', async function () {
      ...
      await this.pair.simulateTrade('4000', '1000');
      await this.marketplace.connect(user).buyBeansAndBuyListing(user2Address, 5000, 0, 100, 100, { value: 30 });
      ...
      const podListingNew = await this.marketplace.podListing(5400, user2Address);
      expect(podListingNew.price.toString()).to.equal('500000');
      expect(podListingNew.amount.toString()).to.equal('100');

      const podListingDeleted = await this.marketplace.podListing(5000, user2Address);
      expect(podListingDeleted.price.toString()).to.equal('0');
      expect(podListingDeleted.amount.toString()).to.equal('0');
    ...
    it('Fails to buy Listing, not enough ETH used', async function () {
      await this.pair.simulateTrade('4000', '1000');
      await expect(this.marketplace.connect(user2).buyBeansAndBuyListing(userAddress, 4000, 0, 0, 100, { value: 24 })).to.be.revertedWith('UniswapV2Router: EXCESSIVE_INPUT_AMOUNT');
    });

    it('Buy Listing non-listed Index Fails', async function () {
      await expect(this.marketplace.connect(user).buyListing(user2Address, 1001, 0, 999)).to.be.revertedWith('Marketplace: Listing does not exist.');
    });

    it('Buy Listing after expired', async function () {
      await this.field.incrementTotalHarvestableE('2000');
      await expect(this.marketplace.connect(user2).buyListing(userAddress, 0, 0, 500)).to.be.revertedWith('Marketplace: Listing has expired');
    });

    it('Buy Listing not enough pods in plot', async function () {
      await expect(this.marketplace.connect(user2).buyListing(userAddress, 0, 0, 750)).to.be.revertedWith('Marketplace: Plot has insufficient amount.');
    });

    it('Buy Listing not enough pods in listing', async function () {
      await this.marketplace.connect(user).listPlot('0', '500', '0', '500000', '0');
      await expect(this.marketplace.connect(user2).buyListing(userAddress, 0, 0, 500)).to.be.revertedWith('Marketplace: Not enough pods in listing');
    });
  ...
  describe("Cancel Listing", async function () {
    ...
    it('Re-list plot cancels and re-lists', async function () {
      const podListing = await this.marketplace.podListing(3000, user2Address);
      expect(podListing.price).to.equal(100000);
      result = await this.marketplace.connect(user2).listPlot('3000', '1000', '0', '200000', '2000');
      await expect(result).to.emit(this.marketplace, 'PodListingCreated').withArgs(user2Address, '3000', 1000, 0, 200000, 2000);
      await expect(result).to.emit(this.marketplace, 'PodListingCancelled').withArgs(user2Address, '3000');
      const podListingRelisted = await this.marketplace.podListing(3000, user2Address);
    ...
    it('Cancels Listing, Emits Listing Cancelled Event', async function () {
      const podListing = await this.marketplace.podListing(3000, user2Address);
      expect(podListing.price).to.equal(100000);
      expect(podListing.expiry.toString()).to.equal('2000');
      result = (await this.marketplace.connect(user2).cancelListing('3000'));
      const podListingCancelled = await this.marketplace.podListing(3000, user2Address);
  ...
  describe("Buy Offer", async function () {
    ...
    it('Sell Buy Offer cancels podListing', async function () {
      result = await this.marketplace.connect(user2).listOrder('10000', '500000', '2000');
      await this.field.connect(user).sowBeansAndIndex('1000');
      await this.marketplace.connect(user).listPlot('6000', '1000', '0', '50000', '5000');
