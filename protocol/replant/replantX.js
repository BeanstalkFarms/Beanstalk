...
async function replantX(account, _deposits, name, chunkSize = 180, init2 = false, initData = []) {
  ...
  if (init2) {
    ...
    const receipt = await diamondCut
      .connect(account)
      .diamondCut([], initFacetAddress, functionCall);
  ...
  process.stdout.write("\r\x1b[K");
  process.stdout.write(
    `${_deposits.length}/${_deposits.length}: ${getProcessString(1, 1)} gas used: ${strDisplay(totalGasUsed)}`
  );
  console.log("\n");
}
