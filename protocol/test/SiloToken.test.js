...
describe("Silo Token", function () {
  ...
  describe("deposit", function () {
    describe("reverts", function () {
      it("reverts if BDV is 0", async function () {
        await expect(
          this.silo.connect(user).deposit(this.siloToken.address, "0", EXTERNAL)
        ).to.revertedWith("Silo: No Beans under Token.");
      });

      it("reverts if deposits a non whitelisted token", async function () {
        await expect(
          this.silo.connect(user).deposit(this.siloToken2.address, "0", EXTERNAL)
    ...
    describe("single deposit", function () {
      ...
      it("properly updates the total balances", async function () {
        ...
        expect(await this.siloGetters.getGerminatingTotalDeposited(this.siloToken.address)).to.eq(
          "0"
        );
        expect(await this.siloGetters.getTotalDepositedBdv(this.siloToken.address)).to.eq("1000");
        expect(
          await this.siloGetters.getGerminatingTotalDepositedBdv(this.siloToken.address)
        ).to.eq("0");
      ...
      it("properly adds the crate", async function () {
        ...
        const stemOfDeposit = await this.silo.mockSeasonToStem(
          this.siloToken.address,
          toBN(season).sub("2")
        );

        const deposit = await this.siloGetters.getDeposit(
          userAddress,
          this.siloToken.address,
          stemOfDeposit
        );
      ...
      it("emits Deposit event", async function () {
        ...
        await expect(this.result)
          ...
          .withArgs(userAddress, this.siloToken.address, stemOfDeposit, "1000", "1000");
      });
      ...
      it("verifies the grown stalk for deposit is correct", async function () {
        ...
        expect(
          await this.siloGetters.grownStalkForDeposit(
            userAddress,
            this.siloToken.address,
            stemOfDeposit
          )
    ...
    describe("2 deposits 2 users", function () {
      ...
      it("properly adds the crate", async function () {
        ...
        let deposit = await this.siloGetters.getDeposit(
          userAddress,
          this.siloToken.address,
          stemOfDeposit
        );
        expect(deposit[0]).to.eq("1000");
        expect(deposit[1]).to.eq("1000");
        deposit = await this.siloGetters.getDeposit(
          user2Address,
          this.siloToken.address,
          stemOfDeposit
        );
  ...
  describe("withdraw", function () {
    ...
    describe("reverts", function () {
      it("reverts if amount is 0", async function () {
        ...
        await expect(
          this.silo.connect(user).withdrawDeposit(this.siloToken.address, stem, "1001", EXTERNAL)
      ...
      it("reverts if deposits + withdrawals is a different length", async function () {
        await expect(
          this.silo
            .connect(user)
            .withdrawDeposits(this.siloToken.address, ["1", "2"], ["1001"], EXTERNAL)
    ...
    describe("withdraw token by season", async function () {
      describe("withdraw 1 Bean crate", async function () {
        beforeEach(async function () {
          ...
          this.result = await this.silo
            .connect(user)
            .withdrawDeposit(this.siloToken.address, stem, "1000", EXTERNAL);
        });
        ...
        it("properly updates the user balance", async function () {
          ...
          expect((await this.siloToken.balanceOf(userAddress)).sub(userBalanceBefore)).to.eq(
            "1000"
          );
        });
        ...
        it("emits RemoveDeposit event", async function () {
          ...
          const stem = await this.silo.mockSeasonToStem(this.siloToken.address, toBN(season));
          await expect(this.result)
            .to.emit(this.silo, "RemoveDeposit")
            .withArgs(userAddress, this.siloToken.address, stem, "1000", "1000");
        });
      });
      ...
      describe("withdraw part of a bean crate", function () {
        beforeEach(async function () {
          ...
          this.result = await this.silo
            .connect(user)
            .withdrawDeposit(this.siloToken.address, stem, "500", EXTERNAL);
        });
        ...
        it("properly updates the total balances", async function () {
          ...
          expect(
            (
              await this.siloGetters.getGerminatingStalkAndRootsForSeason(
        ...
        it("emits RemoveDeposit event", async function () {
          ...
          await expect(this.result)
            .to.emit(this.silo, "RemoveDeposit")
            .withArgs(userAddress, this.siloToken.address, stem, "500", "500");
        });
      });
    });
    ...
    describe("withdraw token by seasons", async function () {
      describe("1 full and 1 partial token crates", function () {
        beforeEach(async function () {
          ...
          this.result = await this.silo
            .connect(user)
            .withdrawDeposits(this.siloToken.address, [stem0, stem1], ["500", "1000"], EXTERNAL);
        });
      ...
      describe("2 token crates", function () {
        beforeEach(async function () {
          ...
          this.result = await this.silo
            .connect(user)
            .withdrawDeposits(this.siloToken.address, [stem0, stem1], ["1000", "1000"], EXTERNAL);
        });
  ...
  describe("Transfer", async function () {
    describe("reverts", async function () {
      ...
      it("reverts if the amounts array is empty", async function () {
        await expect(
          this.silo
            .connect(user)
            .transferDeposits(userAddress, user2Address, this.siloToken.address, [], [])
    ...
    describe("Single", async function () {
      beforeEach(async function () {
        await this.silo.connect(user).deposit(this.siloToken.address, "100", EXTERNAL);
        this.stem = await this.silo.mockSeasonToStem(
          this.siloToken.address,
          await this.seasonGetters.season()
        );
        ...
        await this.silo
          .connect(user)
          .transferDeposit(userAddress, user2Address, this.siloToken.address, this.stem, "50");
      });
      ...
      it("totalStalk is unchanged", async function () {
        ...
        expect(await this.siloGetters.totalStalk()).to.be.equal(
          toBN(this.previousTotalStalk).add("200")
        );
      });
    });
    ...
    describe("Single all", async function () {
      beforeEach(async function () {
        ...
        await this.silo
          .connect(user)
          .transferDeposit(userAddress, user2Address, this.siloToken.address, this.stem, "100");
      });
    ...
    describe("Multiple", async function () {
      beforeEach(async function () {
        await this.silo.connect(user).deposit(this.siloToken.address, "100", EXTERNAL);
        this.stem10 = await this.silo.mockSeasonToStem(
          this.siloToken.address,
          await this.seasonGetters.season()
        );
        await this.season.siloSunrise("0");
        await this.silo.connect(user).deposit(this.siloToken.address, "100", EXTERNAL);
        this.stem11 = await this.silo.mockSeasonToStem(
          this.siloToken.address,
          await this.seasonGetters.season()
        );
        ...
        this.staticResult = await this.silo
          .connect(user)
    ...
    describe("Single with allowance", async function () {
      beforeEach(async function () {
        ...
        await this.approval
          .connect(user)
          .increaseDepositAllowance(ownerAddress, this.siloToken.address, "100");
        this.stem = await this.silo.mockSeasonToStem(
          this.siloToken.address,
          await this.seasonGetters.season()
        );
      ...
      it("properly updates users token allowance", async function () {
        expect(
          await this.approval.depositAllowance(userAddress, ownerAddress, this.siloToken.address)
        ).to.be.equal("50");
      });
    });
    ...
    describe("Single with no allowance", async function () {
      ...
      it("reverts with no allowance", async function () {
        const stem = await this.silo.mockSeasonToStem(
          this.siloToken.address,
          await this.seasonGetters.season()
        );
        await expect(
          this.silo
            .connect(owner)
            .transferDeposit(userAddress, user2Address, this.siloToken.address, stem, "50")
    ...
    describe("Multiple with allowance", async function () {
      beforeEach(async function () {
        await this.silo.connect(user).deposit(this.siloToken.address, "100", EXTERNAL);
        this.stem1 = await this.silo.mockSeasonToStem(
          this.siloToken.address,
          await this.seasonGetters.season()
        );
        await this.season.siloSunrise("0");
        await this.silo.connect(user).deposit(this.siloToken.address, "100", EXTERNAL);
        this.stem2 = await this.silo.mockSeasonToStem(
          this.siloToken.address,
          await this.seasonGetters.season()
        );
        ...
        await this.approval
          .connect(user)
          .increaseDepositAllowance(ownerAddress, this.siloToken.address, "200");
        await this.silo
          .connect(owner)
          .transferDeposits(
            userAddress,
  ...
  describe("Update Unripe Deposit", async function () {
    it("enrootDeposit fails if not unripe token", async function () {
      await expect(this.enroot.connect(user).enrootDeposit(BEAN, "1", "1")).to.be.revertedWith(
        "Silo: token not unripe"
      );
    });

    it("enrootDeposits fails if not unripe token", async function () {
      await expect(this.enroot.connect(user).enrootDeposits(BEAN, ["1"], ["1"])).to.be.revertedWith(
        "Silo: token not unripe"
      );
    });
    ...
    describe("1 deposit, some", async function () {
      beforeEach(async function () {
        ...
        this.result = await this.enroot
          .connect(user)
          .enrootDeposit(UNRIPE_BEAN, this.stem10, to6("5"));
      });
      ...
      it("properly updates the user balance", async function () {
        expect(await this.siloGetters.balanceOfStalk(userAddress)).to.eq(
          pruneToStalk(to6("10")).add(toStalk("0.5"))
        );
      });
      ...
      it("emits Remove and Add Deposit event", async function () {
        const stem10 = await this.silo.mockSeasonToStem(this.siloToken.address, "10");
        await expect(this.result)
          .to.emit(this.enroot, "RemoveDeposit")
          .withArgs(userAddress, UNRIPE_BEAN, stem10, to6("5"), "927823");
        await expect(this.result)
          .to.emit(this.silo, "AddDeposit")
          .withArgs(userAddress, UNRIPE_BEAN, stem10, to6("5"), prune(to6("5")).add(to6("0.5")));
      });
      ...
      it("audit small unripe bean withdrawals dont decrease BDV and Stalks", async function () {
        ...
        let initialTotalStalk = pruneToStalk(initialUnripeBeanDeposited)
          .add(toStalk("0.5"))
          .mul(toBN(10004))
          .div(toBN(10000));

        // verify initial state
        expect(await this.siloGetters.getTotalDeposited(UNRIPE_BEAN)).to.eq(
          initialUnripeBeanDeposited
        );
        expect(await this.siloGetters.getTotalDepositedBdv(UNRIPE_BEAN)).to.eq(
          initialUnripeBeanDepositedBdv
        );
        ...
        await this.silo
          .connect(user)
          .withdrawDeposit(UNRIPE_BEAN, stem, initialUnripeBeanDeposited, EXTERNAL);
        ...
        await this.silo
          .connect(user)
          .withdrawDeposit(UNRIPE_BEAN, stem, smallWithdrawAmount, EXTERNAL);

        // verify UNRIPE_BEAN totalDeposited has been correctly decreased
        expect(await this.siloGetters.getTotalDeposited(UNRIPE_BEAN)).to.eq(
          initialUnripeBeanDeposited.sub(smallWithdrawAmount)
        );
        // verify unripeBean totalDepositedBDV decreases.
        expect(await this.siloGetters.getTotalDepositedBdv(UNRIPE_BEAN)).to.eq(
          toBN(initialUnripeBeanDepositedBdv).sub(1)
        );
    ...
    describe("1 deposit after 1 season, all", async function () {
      ...
      it("emits Remove and Add Deposit event", async function () {
        ...
        await expect(this.result)
          .to.emit(this.enroot, "RemoveDeposit")
          .withArgs(userAddress, UNRIPE_BEAN, stem10, to6("10"), "1855646");
        await expect(this.result)
          .to.emit(this.silo, "AddDeposit")
          .withArgs(userAddress, UNRIPE_BEAN, stem10, to6("10"), to6("5"));
      });
    });
    ...
    describe("2 deposit, all", async function () {
      ...
      it("emits Remove and Add Deposits event", async function () {
        ...
        await expect(this.result)
          .to.emit(this.silo, "RemoveDeposits")
          .withArgs(userAddress, UNRIPE_BEAN, [stem10, stem11], [to6("5"), to6("5")], to6("10"), [
            "927823",
            "927823"
          ]);
        await expect(this.result)
          .to.emit(this.silo, "AddDeposit")
          .withArgs(userAddress, UNRIPE_BEAN, stem10, to6("5"), to6("2.5"));
        await expect(this.result)
          .to.emit(this.silo, "AddDeposit")
          .withArgs(userAddress, UNRIPE_BEAN, stem11, to6("5"), to6("2.5"));
      });
    });
  });
  ...
  describe("Deposit Approval", async function () {
    describe("approve allowance", async function () {
      beforeEach(async function () {
        this.result = await this.approval
          .connect(user)
          .increaseDepositAllowance(user2Address, this.siloToken.address, "100");
      });

      it("properly updates users token allowance", async function () {
        expect(
          await this.approval.depositAllowance(userAddress, user2Address, this.siloToken.address)
        ).to.be.equal("100");
      });

      it("emits DepositApproval event", async function () {
        await expect(this.result)
          .to.emit(this.approval, "DepositApproval")
          .withArgs(userAddress, user2Address, this.siloToken.address, "100");
      });
    });
    ...
    describe("increase and decrease allowance", async function () {
      ...
      it("properly decrease users token allowance", async function () {
        await this.approval
          .connect(user)
          .decreaseDepositAllowance(user2Address, this.siloToken.address, "25");
        expect(
          await this.approval.depositAllowance(userAddress, user2Address, this.siloToken.address)
        ).to.be.equal("75");
      });
      ...
      it("decrease users token allowance below zero", async function () {
        await expect(
          this.approval
            .connect(user)
            .decreaseDepositAllowance(user2Address, this.siloToken.address, "101")
        ).to.revertedWith("Silo: decreased allowance below zero");
      });

      it("emits DepositApproval event on increase", async function () {
        const result = await this.approval
          .connect(user)
          .increaseDepositAllowance(user2Address, this.siloToken.address, "25");
        await expect(result)
          .to.emit(this.approval, "DepositApproval")
          .withArgs(userAddress, user2Address, this.siloToken.address, "125");
      });
      ...
      it("emits DepositApproval event on decrease", async function () {
        ...
        await expect(result)
          .to.emit(this.approval, "DepositApproval")
          .withArgs(userAddress, user2Address, this.siloToken.address, "75");
      });
    });
    ...
    describe("Approve Deposit Permit", async function () {
      describe("reverts", function () {
        it("reverts if depositPermitDomainSeparator is invalid", async function () {
          expect(await this.approval.connect(user).depositPermitDomainSeparator()).to.be.equal(
      ...
      describe("single token permit", async function () {
        describe("reverts", function () {
          ...
          it("reverts when transfer too much", async function () {
            ...
            await expect(
              this.silo
                .connect(user2)
                .transferDeposit(userAddress, user2Address, this.siloToken.address, stem10, "1000")
        ...
        describe("approve permit", async function () {
          ...
          it("allow transfer all single deposit", async function () {
            ...
            const user1Deposit = await this.siloGetters.getDeposit(
              userAddress,
              this.siloToken.address,
              stem10
            );
            ...
            const user2Deposit = await this.siloGetters.getDeposit(
              user2Address,
              this.siloToken.address,
              stem10
            );
          ...
          it("allow transfer some deposit", async function () {
            ...
            await this.silo
              .connect(user2)
              .transferDeposit(userAddress, user2Address, this.siloToken.address, stem10, "400");
          ...
          it("properly updates user token allowance", async function () {
            expect(
              await this.approval.depositAllowance(
                userAddress,
                user2Address,
                this.siloToken.address
              )
  ...
  describe("flash loan exploit", async function () {
    ...
    beforeEach(async function () {
      ...
      stem = await this.silo.mockSeasonToStem(
        this.siloToken.address,
        toBN(await this.seasonGetters.season())
      );
    ...
    it("does not allocate bean mints to the user", async function () {
      await expect(await this.siloGetters.balanceOfEarnedBeans(flashLoanExploiterAddress)).to.eq(
        "0"
      );
    });
    ...
    it("loses all stalk", async function () {
      await expect(
        await this.siloGetters.balanceOfGrownStalk(
          flashLoanExploiterAddress,
          this.siloToken.address
        )
      ).to.eq("0");
