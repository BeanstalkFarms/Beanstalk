...
contract Order is Listing {
    ...
    */
    function _createPodOrder(
        ...
        id = createOrderId(LibTractor._getUser(), pricePerPod, maxPlaceInLine, minFillAmount);
        ...
        emit PodOrderCreated(LibTractor._getUser(), id, beanAmount, pricePerPod, maxPlaceInLine, minFillAmount, emptyPricingFunction, LibPolynomial.PriceType.Fixed);
    }
    ...
    function _createPodOrderV2(
        ...
        require(beanAmount > 0, "Marketplace: Order amount must be > 0.");
        id = createOrderIdV2(LibTractor._getUser(), 0, maxPlaceInLine, minFillAmount, pricingFunction);
        if (s.podOrders[id] > 0) _cancelPodOrderV2(maxPlaceInLine, minFillAmount, pricingFunction, LibTransfer.To.INTERNAL);
        s.podOrders[id] = beanAmount;

        emit PodOrderCreated(LibTractor._getUser(), id, beanAmount, 0, maxPlaceInLine, minFillAmount, pricingFunction, LibPolynomial.PriceType.Dynamic);
    }
    ...
    function _fillPodOrder(
        ...
        require(s.a[LibTractor._getUser()].field.plots[index] >= (start.add(amount)), "Marketplace: Invalid Plot.");
        ...
        LibTransfer.sendToken(C.bean(), costInBeans, LibTractor._getUser(), mode);
        
        if (s.podListings[index] != bytes32(0)) _cancelPodListing(LibTractor._getUser(), index);
        
        _transferPlot(LibTractor._getUser(), o.account, index, start, amount);

        if (s.podOrders[id] == 0) delete s.podOrders[id];
        
        emit PodOrderFilled(LibTractor._getUser(), o.account, id, index, start, amount, costInBeans);
    }
    ...
    function _cancelPodOrder(
        ...
        bytes32 id = createOrderId(LibTractor._getUser(), pricePerPod, maxPlaceInLine, minFillAmount);
        uint256 amountBeans = s.podOrders[id];
        LibTransfer.sendToken(C.bean(), amountBeans, LibTractor._getUser(), mode);
        delete s.podOrders[id];
        emit PodOrderCancelled(LibTractor._getUser(), id);
    }
    ...
    function _cancelPodOrderV2(
        ...
        bytes32 id = createOrderIdV2(LibTractor._getUser(), 0, maxPlaceInLine, minFillAmount, pricingFunction);
        uint256 amountBeans = s.podOrders[id];
        LibTransfer.sendToken(C.bean(), amountBeans, LibTractor._getUser(), mode);
        delete s.podOrders[id];
