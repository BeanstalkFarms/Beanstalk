...
describe('Governance', function () {
  ...
  beforeEach(async function () {
    ...
    await this.silo.depositSiloAssetsE(userAddress, '500', '1000000', false);
    await this.silo.depositSiloAssetsE(ownerAddress, '500', '1000000', false);
  });

  describe('vote', function () {
    beforeEach(async function () {
      await this.silo.depositSiloAssetsE(user3Address, '500', '1000000', false);
  ...
  describe('vote and withdraw', function () {
    beforeEach(async function () {
      ...
      await this.silo.withdrawSiloAssetsE(userAddress, '500', '500000', false);
    });

    it('reverts when owner withdraws too much', async function () {
      await expect(this.silo.withdrawSiloAssetsE(ownerAddress, '500', '999500', false)).to.be.revertedWith('Silo: Proposer must have min Stalk.');
    });

    it('allows owner to withdraw enough', async function () {
      await this.silo.withdrawSiloAssetsE(ownerAddress, '500', '999400', false);
    ...
    it('roots and stalk are correct after one deposit and withdrawal', async function () {
      await this.silo.depositSiloAssetsE(userAddress, '500', '1000000', false);
      await this.silo.withdrawSiloAssetsE(userAddress, '500', '1000000', false);
      expect(await this.silo.balanceOfStalk(userAddress)).to.eq('500000');
    })

    it('roots and stalk are correct after many deposits and withdrawals', async function () {
      await this.silo.depositSiloAssetsE(userAddress, '500', '1000000', false);
      await this.silo.depositSiloAssetsE(userAddress, '1000', '1000000', false);
      await this.silo.withdrawSiloAssetsE(userAddress, '500', '500000', false);
      await this.silo.withdrawSiloAssetsE(userAddress, '500', '500000', false);
      await this.silo.withdrawSiloAssetsE(userAddress, '500', '500000', false);
      expect(await this.governance.rootsFor(1)).to.be.equal(await this.silo.totalRoots());
    ...
    it('roots and stalk are correct after proposer withdraws under the min required for a bip', async function () {
      await this.silo.withdrawSiloAssetsE(ownerAddress, '500', '500000', false);
      expect(await this.silo.balanceOfStalk(userAddress)).to.eq('500000');
      expect(await this.governance.rootsFor(0)).to.be.equal(await this.silo.balanceOfRoots(ownerAddress));
    })
