...
contract SunTest is TestHelper {
    ...
    function test_sunOnlySilo(int256 deltaB, uint256 caseId) public {
        ...
        uint256 initialBeanBalance = IBean(BEAN).balanceOf(address(bs));
        ...
        if (deltaB >= 0) {
            assertEq(
                IBean(BEAN).balanceOf(address(bs)),
                uint256(deltaB),
        ...
        if (deltaB <= 0) {
            assertEq(
                initialBeanBalance - IBean(BEAN).balanceOf(address(bs)),
                0,
                "invalid bean minted -deltaB"
            );
        }
    ...
    function test_sunFertilizerFieldAndSilo(
        ...
        uint256 beansInBeanstalk = IBean(BEAN).balanceOf(address(bs));
        ...
        if (deltaB >= 0) {
            assertEq(
                IBean(BEAN).balanceOf(address(bs)) - beansInBeanstalk,
                uint256(deltaB),
                "invalid bean minted +deltaB"
            );
    ...
    function test_multipleSunrisesWithTwoFieldsFertilizerAndSilo(
        ...
        for (uint256 i; i < numOfSeasons; i++) {
            ...
            uint256 priorBeansInBeanstalk = IBean(BEAN).balanceOf(address(bs));
            ...
            if (deltaB >= 0) {
                assertEq(
                    IBean(BEAN).balanceOf(address(bs)) - priorBeansInBeanstalk,
                    uint256(deltaB),
                    "invalid bean minted +deltaB"
                );
