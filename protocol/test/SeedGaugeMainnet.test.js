...
testIfRpcSet("SeedGauge Init Test", function () {
  ...
  describe.skip("init state", async function () {
    it("totalDepositedBDV", async function () {
      ...
      console.log("BEAN:", await beanstalk.getTotalDepositedBdv(BEAN));
      console.log("Bean3crv:", await beanstalk.getTotalDepositedBdv(BEAN_3_CURVE));
      ...
      console.log("BeanETH:", await beanstalk.totalMigratedBdv(BEAN_ETH_WELL));
      console.log("Unripe Bean:", await beanstalk.totalMigratedBdv(UNRIPE_BEAN));
      console.log("Unripe LP:", await beanstalk.totalMigratedBdv(UNRIPE_LP));
    });
    ...
    it("totalBDV", async function () {
      // ~44m total BDV
      expect(await beanstalk.getTotalBdv()).to.be.within(to6("43000000"), to6("44000000"));
    });
    ...
    it("bean To MaxLPGpRatio", async function () {
      expect(await beanstalk.getBeanToMaxLpGpPerBdvRatio()).to.be.equal(
        to18("33.333333333333333333")
      );
      expect(await beanstalk.getBeanToMaxLpGpPerBdvRatioScaled()).to.be.equal(
        to18("66.666666666666666666")
      );
    });

    it("lockedBeans", async function () {
      // ~25.5m locked beans, ~35.8m total beans
      expect(await beanstalk.getLockedBeans()).to.be.within(
        to6("25900000.000000"),
        to6("26000000.000000")
      );
    });
    ...
    it("usd Liquidity", async function () {
      ...
      expect(await beanstalk.getTwaLiquidityForWell(BEAN_ETH_WELL)).to.be.within(
        to18("13100000"),
        to18("13300000")
      );
      // ~13.2m usd liquidity in Bean:Eth
      expect(await beanstalk.getTotalUsdLiquidity()).to.be.within(
        to18("13100000"),
        to18("13300000")
      );
    });
  ...
  describe.skip("bean3crv dewhitelisted", async function () {
    ...
    it("reverts on deposit", async function () {
      await expect(
        beanstalk.connect(user).deposit(BEAN_3_CURVE, to18("100"), INTERNAL)
    ...
    it("allows legacy bean3crv to be mown", async function () {
      initalStalk = await beanstalk.balanceOfStalk("0x1B7eA7D42c476A1E2808f23e18D850C5A4692DF7");
      await beanstalk.mow("0x1B7eA7D42c476A1E2808f23e18D850C5A4692DF7", BEAN_3_CURVE);
      newStalk = await beanstalk.balanceOfStalk("0x1B7eA7D42c476A1E2808f23e18D850C5A4692DF7");
  ...
  describe.skip("silo v3.1 migration", async function () {
    ...
    it("correctly updates lastStem for a user", async function () {
      ...
      expect(await beanstalk.getLastMowedStem(testAccount, BEAN)).to.equal("12648");
      expect(await beanstalk.getLastMowedStem(testAccount, BEAN_3_CURVE)).to.equal("5927");
      expect(await beanstalk.getLastMowedStem(testAccount, BEAN_ETH_WELL)).to.equal("15372");
      expect(await beanstalk.getLastMowedStem(testAccount, UNRIPE_BEAN)).to.equal("0");
      expect(await beanstalk.getLastMowedStem(testAccount, UNRIPE_LP)).to.equal("0");

      await beanstalk.mow("0x43a9dA9bAde357843fBE7E5ee3Eedd910F9fAC1e", BEAN);

      expect(await beanstalk.getLastMowedStem(testAccount, BEAN)).to.equal(to6("12699"));
      expect(await beanstalk.getLastMowedStem(testAccount, BEAN_3_CURVE)).to.equal(to6("5927"));
      expect(await beanstalk.getLastMowedStem(testAccount, BEAN_ETH_WELL)).to.equal(to6("15372"));
      expect(await beanstalk.getLastMowedStem(testAccount, UNRIPE_BEAN)).to.equal("0");
      expect(await beanstalk.getLastMowedStem(testAccount, UNRIPE_LP)).to.equal("0");
    });
    ...
    it("correctly updates a deposit for user", async function () {
      ...
      this.result = await beanstalk
        .connect(account)
        .transferDeposit(account.address, user.address, BEAN, 12648000000, to6("1"));
      ...
      await expect(this.result)
        ...
        .withArgs(account.address, BEAN, "12648", "130214066", "130214066");
      ...
      await expect(this.result)
        ...
        .withArgs(account.address, BEAN, "12648000000", "130214066", "130214066");
    });
  });
});
