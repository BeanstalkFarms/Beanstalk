...
contract MarketplaceFacet is Marketplace {
    ...
    **/
    ...
    function listBuyOffer(uint232 maxPlaceInLine, uint24 pricePerPod, uint256 amountBeans) public returns (bytes20 blobId) {
        bean().transferFrom(msg.sender, address(this), amountBeans);
        return _listBuyOffer(maxPlaceInLine, pricePerPod, amountBeans);
    }

    function claimAndListBuyOffer(uint232 maxPlaceInLine, uint24 pricePerPod, uint256 amountBeans, LibClaim.Claim calldata claim) public  returns (bytes20 blobId) {
        allocateBeans(claim, amountBeans, address(this));
        return _listBuyOffer(maxPlaceInLine, pricePerPod, amountBeans);
    }

    function buyBeansAndListBuyOffer(uint232 maxPlaceInLine, uint24 pricePerPod, uint256 amountBeans, uint256 buyBeanAmount) public payable returns (bytes20 blobId) {
    ...
    function claimAndBuyBeansAndListBuyOffer(uint232 maxPlaceInLine, uint24 pricePerPod, uint256 amountBeans, uint256 buyBeanAmount, LibClaim.Claim calldata claim) public payable returns (bytes20 blobId) {
        allocateBeans(claim, amountBeans, address(this));
        return _buyBeansAndListBuyOffer(maxPlaceInLine, pricePerPod, amountBeans, buyBeanAmount);
    }

    function _buyBeansAndListBuyOffer(uint232 maxPlaceInLine, uint24 pricePerPod, uint256 amountBeans, uint256 buyBeanAmount) internal returns (bytes20 blobId) {
    ...
    function _listBuyOffer(uint232 maxPlaceInLine, uint24 pricePerPod, uint256 amountBeans) internal returns (bytes20 blobId) {
        require(0 < pricePerPod, "Marketplace: Pod price must be greater than 0.");
        uint256 amount = (amountBeans * 1000000) / pricePerPod;
        return  __listBuyOffer(maxPlaceInLine,pricePerPod,amount);
    }

    function buyOffer(bytes20 index) public view returns (Storage.BuyOffer memory) {
