...
const { BEAN, USDT, WETH, CURVE_REGISTRY, CRYPTO_REGISTRY, THREE_POOL, TRI_CRYPTO, TRI_CRYPTO_POOL, THREE_CURVE, BEAN_3_CURVE, USDC, WBTC, DAI, LUSD_3_CURVE, LUSD, MAX_UINT256 } = require('./utils/constants')
...
testIfRpcSet('Farm', function () {
    before(async function () {
      ...
      wrapEth = await beanstalk.interface.encodeFunctionData(
    ...
    describe("Farm Revert", async function () {
      it('revert if not valid selector', async function () {
        await expect(beanstalk.farm(['0xfd9f1e10'])).to.be.revertedWith('Diamond: Function does not exist')
      })
    ...
    describe.skip('Farm Exchange', async function () {
      describe('tri-crypto', async function () {
        ...
        it('wraps Eth and exchanges to usdt internal', async function () {
          ...
          expect(await beanstalk.getInternalBalance(user.address, this.usdt.address)).to.be.equal('3043205584')
      ...
      describe('weth:crv', async function () {
        it('Wrap Eth, Exchange WETH -> CRV internal', async function () {
          ...
          expect(await beanstalk.getInternalBalance(user.address, this.crv.address)).to.be.equal('1338512415451289655561')
    ...
    describe.skip("Farm Exchange Underlying", async function () {
      ...
      it("Wrap Eth, Exchange WETH -> USDT, Exchange USDT -> LUSD internal", async function () {
        ...
        expect(await beanstalk.getInternalBalance(user.address, this.lusd.address)).to.be.equal('99791486902027823650')
    ...
    describe.skip("Farm Liquidity ", async function () {
      ...
      describe("tri-crypto", async function () {
        ...
        it('Wraps Eth, Adds WETH as tri-crypto internal', async function () {
          ...
          expect(await this.triCrypto.balanceOf(beanstalk.address)).to.be.equal('2019589947833455380')
          expect(await beanstalk.getInternalBalance(user.address, this.triCrypto.address)).to.be.equal('2019589947833455380')
        ...
        it('Wraps Eth, Adds WETH as tri-crypto internal, removes tri-crypto liquidity 1 token to WETH', async function () {
          ...
          expect(await beanstalk.getInternalBalance(user.address, this.weth.address)).to.be.equal('999353626969234502')
        ...
        it('Wraps Eth, Adds WETH as tri-crypto internal, removes tri-crypto liquidity equally', async function () {
          ...
          expect(await beanstalk.getInternalBalance(user.address, this.weth.address)).to.be.equal('332554978398190452')
          expect(await this.weth.balanceOf(user.address)).to.be.equal('0')

          expect(await this.usdt.balanceOf(beanstalk.address)).to.be.equal('1019226369')
          expect(await beanstalk.getInternalBalance(user.address, this.usdt.address)).to.be.equal('1019226369')
          expect(await this.usdt.balanceOf(user.address)).to.be.equal('0')

          expect(await this.wbtc.balanceOf(beanstalk.address)).to.be.equal('2504936')
          expect(await beanstalk.getInternalBalance(user.address, this.wbtc.address)).to.be.equal('2504936')
        ...
        it('Wraps Eth, Adds WETH as tri-crypto internal, removes tri-crypto liquidity imbalance', async function () {
          ...
          await expect(beanstalk.connect(user).farm([wrapEth, addLP, removeLPImb], { value: to18('1') })).to.be.revertedWith('Curve: tri-crypto not supported')
        })
      })
      describe("3-pool", async function () {
        ...
        it('Wraps Eth, Exchange WETH -> USDT, add USDT as 3CRV internal', async function () {
          ...
          expect(await this.threeCurve.balanceOf(beanstalk.address)).to.be.equal('2981268357742150365108')
          expect(await beanstalk.getInternalBalance(user.address, this.threeCurve.address)).to.be.equal('2981268357742150365108')
        ...
        it('Wraps Eth, Exchange WETH -> USDT, add USDT as 3CRV, removes 3CRV equally', async function () {
          ...
          await beanstalk.connect(user).farm([wrapEth, exchange, addLP, removeLP], { value: to18('1') })

          expect(await this.usdc.balanceOf(beanstalk.address)).to.be.equal('1096956614')
          expect(await beanstalk.getInternalBalance(user.address, this.usdc.address)).to.be.equal('1096956614')
          ...
          expect(await beanstalk.getInternalBalance(user.address, this.usdt.address)).to.be.equal('727891990')
          expect(await this.usdt.balanceOf(user.address)).to.be.equal('0')

          expect(await this.dai.balanceOf(beanstalk.address)).to.be.equal('1218092928789910236576')
          expect(await beanstalk.getInternalBalance(user.address, this.dai.address)).to.be.equal('1218092928789910236576')
        ...
        it('Wraps Eth, Exchange WETH -> USDT, add USDT as 3CRV, removes 3CRV imbalance', async function () {
          ...
          await beanstalk.connect(user).farm([wrapEth, exchange, addLP, removeLPImb], { value: to18('1') })

          expect(await this.usdc.balanceOf(beanstalk.address)).to.be.equal(to6('100'))
          expect(await beanstalk.getInternalBalance(user.address, this.usdc.address)).to.be.equal(to6('100'))
          expect(await this.usdc.balanceOf(user.address)).to.be.equal('1000000')

          expect(await this.usdt.balanceOf(beanstalk.address)).to.be.equal(to6('100'))
          expect(await beanstalk.getInternalBalance(user.address, this.usdt.address)).to.be.equal(to6('100'))
          expect(await this.usdt.balanceOf(user.address)).to.be.equal('0')

          expect(await this.dai.balanceOf(beanstalk.address)).to.be.equal(to6('100'))
          expect(await beanstalk.getInternalBalance(user.address, this.dai.address)).to.be.equal(to6('100'))
          expect(await this.dai.balanceOf(user.address)).to.be.equal('0')
        })
      })
    })
