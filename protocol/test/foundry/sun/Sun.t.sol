...
contract SunTest is Sun, TestHelper {
    ...
    function _testSunrise(
        ...
        internal
        returns (uint256 toFert, uint256 toField, uint256 toSilo, uint256 newHarvestable, uint256 soil)
    {
        uint256 caseId = 8; // need to fix
        toFert = hasFert ? newBeans.div(3) : uint256(0); //
        toField = hasField ? newBeans.sub(toFert).div(2) : uint256(0);
        // divide remainder by two, round down
    ...
    function test_deltaB_positive_podRate_low() public {
        field.incrementTotalPodsE(10_000);
        season.setAbovePegE(true);
        season.sunSunrise(30_000, 0); // deltaB = +300; case 0 = low pod rate
        vm.roll(30); // after dutch Auction
        assertEq(uint256(field.totalSoil()), 14_850);
        // 300/3 = 100 *1.5 = 150
    }
    ...
    function test_deltaB_positive_podRate_medium() public {
        field.incrementTotalPodsE(10_000);
        season.setAbovePegE(true);
        season.sunSunrise(30_000, 8); // deltaB = +300; case 0 = medium pod rate
        vm.roll(30); // after dutch Auction
    ...
    function test_deltaB_positive_podRate_high() public {
        field.incrementTotalPodsE(10_000);
        season.setAbovePegE(true);
        season.sunSunrise(30_000, 25); // deltaB = +300; case 0 = high pod rate
        vm.roll(30); // after dutch Auction
