...
describe("Silo", function () {
  ...
  describe("Silo Balances After Withdrawal", function () {
    beforeEach(async function () {
      await this.silo
        .connect(user)
        .withdrawDeposit(this.bean.address, to6("2"), to6("500"), EXTERNAL); //we deposited at grownStalkPerBdv of 2, need to withdraw from 2
    });
  ...
  describe("Silo Sunrise", async function () {
    describe("Single", async function () {
      ...
      it("properly updates the earned balances", async function () {
        expect(await this.siloGetters.balanceOfGrownStalk(userAddress, this.bean.address)).to.eq(
          toStalk("0.6")
        );
      ...
      it("properly updates the total balances", async function () {
        ...
        expect(await this.siloGetters.balanceOfRoots(userAddress)).to.eq(
          toStalk("1000000000000000")
        );
      });
  ...
  describe("Single Earn", async function () {
    ...
    it("user2 earns rest", async function () {
      ...
      expect(await this.siloGetters.balanceOfRoots(user2Address)).to.eq(
        "10005714285714285714285714"
      );
    });
    ...
    it("user can withdraw earned beans", async function () {
      ...
      await this.silo
        .connect(user)
        .withdrawDeposit(this.bean.address, stemTip, to6("25"), EXTERNAL);
      ...
      await this.silo
        .connect(user)
        .withdrawDeposit(this.bean.address, stemTip, to6("50"), EXTERNAL);
    });
    ...
    it("user can withdraws earned beans", async function () {
      ...
      await this.silo
        .connect(user)
        .withdrawDeposits(this.bean.address, [stemTip], [to6("25")], EXTERNAL);

      // add Bean deposit, such that the stem tip matches with the earned beans, and verify withdraw.
      await this.silo.connect(user).deposit(this.bean.address, to6("25"), EXTERNAL);
      await this.silo
        .connect(user)
        .withdrawDeposits(this.bean.address, [stemTip], [to6("50")], EXTERNAL);
    });
    ...
    it("user can withdraws multiple earned beans", async function () {
      ...
      await this.silo
        .connect(user)
        .withdrawDeposits(this.bean.address, [stemTip, stemTip1], [to6("50"), to6("50")], EXTERNAL);
    });
    ...
    it("user can transfer earned beans", async function () {
      ...
      await this.silo
        .connect(user)
        .transferDeposit(userAddress, user2Address, this.bean.address, stemTip, to6("25"));
      // add Bean deposit, such that the stem tip matches with the earned beans, and verify withdraw.
      await this.silo.connect(user).deposit(this.bean.address, to6("25"), EXTERNAL);
      await this.silo
        .connect(user)
        .transferDeposit(userAddress, user2Address, this.bean.address, stemTip, to6("50"));
    });
    ...
    it("user can transferDeposits earned beans", async function () {
      ...
      await this.silo
        .connect(user)
        .transferDeposits(userAddress, user2Address, this.bean.address, [stemTip], [to6("50")]);
    });
    ...
    it("user can transferDeposits earned beans", async function () {
      ...
      await this.silo
        .connect(user)
        .transferDeposits(userAddress, user2Address, this.bean.address, [stemTip], [to6("25")]);
  ...
  describe("ERC1155 Deposits", async function () {
    ...
    it("mints an ERC1155 when depositing an whitelisted asset", async function () {
      ...
      await expect(this.result)
        .to.emit(this.silo, "TransferSingle")
        .withArgs(user3Address, ethers.constants.AddressZero, user3Address, depositID, to6("1000"));
    });
    ...
    it("removes ERC1155 balance when withdrawing an whitelisted asset", async function () {
      ...
      this.result = await this.silo
        .connect(user)
        .withdrawDeposit(this.bean.address, stem, to6("500"), EXTERNAL);
      await expect(this.result).to.emit(this.silo, "TransferSingle").withArgs(
        userAddress, // operator
        userAddress, // from
    ...
    it("transfers an ERC1155 deposit", async function () {
      ...
      this.result = await this.silo
        .connect(user)
        .safeTransferFrom(userAddress, user3Address, depositID, to6("1000"), 0x00);
  ...
  describe("ERC1155 Metadata", async function () {
    ...
    it("reverts if the depositID is invalid", async function () {
      ...
      await expect(this.metadata.uri(invalidID0)).to.be.revertedWith(
        "Silo: metadata does not exist"
      );
      await expect(this.metadata.uri(invalidID1)).to.be.revertedWith(
        "Silo: metadata does not exist"
      );
    });
  });
  ...
  describe("germination", async function () {
    ...
    describe("deposits", async function () {
      it("properly updates the user balances", async function () {
        expect(await this.siloGetters.balanceOfGerminatingStalk(user3.address)).to.eq(
          toStalk("1000")
        );
        expect(await this.siloGetters.balanceOfGerminatingStalk(user4.address)).to.eq(
          toStalk("1000")
        );
      });
      ...
      it("emit events", async function () {
        ...
        expect(this.result)
          .to.emit(this.silo, "FarmerGerminatingStalkBalanceChanged")
          .withArgs(user4.address, toStalk("1000"));
        expect(this.result)
          .to.emit(this.silo, "TotalGerminatingBalanceChanged")
          .withArgs("3", BEAN, to6("1000"), to6("1000"));
        expect(this.result)
          .to.emit(this.silo, "TotalGerminatingStalkChanged")
    ...
    describe("withdraw", async function () {
      beforeEach(async function () {
        this.result = await this.silo
          .connect(user4)
          .withdrawDeposit(this.bean.address, to6("6"), to6("1000"), EXTERNAL);
      });
      it("properly updates the user balances", async function () {
