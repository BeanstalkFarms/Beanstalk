...
async function deployBasinAndIntegrationBip(
  mock,
  bipAccount = undefined,
  basinAccounts = undefined,
  priceContractAccount = undefined
) {
...
async function deployBasin(mock = true, accounts = undefined, verbose = true, justDeploy = false) {
  ...
  const aquifer = await deployWellContractAtNonce(
    "Aquifer",
    AQUIFER_DEPLOY_NONCE,
    [],
    account,
    verbose
  );
  ...
  const constantProduct2 = await deployWellContractAtNonce(
    "ConstantProduct2",
  ...
  let multiFlowPump = await deployWellContractAtNonce(
    "MultiFlowPump",
  ...
  const wellImplementation = await deployWellContractAtNonce(
    "Well",
    WELL_IMPLEMENTATION_DEPLOY_NONCE,
    [],
    account,
    false
  );
  ...
  const initData = wellImplementation.interface.encodeFunctionData("init", [
    WELL_NAME,
    WELL_SYMBOL
  ]);
  ...
  const well = await getWellContractAt(
    ...
    await aquifer
      .connect(account)
      .callStatic.boreWell(wellImplementation.address, immutableData, initData, WELL_DEPLOY_SALT)
  );

  const wellTxn = await aquifer
    .connect(account)
    .boreWell(wellImplementation.address, immutableData, initData, WELL_DEPLOY_SALT);
  ...
  const instantaneousReserves = await multiFlowPump.readInstantaneousReserves(well.address, "0x");
...
async function deployBasinWithMockPump(
  mock = true,
  accounts = undefined,
  verbose = true,
  justDeploy = false
  ...
  if (verbose) console.log("Pump Statistics:");
  const instantaneousReserves = await mockPump.readInstantaneousReserves(well.address, "0x");
  if (verbose) console.log("Instantaneous Bean Reserve:", instantaneousReserves[0].toString());
  if (verbose) console.log("Instantaneous WETH Reserve:", instantaneousReserves[1].toString());
