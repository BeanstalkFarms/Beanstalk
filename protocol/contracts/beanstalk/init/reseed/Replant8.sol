...
contract Replant8 is InitWhitelist {
    ...
    //Bean 3Crv Pool
    OldCurveFactory constant OLD_FACTORY =
        OldCurveFactory(0x0959158b6040D32d04c301A72CBFD6b39E21c9AE);
    address constant BASEPOOL = 0xbEbc44782C7dB0a1A60Cb6fe97d0b483032FF1C7;
    uint256 constant A = 1;
    uint256 constant FEE = 4000000;
    NewCurveFactory constant NEW_FACTORY =
        NewCurveFactory(0xB9fC157394Af804a3578134A6585C0dc9cc990d4);
    uint256 constant MIN_LP_OUT = 200e6;

    bytes32 constant UNRIPE_BEAN_SALT =
        0x3236313439353338320000000000000000000000000000000000000000000000;
    ...
    bytes32 constant UNRIPE_BEAN3CRV_SALT =
        0x3131313133343838393900000000000000000000000000000000000000000000;
    ...
    bytes32 constant UNRIPE_BEAN_MERKLE_ROOT =
        0x4387a1240f0b31c891fabe2eb6dabe6089dd3eba0a8730f7da6d6d3580e8880a;
    bytes32 constant UNRIPE_LP_MERKLE_ROOT =
        0xbcad44f78fd24cc53104883e6c8badbc8dbd1c60283c85d3a584f9215f2cb086;
    ...
    function init() external {
        ...
        address metapool = OLD_FACTORY.deploy_metapool(
            BASEPOOL,
            NAME,
            SYMBOL,
            address(bean),
        ...
        uint256 newLP = C.curveZap().add_liquidity(metapool, [INITIAL_LP, 0, INITIAL_LP, 0], 0);

        IERC20(metapool).transfer(msg.sender, newLP);

        BeanstalkERC20 ub = new BeanstalkERC20{salt: UNRIPE_BEAN_SALT}(
            address(this),
            "Unripe Bean",
            "urBEAN"
        );
        ub.mint(
            address(this),
            NON_DEPOSITED_UNRIPE_BEANS + DEPOSITED_UNRIPE_BEANS + EARNED_UNRIPE_BEANS
        );
        s.siloBalances[address(ub)].deposited = uint128(
            DEPOSITED_UNRIPE_BEANS + EARNED_UNRIPE_BEANS
        );
        addUnripeToken(address(ub), address(bean), UNRIPE_BEAN_MERKLE_ROOT);

        BeanstalkERC20 ub3 = new BeanstalkERC20{salt: UNRIPE_BEAN3CRV_SALT}(
            address(this),
            "Unripe BEAN3CRV",
            "urBEAN3CRV"
        );
    ...
    function addUnripeToken(address unripeToken, address underlyingToken, bytes32 root) private {
