...
describe("BeanstalkPrice", function () {
  before(async function () {
    ...
    await this.threeCurve
      .connect(user)
      .approve(this.beanThreeCurve.address, ethers.constants.MaxUint256);
    ...
    await this.beanThreeCurve
      .connect(user)
      .approve(this.threeCurve.address, ethers.constants.MaxUint256);
    await this.beanThreeCurve
      .connect(user)
  ...
  describe("Price", async function () {
    ...
    it("deltaB > 0, wells only", async function () {
      ...
      await setReserves(owner, this.well, [to6("500000"), to18("1000")]);
      await advanceTime(1800);
