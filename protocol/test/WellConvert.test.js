...
const { deployWell, setReserves, whitelistWell, impersonateBeanEthWell, deployMockWell } = require('../utils/well.js');
...
describe('Well Convert', function () {
  ...
  describe('calculates beans to peg', async function () {
    it('p > 1', async function () {
      ...
      expect(await beanstalk.getAmountOut(BEAN, this.well.address, maxAmountIn)).to.be.equal('3338505354221892343955');
    });
  ...
  describe('calclates lp to peg', async function () {
    ...
    it('p < 1', async function () {
      ...
      expect(await beanstalk.getAmountOut(this.well.address, BEAN, maxAmountIn)).to.be.equal(to6('200000'));
    });
  })
  ...
  describe('convert beans to lp', async function () {
    describe('p > 1', async function () {
      ...
      it('reverts if not whitelisted well', async function () {
        ...
        await expect(mockBeanstalk.connect(owner).convertInternalE(
          bean.address,
          to6('100000'),
          convertData
      ...
      it('deposit and convert below max', async function () {
        ...
        await mockBeanstalk.siloSunrise('0')
        await beanstalk.connect(owner).convert(
          convertData,
          ['0'],
          [to6('100000')]
        )
      ...
      it('reverts when USD oracle is broken', async function () {
        ...
        const convertData = ConvertEncoder.convertBeansToWellLP(to6('100000'), '1338505354221892343955', this.well.address)
        await expect(mockBeanstalk.connect(owner).callStatic.convertInternalE(
          bean.address,
          to6('100000'),
          convertData
