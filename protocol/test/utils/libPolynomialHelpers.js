...
function evaluatePolynomial(f, x, pieceIndex) {
  ...
  while (degreeIndex <= 3) {
    ...
    const coefExponent = math.pow(
      math.bignumber(10),
      math.bignumber(f.exponents[math.add(pieceIndex * 4, degreeIndex)])
    );
    const term = math.floor(
      math.chain(_x).pow(degreeIndex).multiply(coefValue).divide(coefExponent).done()
    );
...
function evaluatePolynomialIntegration(f, start, end, pieceIndex) {
  ...
  while (degreeIndex <= 3) {
    ...
    var coefExponent = math.pow(
      math.bignumber(10),
      math.bignumber(f.exponents[math.add(pieceIndex * 4, degreeIndex)])
    );

    if (f.signs[pieceIndex * 4 + degreeIndex] == 1) {
      positiveSum = math.add(
        positiveSum,
        math.floor(
          math
            .chain(_x2)
...
function getAmountOrder(f, placeInLine, amountPodsFromOrder) {
  ...
  while (!integrateToEnd) {
    ...
    beanAmount = math.add(
      beanAmount,
      math.bignumber(evaluatePolynomialIntegration(f, start, endIntegration, currentPieceIndex))
    );
    ...
    if (!integrateToEnd) {
      ...
      if (currentPieceIndex == f.numPieces - 1) {
        ...
        currentPieceIndex++;
        if (currentPieceIndex != f.numPieces - 1)
          nextPieceStart = math.bignumber(f.breakpoints[currentPieceIndex + 1]);
      }
    }
  }
