...
contract BeanstalkDeployer is Utils {
    ...
    function forkMainnetAndUpgradeAllFacets(uint256 blockNumber) internal {
        ...
        IDiamondLoupe.Facet[] memory currentFacets = IDiamondLoupe(LibConstant.BEANSTALK).facets();
        ...
        for (uint256 i = 0; i < currentFacets.length; i++) {
            ...
            bytes4[] memory selectors = IDiamondLoupe(LibConstant.BEANSTALK).facetFunctionSelectors(
        ...
        vm.startPrank(IMockFBeanstalk(LibConstant.BEANSTALK).owner());
        // perform the diamond cut (upgrades Beanstalk)
        IDiamondCut(LibConstant.BEANSTALK).diamondCut(cut, address(0), new bytes(0));
        vm.stopPrank();
    }
