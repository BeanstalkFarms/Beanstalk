...
/**
 * @author Brean
 * @title CaseFacet stores the data for each case.
 ...
 * L_n = mL * L_n-1 + bL
 * 
 * @dev CaseFacet uses constants order to minimize gas costs.
 * A seperate facet was used due to contract size constraints.
...
library LibCases {
    ...
    function getDataFromCase(uint256 caseId) public pure returns (bytes8 caseData){
        if(caseId < 64){
            if(caseId < 32) {
                if(caseId < 16) {
                    if(caseId < 8) {
