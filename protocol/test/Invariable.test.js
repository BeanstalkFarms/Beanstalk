...
const { BEAN, MAX_UINT256, BEAN_ETH_WELL, WETH, UNRIPE_BEAN, UNRIPE_LP, ZERO_BYTES } = require("./utils/constants");
...
const { initalizeUsersForToken, endGermination, endGerminationWithMockToken, setRecapitalizationParams } = require('./utils/testHelpers.js')
...
describe('Invariants', function () {
  ...
  before(async function () {
    ...
    await beanstalk.connect(user).deposit(BEAN, to6('100000'), EXTERNAL)
    await beanstalk.connect(user).deposit(BEAN, to6('50000'), EXTERNAL)
    await beanstalk.connect(user).deposit(UNRIPE_BEAN, to6('10000'), EXTERNAL)
    await beanstalk.connect(user).deposit(UNRIPE_LP, to6('10000'), EXTERNAL)
    ...
    await mockBeanstalk.mockEndTotalGerminationForToken(BEAN);
    await mockBeanstalk.mockEndTotalGerminationForToken(UNRIPE_BEAN);
    await mockBeanstalk.mockEndTotalGerminationForToken(UNRIPE_LP);
  });
  ...
  describe("Reverts exploits", async function () {
    ...
    it("reverts at internal accounting exploit", async function () {
      // await expect(mockBeanstalk.exploitUserInternalTokenBalance()).to.be.revertedWith("INV: Insufficient token balance");
      // await expect(mockBeanstalk.exploitUserSendTokenInternal()).to.be.revertedWith("INV: Insufficient token balance");
      // await expect(mockBeanstalk.exploitFertilizer()).to.be.revertedWith("INV: Insufficient token balance");
      // // mockBeanstalk.mockSetSopWell(BEAN_ETH_WELL);
      await expect(mockBeanstalk.exploitSop(this.well.address)).to.be.revertedWith("INV: Insufficient token balance");
    })


    // it("reverts at token flow exploit", async function () {
    //   await expect(mockBeanstalk.exploitTokenBalance()).to.be.revertedWith("INV: noNetFlow Token balance changed");
    //   await expect(mockBeanstalk.exploitUserSendTokenExternal0()).to.be.revertedWith("INV: noNetFlow Token balance changed");
    //   await expect(mockBeanstalk.exploitUserSendTokenExternal1()).to.be.revertedWith("INV: noOutFlow Token balance decreased");
    //   await expect(mockBeanstalk.exploitUserDoubleSendTokenExternal()).to.be.revertedWith("INV: oneOutFlow multiple token balances decreased");
    //   await expect(mockBeanstalk.exploitBurnStalk0()).to.be.revertedWith("INV: noNetFlow Stalk decreased");
    //   await expect(mockBeanstalk.exploitBurnStalk1()).to.be.revertedWith("INV: noOutFlow Stalk decreased");
    // })
