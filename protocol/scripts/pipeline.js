var fs = require("fs");
const { PIPELINE, PIPELINE_DEPLOYER } = require("../test/hardhat/utils/constants");
...
async function impersonate(verbose = false) {
  ...
  await network.provider.send("hardhat_setCode", [PIPELINE, JSON.parse(json).deployedBytecode]);
