...
contract MockBean3Curve {
    ...
    uint256 constant rate_multiplier = 1e30;
    ...
    function add_liquidity(uint256[] memory amounts, uint256 min_mint_amount) external returns (uint256) {
        ...
        for (uint256 i = 0; i < amounts.length; i++) {
            ...
            new_balances[i] = new_balances[i].add(amount);
        }
        _update();
        balances = new_balances;
        ...
        for (uint256 i = 0; i < amounts.length; i++) {
            uint256 amount = amounts[i];
            console.log("Amount: %s, Transfer: %s", amount, IERC20(coins[i]).balanceOf(address(this)));
            if (amount > 0)
                IERC20(coins[i]).transferFrom(msg.sender, address(this), amount);  // dev: failed transfer
        }

        //  Mint pool tokens
        supply = supply.add(mint_amount);
        _balanceOf[msg.sender] = _balanceOf[msg.sender].add(mint_amount);
        console.log("Mint: %s", mint_amount);
        return mint_amount;
    }
    ...
    function _calc_withdraw_one_coin(uint256 _burn_amount, int128 _i_, uint256[N_COINS] memory _balances) internal returns (uint256, uint256) {
...
//     return _calc_withdraw_one_coin(_burn_amount, i, balances)[0]
    ...
    function _xp_mem(uint256[N_COINS] memory _rates, uint256[N_COINS] memory _balances) private returns (uint256[N_COINS] memory result) {
        for (uint256 i = 0; i < N_COINS; i++) {
            result[i] = _rates[i] * _balances[i] / PRECISION;
        }
    }

    function get_D(uint256[N_COINS] memory xp, uint256 a) private returns (uint D) {
    ...
    function get_y_D(uint256 A, uint256 i, uint256[N_COINS] memory xp, uint256 D) private pure returns (uint256) {
