...
export class Convert extends StepClass<BasicPreparedResult> {
  ...
  async run(_amountInStep: ethers.BigNumber, context: RunContext) {
    ...
    return {
      ...
      prepare: () => {
        ...
        return {
          ...
          callData: Convert.sdk.contracts.beanstalk.interface.encodeFunctionData("convert", [
            siloConvert.calculateEncoding(this._tokenIn, this._tokenOut, amountIn, minAmountOut),
      ...
      decode: (data: string) =>
        Convert.sdk.contracts.beanstalk.interface.decodeFunctionData("convert", data),
      decodeResult: (result: string) =>
        Convert.sdk.contracts.beanstalk.interface.decodeFunctionResult("convert", result)
    };
  }
}
