...
const draftConvertBeanToBeanEthWell = async () => {
    ...
    advancedFarmCalls.push({
        callData: await wrapExternalCall(
            BEAN_ETH_WELL,
            this.well.interface.encodeFunctionData("addLiquidity", [[to6('200'), to18('0')], ethers.constants.Zero, PIPELINE, ethers.constants.MaxUint256])
        ),
        clipboard: ethers.utils.hexlify("0x000000")
    });
...
const draftConvertBeanEthWellToUDSTViaCurveTricryptoThenToBeanVia3Crv = async (amountOfLpToRemove, minTokenAmountOut) => {
    ...
    wethToUsdtPipelineData = encodeAdvancedData(1, 0, [2, SLOT_SIZE, SELECTOR_SIZE+SLOT_SIZE*3])

    // exchange WETH -> USDT
    pipe3 = [this.curveTricryptoPool.address, selector2, wethToUsdtPipelineData]
    ...
    // put into 2nd (3rd) param
    usdtToBeanPipelineData = encodeAdvancedData(1, 0, [4, SLOT_SIZE, SELECTOR_SIZE+SLOT_SIZE*3])

    // exchange USDT -> BEAN
    pipe4 = [this.curveBean3crvPool.address, selector3, usdtToBeanPipelineData]

    // get balance of BEAN in pipeline
    ...
    advancedFarm1 = await this.beanstalk.interface.encodeFunctionData(
        "advancedPipe",
        [
            [pipe0, pipe1, pipe2, pipe3, usdtPipe, pipe4, beanPipe],
            0
        ]
    )
...
const draftConvertBeanEthWellToUDSCViaUniswapThenToBeanVia3Crv = async (amountOfLpToRemove, minTokenAmountOut) => {
    ...
    // paste to amountIn index 3 (SLOT_SIZE*3)
