...
library LibPolynomial {
    ...
    */
    ...
    function evaluatePolynomial(
        ...
        for (uint256 degree; degree <= MAX_DEGREE; ++degree) {
            if (signs[degree]) {
                positiveSum = positiveSum.add(
                    pow(x, degree).mul(significands[degree]).div(pow(10, exponents[degree]))
                );
            } else {
                negativeSum = negativeSum.add(
                    pow(x, degree).mul(significands[degree]).div(pow(10, exponents[degree]))
                );
            }
        }
    ...
    function evaluatePolynomialIntegration(
        ...
        for (uint256 degree; degree <= MAX_DEGREE; ++degree) {
            if (signs[degree]) {
                ...
                positiveSum = positiveSum.add(
                    pow(end, 1 + degree).mul(significands[degree]).div(
                        pow(10, exponents[degree]).mul(1 + degree)
                    )
                );

                positiveSum = positiveSum.sub(
                    pow(start, 1 + degree).mul(significands[degree]).div(
                        pow(10, exponents[degree]).mul(1 + degree)
                    )
                );
            } else {
    ...
    */
     ...
     * @param f The function data of the piecewise polynomial.
     */
    function getPiecewiseBreakpoint(
        bytes calldata f,
        uint256 pieceIndex
    ) internal pure returns (uint256) {
     ...
     */
    function getSignificands(
        bytes calldata f,
        uint256 pieceIndex,
        uint256 numPieces
    ) internal pure returns (uint256[4] memory significands) {
        bytes memory significandSlice = f.sliceToMemory(
            (pieceIndex.mul(128)).add(numPieces.mul(32)).add(32),
            128
        );
     ...
     * @param numPieces The number of pieces in the piecewise polynomial.
     */
    function getExponents(
        bytes calldata f,
        uint256 pieceIndex,
        uint256 numPieces
    ) internal pure returns (uint8[4] memory exponents) {
        bytes memory exponentSlice = f.sliceToMemory(
            (pieceIndex.mul(4)).add(numPieces.mul(160)).add(32),
            4
        );
    ...
    function getSigns(
        ...
        uint256 pieceIndex,
        uint256 numPieces
    ) internal pure returns (bool[4] memory signs) {
        bytes memory signSlice = f.sliceToMemory(
            (pieceIndex.mul(4)).add(numPieces.mul(164)).add(32),
            4
        );
