...
describe("SeedGauge Init Test", function () {
  ...
  describe("init state", async function () {
    ...
    it("average grown stalk per BDV per Season", async function () {
      expect(await this.beanstalk.getAverageGrownStalkPerBdvPerSeason()).to.be.equal(
        to6("5.343518")
      );
    });
    ...
    it("L2SR", async function () {
      ...
      expect(await this.beanstalk.getLiquidityToSupplyRatio()).to.be.within(
        to18("0.93"),
        to18("0.94")
      );
    });
    ...
    it("bean To MaxLPGpRatio", async function () {
      expect(await this.beanstalk.getBeanToMaxLpGpPerBdvRatio()).to.be.equal(
        to18("33.333333333333333333")
      );
      expect(await this.beanstalk.getBeanToMaxLpGpPerBdvRatioScaled()).to.be.equal(
    ...
    it("lockedBeans", async function () {
      ...
      expect(await this.beanstalk.getLockedBeans()).to.be.within(
        to6("25900000.000000"),
        to6("26000000.000000")
      );
    });
    ...
    it("lockedBeans with input", async function () {
      ...
      expect(
        await this.beanstalk.getLockedBeansFromTwaReserves(cumulativeReserves, seasonTime.timestamp)
    ...
    it("usd Liquidity", async function () {
      ...
      expect(await this.beanstalk.getTwaLiquidityForWell(BEAN_ETH_WELL)).to.be.within(
        to18("13100000"),
        to18("13300000")
      );
      // ~13.2m usd liquidity in Bean:Eth
      expect(await this.beanstalk.getTotalUsdLiquidity()).to.be.within(
        to18("13100000"),
        to18("13300000")
      );
    });
  ...
  describe("bean3crv dewhitelisted", async function () {
    ...
    it("reverts on deposit", async function () {
      await expect(
        this.beanstalk.connect(user).deposit(BEAN_3_CURVE, to18("100"), INTERNAL)
    ...
    it("allows legacy bean3crv to be mown", async function () {
      ...
      await this.beanstalk.mow("0x1B7eA7D42c476A1E2808f23e18D850C5A4692DF7", BEAN_3_CURVE);
      newStalk = await this.beanstalk.balanceOfStalk("0x1B7eA7D42c476A1E2808f23e18D850C5A4692DF7");
  ...
  describe("silo v3.1 migration", async function () {
    ...
    it("correctly updates lastStem for a user", async function () {
      ...
      await this.beanstalk.mow("0x43a9dA9bAde357843fBE7E5ee3Eedd910F9fAC1e", BEAN);

      expect(await this.beanstalk.getLastMowedStem(testAccount, BEAN)).to.equal(to6("12699"));
      expect(await this.beanstalk.getLastMowedStem(testAccount, BEAN_3_CURVE)).to.equal(
        to6("5927")
      );
      expect(await this.beanstalk.getLastMowedStem(testAccount, BEAN_ETH_WELL)).to.equal(
        to6("15372")
      );
      expect(await this.beanstalk.getLastMowedStem(testAccount, UNRIPE_BEAN)).to.equal("0");
      expect(await this.beanstalk.getLastMowedStem(testAccount, UNRIPE_LP)).to.equal("0");
    ...
    it("correctly updates a deposit for user", async function () {
      ...
      this.result = await this.beanstalk
        .connect(account)
      ...
      await expect(this.result)
        .to.emit(this.beanstalk, "TransferSingle")
        .withArgs(
          account.address,
          account.address,
      ...
      await expect(this.result)
        .to.emit(this.beanstalk, "RemoveDeposit")
      ...
      await expect(this.result)
        .to.emit(this.beanstalk, "AddDeposit")
