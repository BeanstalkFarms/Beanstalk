...
testIfRpcSet("PreFertilizer", function () {
  before(async function () {
    ...
    await expect(mockBeanstalk.connect(user).mint(to6("10000"))).to.be.revertedWith(
      "Fertilizer: Not started"
    );
  ...
  it("reverts on balance of 0", async function () {
    await expect(
      mockBeanstalk.balanceOf(ethers.constants.AddressZero, to6("6"))
    ).to.be.revertedWith("ERC1155: balance query for the zero address");
    await expect(
      mockBeanstalk.lastBalanceOf(ethers.constants.AddressZero, to6("6"))
    ).to.be.revertedWith("ERC1155: balance query for the zero address");
  });
  ...
  describe("Below Remaining", async function () {
    ...
    describe("Contribute from eth", async function () {
      ...
      it("changes user balance", async function () {
        expect(await mockBeanstalk.balanceOf(user.address, to6("6"))).to.equal(
          this.usdcOut.div(1e6)
        );
  ...
  describe("Above remaining", async function () {
    ...
    describe("Contribute from eth", async function () {
      it("reverts", async function () {
        const ethIn = ethers.utils.parseEther("0.01");
        await expect(mockBeanstalk.buyAndMint("0", { value: ethIn })).to.be.revertedWith(
          "Fertilizer: Not enough remaining"
        );
      });
    });
  });
  ...
  describe("Transfer", async function () {
    it("revert if not enough", async function () {
      await expect(
        mockBeanstalk.safeTransferFrom(
          user.address,
          user.address,
          to6("6"),
          "10001",
    ...
    it("works if enough", async function () {
      await mockBeanstalk.connect(user).mint(to6("10000"));
      await mockBeanstalk.safeTransferFrom(
        user.address,
        user2.address,
        to6("6"),
        "1000",
    ...
    it("batch works if enough", async function () {
      await mockBeanstalk.connect(user).mint(to6("10000"));
      await mockBeanstalk.safeBatchTransferFrom(
        user.address,
        user2.address,
        [to6("6")],
        ["1000"],
