...
async function replant10(account, mock, verbose) {
  ...
  selectorsToRemove = deployedSelectors
    .filter(
      (d) =>
        d.facetAddress !== "0xDFeFF7592915bea8D040499E961E332BD453C249" &&
  ...
  const diamondCut = diamondCutFacet.interface.encodeFunctionData(
    "diamondCut",
  ...
  const addFertilizer = fertilizerFacet.interface.encodeFunctionData(
    "addFertilizerOwner",
    addFertilizerParams
  );
  ...
  if (mock) {
    ...
    await fs.writeFileSync(
      `./replant/gnosis/diamondCut.json`,
      JSON.stringify(
        { to: BEANSTALK, parameters: Object.values(diamondCutParams), data: diamondCut },
        null,
        4
      )
    );
    await fs.writeFileSync(
      `./replant/gnosis/approval.json`,
      JSON.stringify({ to: USDC, parameters: approvalParams, data: approval }, null, 4)
    );
    await fs.writeFileSync(
      `./replant/gnosis/addFertilizer.json`,
      JSON.stringify(
        { to: BEANSTALK, parameters: addFertilizerParams, data: addFertilizer },
        null,
        4
      )
    );
