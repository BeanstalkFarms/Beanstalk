...
describe("Curve", function () {
  ...
  describe("Add Liquidity", async function () {
    describe("To external", async function () {
      ...
      it("adds lp tokens to external balance", async function () {
        expect(
          await this.token.getExternalBalance(userAddress, this.beanMetapool.address)
        ).to.be.equal(to18("2000"));
      });

      it("adds lp tokens to internal balance", async function () {
        expect(
          await this.token.getInternalBalance(userAddress, this.beanMetapool.address)
        ).to.be.equal(to18("0"));
      });
    });
  ...
  describe("Exchange", async function () {
    describe("To external", async function () {
      ...
      it("adds lp tokens to internal balance", async function () {
        expect(
          await this.token.getInternalBalance(user2Address, this.threeCurve.address)
        ).to.be.equal("0");
      });

      it("adds lp tokens to external balance", async function () {
        expect(
          await this.token.getExternalBalance(user2Address, this.threeCurve.address)
        ).to.be.equal("9990916598540524929");
      });
    });
  ...
  describe("Remove Liqudity", async function () {
    describe("To external", async function () {
      ...
      it("adds lp tokens to internal balance", async function () {
        expect(
          await this.token.getInternalBalance(userAddress, this.threeCurve.address)
        ).to.be.equal("0");
        expect(await this.token.getInternalBalance(userAddress, this.bean.address)).to.be.equal(
          "0"
        );
      });
      ...
      it("adds lp tokens to external balance", async function () {
        expect(
          await this.token.getExternalBalance(userAddress, this.threeCurve.address)
        ).to.be.equal(to18("250"));
        expect(await this.token.getExternalBalance(userAddress, this.bean.address)).to.be.equal(
          to6("9250")
        );
      });
    });
  ...
  describe.skip("farm LP and Deposit", async function () {
    ...
    it("add lp and deposit", async function () {
      ...
      const dep = await this.siloGetters.getDeposit(
        user2Address,
        this.beanMetapool.address,
        stemBean
      );
