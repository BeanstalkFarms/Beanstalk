...
describe("Well Convert", function () {
  before(async function () {
    ...
    await this.wellToken
      .connect(owner)
      .approve(this.beanstalk.address, ethers.constants.MaxUint256);
  ...
  describe("calclates beans to peg", async function () {
    it("p > 1", async function () {
      ...
      expect(await this.beanstalk.getAmountOut(BEAN, this.well.address, maxAmountIn)).to.be.equal(
  ...
  describe("calclates lp to peg", async function () {
    ...
    it("p < 1", async function () {
      ...
      expect(await this.beanstalk.getAmountOut(this.well.address, BEAN, maxAmountIn)).to.be.equal(
        to6("200000")
      );
    });
  });
    ...
    describe("p > 1", async function () {
      ...
      it("reverts if not whitelisted well", async function () {
        ...
        await expect(
          this.convert
            .connect(owner)
            .convertInternalE(this.bean.address, to6("100000"), convertData)
      ...
      it("convert below max", async function () {
        ...
        const [toToken, fromToken, toAmount, fromAmount] = await this.convert
          .connect(owner)
          .callStatic.convertInternalE(this.bean.address, to6("100000"), convertData);
      ...
      it("convert equal to max", async function () {
        ...
        const [toToken, fromToken, toAmount, fromAmount] = await this.convert
          .connect(owner)
          .callStatic.convertInternalE(this.bean.address, to6("200000"), convertData);
        expect(fromToken).to.be.equal(BEAN);
      ...
      it("convert greater than max", async function () {
        ...
        const [toToken, fromToken, toAmount, fromAmount] = await this.convert
          .connect(owner)
          .callStatic.convertInternalE(this.bean.address, to6("400000"), convertData);
        expect(fromToken).to.be.equal(BEAN);
        expect(fromAmount).to.be.equal(to6("200000"));
        expect(toToken).to.be.equal(this.well.address);
        expect(toAmount).to.be.equal("3338505354221892343955");
      ...
      it("deposit and convert below max", async function () {
        ...
        await this.season.siloSunrise("0");
        await this.convert.connect(owner).convert(convertData, ["0"], [to6("100000")]);
  ...
  describe("convert lp to beans", async function () {
    describe("p <= 1", async function () {
      ...
      it("deposit and convert below max", async function () {
        ...
        await this.season.siloSunrise("0");
        await this.season.siloSunrise("0");
        await this.convert.connect(owner).convert(convertData, ["0"], [to18("2000")]);
