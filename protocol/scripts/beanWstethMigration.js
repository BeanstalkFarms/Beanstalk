...
async function finishWstethMigration(mock = true, verbose = false) {
  ...
  await beanEthWell
    .connect(owner)
    .removeLiquidity(wellTokenBalance, [0, 0], owner.address, ethers.constants.MaxUint256);
  ...
  await beanWstethWell
    .connect(owner)
    .addLiquidity([beanBalance, wstethAmount], "0", owner.address, ethers.constants.MaxUint256);
