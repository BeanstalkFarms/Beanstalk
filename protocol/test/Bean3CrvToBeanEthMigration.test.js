...
describe("Bean:3Crv to Bean:Eth Migration", function () {
  ...
  describe("Initializes migration", async function () {
    ...
    it("Sends underlying balance to BCM", async function () {
      expect(await this.beanstalk.getExternalBalance(BCM, BEAN_3_CURVE)).to.be.equal(
        underlyingBefore
      );
    });
    ...
    describe("Interactions with Unripe fail", async function () {
      it("chop fails", async function () {
        ...
        await expect(
          this.beanstalk.connect(publius).chop(UNRIPE_LP, to6("1"), 1, 0)
        ).to.be.revertedWith("Chop: no underlying");
      });

      it("deposit fails", async function () {
        await this.beanstalk.connect(publius).withdrawDeposit(UNRIPE_LP, "-56836", to6("1"), 1);
        await expect(
          this.beanstalk.connect(publius).deposit(UNRIPE_LP, to6("1"), 1)
        ).to.be.revertedWith("Silo: No Beans under Token.");
      });

      it("enrootDeposit fails", async function () {
        await expect(
          this.beanstalk.connect(publius).enrootDeposit(UNRIPE_LP, "-56836", to6("1"))
        ).to.be.revertedWith("SafeMath: subtraction overflow");
      });

      it("enrootDeposits fails", async function () {
        await expect(
          this.beanstalk.connect(publius).enrootDeposits(UNRIPE_LP, ["-56836"], [to6("1")])
        ).to.be.revertedWith("SafeMath: subtraction overflow");
      });
  ...
  describe("Completes Migration", async function () {
    ...
    it("successfully adds underlying", async function () {
      ...
      expect(
        await this.beanstalk.getUnderlying(UNRIPE_LP, await this.unripeLp.totalSupply())
