...
describe("Sun", function () {
  ...
  it("sunrise reward", async function () {
    ...
    const startingBeanBalance =
      (await this.tokenFacet.getAllBalance(owner.address, BEAN)).totalBalance.toNumber() /
      Math.pow(10, 6);
    for (const mockVal of mockedValues) {
      ...
      await expect(this.result)
        .to.emit(this.season, "Incentivization")
  ...
  it("ends germination", async function () {
    ...
    await expect(this.result)
      .to.emit(this.season, "TotalGerminatingStalkChanged")
      .withArgs("4", toStalk("-1000"));
    await expect(this.result)
      .to.emit(this.season, "TotalStalkChangedFromGermination")
  ...
  it("rewards more than type(uint128).max/10000 to silo", async function () {
    await expect(
      this.season.siloSunrise("340282366920938463463374607431768211456")
  ...
  it("rewards more than type(uint128).max Soil below peg", async function () {
    await expect(
      this.season.sunSunrise("-340282366920938463463374607431768211456", "0")
    ).to.be.revertedWith("SafeCast: value doesn't fit in 128 bits");
  });
});
