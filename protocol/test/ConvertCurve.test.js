...
describe("Curve Convert", function () {
  ...
  describe.skip("calculates beans to peg LEGACY", async function () {
    it("p > 1", async function () {
      ...
      expect(await beanstalk.getMaxAmountIn(bean.address, this.beanMetapool.address)).to.be.equal(
  ...
  describe("calculates beans to peg", async function () {
    it("p > 1", async function () {
      await expect(
        beanstalk.getMaxAmountIn(bean.address, this.beanMetapool.address)
  ...
  describe("calculates lp to peg", async function () {
    it("p > 1", async function () {
      ...
      expect(await beanstalk.getMaxAmountIn(this.beanMetapool.address, bean.address)).to.be.equal(
  ...
  describe.skip("convert beans to lp LEGACY", async function () {
    describe("revert", async function () {
      ...
      it("Not whitelisted pool", async function () {
        ...
        await expect(beanstalk.connect(owner).convert(convertData, [], [])).to.be.revertedWith(
    ...
    describe("below max", async function () {
      ...
      it("it gets amount out", async function () {
        expect(await beanstalk.getAmountOut(BEAN, BEAN_3_CURVE, to6("100"))).to.be.equal(
      ...
      it("returns correct values", async function () {
        ...
        this.result = await beanstalk
          .connect(user)
          .callStatic.convert(
            ConvertEncoder.convertBeansToCurveLP(to6("100"), to18("99"), this.beanMetapool.address),
            [stemBean],
            [to6("100")]
          );
      ...
      describe("it converts", async function () {
        ...
        it("properly updates total values", async function () {
          ...
          expect(await beanstalk.getTotalDeposited(this.beanMetapool.address)).to.eq(
            "100634476734756985505"
          );
        ...
        it("properly updates user deposits", async function () {
          const stemBean = await beanstalk.seasonToStem(bean.address, "12");
          const stemMetapool = await beanstalk.seasonToStem(this.beanMetapool.address, "12");
          expect((await beanstalk.getDeposit(user.address, bean.address, stemBean))[0]).to.eq(
            to6("100")
          );
          const deposit = await beanstalk.getDeposit(
            user.address,
            this.beanMetapool.address,
            stemMetapool
          );
        ...
        it("emits events", async function () {
          ...
          await expect(this.result)
            ...
            .withArgs(user.address, bean.address, [stemBean], [to6("100")], to6("100"), [
              to6("100")
            ]);
          await expect(this.result)
    ...
    describe("above max", function () {
      ...
      it("it gets amount out", async function () {
        expect(await beanstalk.getAmountOut(BEAN, BEAN_3_CURVE, to6("200"))).to.be.equal(
      ...
      describe("it converts", async function () {
        ...
        it("emits events", async function () {
          ...
          await expect(this.result)
            .to.emit(beanstalk, "RemoveDeposits")
            .withArgs(user.address, bean.address, [stemBean], [to6("200")], to6("200"), [
              to6("200")
            ]);
          const stemMetapool = await beanstalk.seasonToStem(this.beanMetapool.address, "12");
          await expect(this.result)
    ...
    describe("multiple crates", function () {
      ...
      describe("it converts", async function () {
        ...
        it("properly updates user deposits", async function () {
          const stemBean10 = await beanstalk.seasonToStem(bean.address, "10");
          const stemBean14 = await beanstalk.seasonToStem(bean.address, "14");
          expect((await beanstalk.getDeposit(user.address, bean.address, stemBean10))[0]).to.eq(
            to6("0")
          );
          expect((await beanstalk.getDeposit(user.address, bean.address, stemBean14))[0]).to.eq(
            to6("0")
          );
  ...
  describe("convert beans to lp", async function () {
    describe("below max", async function () {
      ...
      it("it gets amount out", async function () {
        await expect(beanstalk.getAmountOut(BEAN, BEAN_3_CURVE, to6("100"))).to.be.revertedWith(
          "Convert: Tokens not supported"
        );
      });
  ...
  describe("convert lp to beans", async function () {
    ...
    describe("below max", function () {
      ...
      it("it gets amount out", async function () {
        expect(await beanstalk.getAmountOut(BEAN_3_CURVE, BEAN, to18("100"))).to.be.equal(
          "100618167"
        );
      });
    ...
    describe("above max", function () {
      ...
      it("it gets amount out", async function () {
        expect(
          await beanstalk.getAmountOut(BEAN_3_CURVE, BEAN, "199185758314813528598")
      ...
      describe("it converts", async function () {
        ...
        it("properly updates total values", async function () {
          ...
          expect(await beanstalk.getTotalDepositedBdv(this.beanMetapool.address)).to.eq(
            "800814241"
          );
    ...
    describe("multiple crates", function () {
      ...
      describe("it converts", async function () {
        ...
        it("properly updates user deposits", async function () {
          expect((await beanstalk.getDeposit(user.address, bean.address, this.beanStem))[0]).to.eq(
            "100618167"
          );
          const deposit = await beanstalk.getDeposit(
            user.address,
