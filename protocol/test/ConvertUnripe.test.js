...
const { BEAN, UNRIPE_BEAN, UNRIPE_LP, WETH, BEANSTALK, BEAN_ETH_WELL, ZERO_BYTES} = require('./utils/constants')
  ...
  describe('convert beans to lp', async function () {
    ...
    describe('revert', async function () {
      ...
      it('not enough LP', async function () {
        ...
        await this.well.connect(user).addLiquidity([to6('0'), to18('0.02')], '0', user.address, ethers.constants.MaxUint256);
        const amountOut = await this.well.getAddLiquidityOut([to6('200'), '0'])
        await expect(beanstalk.connect(user).convert(ConvertEncoder.convertUnripeBeansToLP(to6('200'), amountOut.add(toBN('1'))), ['0'], [to6('200')]))
          .to.be.revertedWith('')
      });

      it('p >= 1', async function () {
        await beanstalk.connect(user).deposit(this.unripeBean.address, to6('200'), EXTERNAL);
        await expect(beanstalk.connect(user).convert(ConvertEncoder.convertUnripeBeansToLP(to6('200'), '0'), ['0'], ['1000']))
    ...
    describe('basic', function () {
      beforeEach(async function () {
        await beanstalk.connect(user).deposit(
        ...
        this.result = await beanstalk.connect(user).convert(
          ConvertEncoder.convertUnripeBeansToLP(to6('1000'), '0'), [this.stem], [to6('2000')]
        )
      });
      ...
      it('properly updates total values', async function () {
        ...
        expect(await beanstalk.getGerminatingTotalDeposited(this.unripeLP.address)).to.eq('4711829');
      ...
      it('properly updates user deposits', async function () {
        expect((await beanstalk.getDeposit(user.address, this.unripeBean.address, this.stem))[0]).to.eq(to6('1000'));
        const bdv = await beanstalk.bdv(this.unripeLP.address, '4711829')
      ...
      it('emits events', async function () {
        await expect(this.result).to.emit(beanstalk, 'RemoveDeposits')
          .withArgs(user.address, this.unripeBean.address, ['1'], [to6('1000')], to6('1000'), [to6('100')]);
  ...
  describe('convert lp to beans', async function () {
    ...
    describe('revert', async function () {
      it('not enough Beans', async function () {
        ...
        await expect(beanstalk.connect(user).convert(ConvertEncoder.convertUnripeLPToBeans(to6('2000'), to6('2500')), ['0'], [to6('2000')]))
          .to.be.revertedWith('');
      });

      it('p >= 1', async function () {
        await this.well.connect(user).addLiquidity([to6('0'), to18('1')], to18('0.5'), user.address, ethers.constants.MaxUint256);
        await beanstalk.connect(user).deposit(this.unripeLP.address, to6('1000'), EXTERNAL);
        await expect(beanstalk.connect(user).convert(ConvertEncoder.convertUnripeLPToBeans(to6('2000'), to6('2500')), ['0'], [to6('2000')]))
          .to.be.revertedWith('Convert: P must be < 1.');
      });
    ...
    describe('below max', function () {
      beforeEach(async function () {
        ...
        // call sunrise twice to finish germination.
        await endGermination()
        this.result = await beanstalk.connect(user).convert(ConvertEncoder.convertUnripeLPToBeans(to6('3'), toBN('0')), [this.stem], [to6('1000')])
      });
      ...
      it('properly updates total values', async function () {
        ...
        expect(await beanstalk.getGerminatingTotalDeposited(this.unripeBean.address)).to.eq('636776401');
        
        expect(await beanstalk.getTotalDepositedBdv(this.unripeBean.address)).to.eq('0');
        expect(await beanstalk.getGerminatingTotalDepositedBdv(this.unripeBean.address)).to.eq(189738509);
        ...
        expect(await beanstalk.getGerminatingTotalDepositedBdv(this.unripeLP.address)).to.eq(to6('0'));
  ...
  describe('convert unripe beans to beans', async function () {
    ...
    describe('basic urBEAN-->BEAN convert', function () {
      ...
      beforeEach(async function () {
        ...
        this.result = await beanstalk.connect(user).convert(ConvertEncoder.convertUnripeToRipe(to6('100') , this.unripeBean.address) , ['0'], [to6('100')] );
      });
      ...
      it('getters', async function () {
        ...
        expect(await mockBeanstalk.getPenalizedUnderlying(UNRIPE_BEAN, to6('1'))).to.be.equal(to6('0.00101'))
      ...
      // USER DEPOSITS TEST
      it('properly updates user deposits', async function () {
        expect((await beanstalk.getDeposit(user.address, this.unripeBean.address, 0))[0]).to.eq(to6('100'));
      ...
      it('emits events', async function () {
        await expect(this.result).to.emit(beanstalk, 'RemoveDeposits')
          .withArgs(user.address, this.unripeBean.address, [0], [to6('100')], to6('100'), [to6('10')]);
        await expect(this.result).to.emit(beanstalk, 'AddDeposit')
          .withArgs(user.address, bean.address, 0 , to6('0.1'), to6('10'));
        await expect(this.result).to.emit(beanstalk, 'Convert')
