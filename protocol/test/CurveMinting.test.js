  ...
  describe("Curve", async function () {
    ...
    it("tracks a basic TWAL", async function () {
      ...
      await expect(this.result)
        .to.emit(this.season, "UpdateTWAPs")
        .withArgs([toBean("1000000"), to18("1000000")]);
    });
    ...
    it("2 separate TWAL", async function () {
      ...
      await expect(this.result)
        .to.emit(this.season, "UpdateTWAPs")
        .withArgs([toBean("750000"), to18("1000000")]);
    });
    ...
    describe("Get Delta B", async function () {
      it("reverts if not a minting pool", async function () {
        await expect(this.seasonGetter.poolDeltaB(BEAN)).to.be.revertedWith(
          "Oracle: Pool not supported"
        );
      });
