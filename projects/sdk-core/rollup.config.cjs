 ...
 */
const config = [
  // do not change the "core" value, it is a 'magic string' that represents
  // the main entry point in various ways
  makeEntry("dist/js/index.js", "core"),

  // This is just an example of how to create a new module entry.
  // This lets you do this on the client side:
  // import { Thing } from "@beanstalk/sdk-core/Thing"
...
function makeEntry(inputFile, name) {
  const outRoot = "dist";
  const typesPath = `./${outRoot}/types/${name === "core" ? "index" : name}.d.ts`;
  ...
  let config = {
    ...
    output: [
      ...
      { file: udmPath, format: "umd", sourcemap: true, name: "SDKCore" }
    ],
