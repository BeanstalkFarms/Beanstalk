...
describe('Unripe', function () {
  ...
  describe('deposit underlying', async function () {
    ...
    it('getters', async function () {
      ...
      expect(await mockBeanstalk.balanceOfUnderlying(UNRIPE_BEAN, user.address)).to.be.equal(to6('100'))
      expect(await mockBeanstalk.balanceOfPenalizedUnderlying(UNRIPE_BEAN, user.address)).to.be.equal('0')
    })
  ...
  describe('penalty go down', async function () {
    ...
    it('getters', async function () {
      ...
      expect(await mockBeanstalk.getTotalUnderlying(UNRIPE_BEAN)).to.be.equal(to6('100'))
      expect(await mockBeanstalk.isUnripe(UNRIPE_BEAN)).to.be.equal(true)
      expect(await mockBeanstalk.getPenalizedUnderlying(UNRIPE_BEAN, to6('1'))).to.be.equal(to6('0.001'));
      expect(await mockBeanstalk.getUnderlying(UNRIPE_BEAN, to6('1'))).to.be.equal(to6('0.1'))
      expect(await mockBeanstalk.balanceOfUnderlying(UNRIPE_BEAN, user.address)).to.be.equal(to6('100'))
      expect(await mockBeanstalk.balanceOfPenalizedUnderlying(UNRIPE_BEAN, user.address)).to.be.equal(to6('1'))
    })
  ...
  describe('chop', async function () {
    beforeEach(async function () {
      ...
      await mockBeanstalk.connect(owner).setPenaltyParams(to6('100'), to6('100'))
      this.result = await mockBeanstalk.connect(user).chop(UNRIPE_BEAN, to6('1'), EXTERNAL, EXTERNAL)
    })
    ...
    it('getters', async function () {
      ...
      expect(await mockBeanstalk.balanceOfUnderlying(UNRIPE_BEAN, user.address)).to.be.equal(to6('99.999'))
      expect(await mockBeanstalk.balanceOfPenalizedUnderlying(UNRIPE_BEAN, user.address)).to.be.equal(to6('0.99999'))
    })
  ...
  describe('chop', async function () {
    beforeEach(async function () {
      ...
      await mockBeanstalk.connect(owner).setPenaltyParams(to6('100'), to6('100'))
      await beanstalk.connect(user).transferToken(
        UNRIPE_BEAN,
        user.address,
        to6('1'),
        EXTERNAL,
    ...
    this.result = await mockBeanstalk.connect(user).chop(UNRIPE_BEAN, to6('10'), INTERNAL_TOLERANT, EXTERNAL)
    })
  ...
  describe('change underlying', async function () {
    it('changes underlying token', async function () {
      ...
      await expect(this.result).to.emit(mockBeanstalk, 'SwitchUnderlyingToken').withArgs(
        UNRIPE_BEAN,
        USDT
      )
    })
    ...
    it('reverts if underlying balance > 0', async function () {
      ...
      await expect(mockBeanstalk.connect(owner).switchUnderlyingToken(UNRIPE_BEAN, USDT)).to.be.revertedWith('Unripe: Underlying balance > 0')
    })

    it('reverts if not owner', async function () {
      await expect(mockBeanstalk.connect(user).switchUnderlyingToken(UNRIPE_BEAN, USDT)).to.be.revertedWith('LibDiamond: Must be contract owner')
    })
  })
})