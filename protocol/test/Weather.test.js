...
const { MAX_UINT32, UNRIPE_BEAN, UNRIPE_LP, BEAN_3_CURVE, BEAN_ETH_WELL, BEAN, BEAN_WSTETH_WELL, WSTETH, BEANSTALK_PUMP, ZERO_BYTES} = require('./utils/constants.js')
  ...
  [...Array(numberTests).keys()].map(i => i + startTest).forEach(function(v) {
    ...
    describe(testStr.concat((v)), function () {
      ...
      it('Emits The Correct Case Weather', async function () {
        if (this.testData.totalOutstandingBeans !== 0) await expect(this.result).to.emit(beanstalk, 'TemperatureChange')
          .withArgs(
      ...
      it('Checks New Percent To LP', async function () {
        expect(await beanstalk.getBeanToMaxLpGpPerBdvRatio())
        .to.eq(to18(this.testData.newPercentToLp))
      })

      it('Emits The Correct LP Case', async function () {
        if (this.testData.totalOutstandingBeans !== 0) await expect(this.result).to.emit(beanstalk, 'BeanToMaxLpGpPerBdvRatioChange')
          .withArgs(
            await beanstalk.season(), 
            this.testData.Code, 
