...
contract WellFacet is ReentrancyGuard {
    ...
    **/
    ...
    function swapFrom(
        ...
        amountOut = uint256(LibWell.swap(p, fromToken, toToken, int128(amountIn), int128(minAmountOut)));
    ...
    function getSwapOut(
        ...
        amountOut = uint256(LibWell.getSwap(p, fromToken, toToken, int128(amountIn)));
    }
    ...
    function swapTo(
        ...
        amountIn = uint256(-LibWell.swap(p, toToken, fromToken, int128(-amountOut), int128(-maxAmountIn)));
    ...
    function getSwapIn(
        ...
        amountIn = uint256(-LibWell.getSwap(p, toToken, fromToken, int128(-amountOut)));
    }
    ...
    **/
    ...
    function addLiquidity(
        ...
        for (uint i; i < tokenAmounts.length; ++i)
            tokenAmounts[i] = uint128(p.tokens[i].receiveToken(uint256(tokenAmounts[i]), msg.sender, fromMode)); // Downcasting to uint128 is okay because result is < tokensAmounts[i]
        lpAmountOut = LibWell.addLiquidity(p, tokenAmounts, minLPAmountOut, msg.sender, toMode);
    }
    ...
    function getAddLiquidityOut(
        ...
        lpAmountOut = LibWell.getAddLiquidityOut(p, tokenAmounts);
    }
    ...
    **/
    ...
    function removeLiquidity(
        ...
        tokenAmountsOut = LibWell.removeLiquidity(p, lpAmountIn, minTokenAmountsOut, msg.sender, fromMode);
        for (uint i; i < tokenAmountsOut.length; ++i)
            p.tokens[i].sendToken(tokenAmountsOut[i], msg.sender, toMode);
    }
    ...
    function getRemoveLiquidityOut( LibWellStorage.WellInfo calldata p, uint256 lpAmountIn)
        ...
        returns (uint128[] memory tokenAmountsOut) {
            tokenAmountsOut = LibWell.getRemoveLiquidityOut(p, lpAmountIn);
        }
    ...
    function removeLiquidityOneToken(
        ...
        tokenAmountOut = LibWell.removeLiquidityOneToken(p, token, lpAmountIn, minTokenAmountOut, msg.sender, fromMode);
    ...
    function getRemoveLiquidityOneTokenOut(
        ...
        tokenAmountOut = LibWell.getRemoveLiquidityOneTokenOut(p, token, lpAmountIn);
    }
    ...
    function removeLiquidityImbalanced(
        ...
        lpAmountIn = LibWell.removeLiquidityImbalanced(p, maxLPAmountIn, tokenAmountsOut, msg.sender, fromMode);
        for (uint i; i < tokenAmountsOut.length; ++i)
            p.tokens[i].sendToken(tokenAmountsOut[i], msg.sender, toMode);
    }
    ...
    function getRemoveLiquidityImbalancedIn(
        ...
        lpAmountIn = LibWell.getRemoveLiquidityImbalanced(p, amountsOut);
    }
