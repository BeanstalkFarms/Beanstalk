...
const { UNRIPE_BEAN, UNRIPE_LP, BEAN, BEAN_ETH_WELL, ETH_USDT_UNISWAP_V3, ZERO_BYTES, MAX_UINT256, BEAN_WSTETH_WELL } = require('./utils/constants.js')
...
describe('Gauge', function () {
  ...
  describe('Bean to maxLP ratio', function () {
    ...
    describe('L2SR > excessively high L2SR % + P > 1', async function () {
      it("increases Bean to maxLP ratio", async function () {
        ...
        await expect(this.result).to.emit(beanstalk, 'BeanToMaxLpGpPerBdvRatioChange')
          .withArgs(
            3,     // season
            108,    // caseId
  ...
  describe('L2SR calculation', async function () {
    describe("getter", function () {
      ...
      it('outputs correct liquidity values:', async function (){
        ...
        expect(await beanstalk.getWeightedTwaLiquidityForWell(BEAN_ETH_WELL)).to.be.equal(to18('1000000'))
    ...
    describe('with unripe', function() {
      ...
      it('getters', async function () {
        ...
        expect(await mockBeanstalk.getLockedBeansUnderlyingUnripeBean()).to.be.eq(to6('436.332105'))
  ...
  describe('GaugePoints', async function () {
    ...
    it('Cannot exceed the maximum gauge points', async function () {
      expect(await beanstalk.defaultGaugePointFunction(
        to18('1000'),
        50e6,
        49e6
      )).to.be.eq(to18('1000'))
