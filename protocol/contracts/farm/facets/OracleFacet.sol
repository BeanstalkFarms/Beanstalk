...
contract OracleFacet {
    ...
    function capture() public virtual returns (int256) {
        ...
        return LibCurveOracle.capture() + LibUniswapOracle.capture();
    }

    function totalDeltaB() external view returns (int256) {
        return LibCurveOracle.check() + LibUniswapOracle.check();
    }

    function deltaB(address pool) external view returns (int256 deltaB, Pool[2] memory pools) {
        ...
        deltaB = pools[0].deltaB + pools[1].deltaB;
    }
