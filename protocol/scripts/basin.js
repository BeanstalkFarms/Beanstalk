...
async function deployBasin(
  ...
  c.well = await getWellContractAt(
    ...
    await c.aquifer
      .connect(account)
      .callStatic.boreWell(c.wellImplementation.address, immutableData, initData, WELL_DEPLOY_SALT)
  );

  const wellTxn = await c.aquifer
    .connect(account)
    .boreWell(c.wellImplementation.address, immutableData, initData, WELL_DEPLOY_SALT);
  ...
  const instantaneousReserves = await multiFlowPump.readInstantaneousReserves(c.well.address, "0x");
