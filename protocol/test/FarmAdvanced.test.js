...
const { BEAN_3_CURVE, THREE_POOL, THREE_CURVE, WETH, ZERO_ADDRESS, PIPELINE } = require('./utils/constants.js');
...
describe('Farm Advanced', function () {
  ...
  describe("1 Return data", async function () {
    beforeEach(async function () {
      ...
      selector = beanstalk.interface.encodeFunctionData('getInternalBalance', [user.address, bean.address])
      data = encodeAdvancedData(0)
      selector2 = beanstalk.interface.encodeFunctionData('transferToken', [bean.address, user2.address, to6('0'), 1, 1])
    ...
    it("Transfers Beans to user internal", async function () {
      ...
      expect(await beanstalk.getInternalBalance(user2.address, bean.address)).to.be.equal(to6('100'))
    })
  })
  ...
  describe("Multiple return data", async function () {
    beforeEach(async function () {
      ...
      selector2 = beanstalk.interface.encodeFunctionData('readPipe', [[this.mockContract.address, pipe]])
      data12 = encodeAdvancedData(0)
      selector3 = beanstalk.interface.encodeFunctionData('transferToken', [bean.address, ZERO_ADDRESS, to6('0'), 1, 1])
