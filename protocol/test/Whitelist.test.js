...
describe('Whitelist', function () {
  ...
  describe('whitelist', async function () {
    it('reverts if not owner', async function () {
      await expect(beanstalk.connect(user2).whitelistToken(
        this.well.address, 
    ...
    it('whitelists token', async function () {
      this.result = await beanstalk.connect(owner).whitelistTokenWithEncodeType(
        this.well.address,
        beanstalk.interface.getSighash('wellBdv'),
        '10000',
        '1',
      ...
      await expect(this.result).to.emit(beanstalk, 'WhitelistToken').withArgs(
        this.well.address, 
        beanstalk.interface.getSighash('wellBdv'), 
        1,
        10000,
    ...
    it('reverts on whitelisting same token again', async function () {
      this.resultFirst = await beanstalk.connect(owner).whitelistTokenWithEncodeType(
        this.well.address, 
        beanstalk.interface.getSighash('wellBdv'), 
        '10000',
        '1',
      ...
      await expect(beanstalk.connect(owner).whitelistTokenWithEncodeType(
          this.well.address, 
          beanstalk.interface.getSighash('wellBdv'), 
          '10000',
          '1',
    ...
    it('reverts on updating stalk per bdv per season for token that is not whitelisted', async function () {
      await expect(beanstalk.connect(owner).updateStalkPerBdvPerSeasonForToken(this.well.address, 1)).to.be.revertedWith("Token not whitelisted");
    });

    it('reverts on whitelisting token with bad selector', async function () {
      await expect(beanstalk.connect(owner).whitelistToken(
        this.well.address,
        beanstalk.interface.getSighash('wellBdv'),
        '10000',
        '1',
  ...
  describe('update stalk per bdv per season for token', async function () {
    it('reverts if not owner', async function () {
      await expect(beanstalk.connect(user2).updateStalkPerBdvPerSeasonForToken(this.well.address, 1)).to.be.revertedWith('LibDiamond: Must be contract or owner')
    })

    it('updates stalk per bdv per season', async function () {
      //do initial whitelist so there's something to update
      beanstalk.connect(owner).whitelistTokenWithEncodeType(
        this.well.address, 
        beanstalk.interface.getSighash('wellBdv'),
        '10000',
        '1',
        ...
        '0',
        '0')
      this.result = beanstalk.connect(owner).updateStalkPerBdvPerSeasonForToken(
        this.well.address, 
        '50000'
      )
      const settings = await beanstalk.tokenSettings(this.well.address)
      ...
      await expect(this.result).to.emit(beanstalk, 'UpdatedStalkPerBdvPerSeason').withArgs(this.well.address, 50000, currentSeason)
    })
  ...
  describe('dewhitelist', async function () {
    it('reverts if not owner', async function () {
      await expect(beanstalk.connect(user2).dewhitelistToken(this.well.address)).to.be.revertedWith('LibDiamond: Must be contract or owner')
    })
