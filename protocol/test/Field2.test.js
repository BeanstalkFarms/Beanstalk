...
describe('Field', function () {
  ...
  describe('Reverts', function () {
    it('No Soil', async function () {
      await expect(this.field.connect(user).sowBeans('1', EXTERNAL)).to.be.revertedWith('Field: Sowing below min or 0 pods.')
    });

    it('No Soil', async function () {
      await this.field.incrementTotalSoilE('100')
      await expect(this.field.connect(user).sowBeansWithMin('1', '3', EXTERNAL)).to.be.revertedWith('Field: Sowing below min or 0 pods.')
    });

    it('No Pods', async function () {
      await expect(this.field.connect(user).sowBeansWithMin('1', '0', EXTERNAL)).to.be.revertedWith('Field: Sowing below min or 0 pods.')
    });
  });
  ...
  describe('Sow', async function () {
    describe('all soil', async function () {
      beforeEach(async function () {
        ...
        this.result = await this.field.connect(user).sowBeans(to6('100'), EXTERNAL)
      })
    ...
    describe('some soil from internal', async function () {
      beforeEach(async function () {
        ...
        this.result = await this.field.connect(user).sowBeans(to6('100'), INTERNAL)
      })
    ...
    describe('with min', async function () {
      beforeEach(async function () {
        await this.field.incrementTotalSoilE(to6('100'))
        this.result = await this.field.connect(user).sowBeansWithMin(to6('200'), to6('100'), EXTERNAL)
      })
    ...
    describe('with min, but enough soil', async function () {
      beforeEach(async function () {
        await this.field.incrementTotalSoilE(to6('200'))
        this.result = await this.field.connect(user).sowBeansWithMin(to6('100'), to6('50'), EXTERNAL)
      })
    ...
    describe('second plot', async function () {
      beforeEach(async function () {
        await this.field.incrementTotalSoilE(to6('200'))
        this.result = await this.field.connect(user2).sowBeans(to6('100'), EXTERNAL)
        this.result = await this.field.connect(user).sowBeans(to6('100'), EXTERNAL)
      })
  ...
  describe("complex DPD", async function () {
    it("Does not set nextSowTime if Soil > 1", async function () {
      ...
      await this.field.connect(user).sowBeans(to6('1'), EXTERNAL)
    ...
    it("Does not set nextSowTime if Soil already < 1", async function () {
      ...
      await this.field.connect(user).sowBeans(to6('1'), EXTERNAL)
      const weather = await this.season.weather()
      await this.field.connect(user).sowBeans(to6('0.5'), EXTERNAL)
  ...
  describe("Harvest", async function () {
    beforeEach(async function () {
      await this.field.incrementTotalSoilE(to6('200'))
      await this.field.connect(user).sowBeans(to6('100'), EXTERNAL)
      await this.field.connect(user2).sowBeans(to6('100'), EXTERNAL)
    })
