...
contract DepotFacet is Invariable {
     ...
     **/
    function pipe(PipeCall calldata p) external payable fundsSafu returns (bytes memory result) {
    ...
    ) external payable fundsSafu returns (bytes[] memory results) {
    ...
    function etherPipe(
        PipeCall calldata p,
        uint256 value
    ) external payable fundsSafu returns (bytes memory result) {
