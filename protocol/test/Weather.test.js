    ...
    .forEach(function (v) {
      ...
      describe(testStr.concat(v), function () {
        ...
        it("Emits The Correct Case Weather", async function () {
          if (this.testData.totalOutstandingBeans !== 0)
            await expect(this.result)
              .to.emit(this.season, "TemperatureChange")
              .withArgs(
        ...
        it("Checks New Percent To LP", async function () {
          expect(await this.seasonGetter.getBeanToMaxLpGpPerBdvRatio()).to.eq(
        ...
        it("Emits The Correct LP Case", async function () {
          if (this.testData.totalOutstandingBeans !== 0)
            await expect(this.result)
              .to.emit(this.season, "BeanToMaxLpGpPerBdvRatioChange")
              .withArgs(
                await this.seasonGetter.season(),
                this.testData.Code,
