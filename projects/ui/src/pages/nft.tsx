...
const NFTPage: FC<{}> = () => {
  ...
  const parseMints = useCallback(
    async (accountNFTs: Nft[], contractAddress: string, setNFTs: any) => {
      ...
      // getNFTs
      const nftsBaseURL =
        'https://eth-mainnet.alchemyapi.io/nft/v2/demo/getNFTs/';
      const nftsFetchURL = `${nftsBaseURL}?owner=${account?.toLowerCase()}&contractAddresses[]=${contractAddress}`;

      // getNFTMetadata
      const nftMetadataBaseURL =
        'https://eth-mainnet.alchemyapi.io/nft/v2/demo/getNFTMetadata';
      const nftMetadataFetchURL = (id: number) =>
        `${nftMetadataBaseURL}?&contractAddress=${contractAddress}&tokenId=${id.toString()}`;
      ...
      const [
        ...
        fetch(nftsFetchURL, requestOptions).then((response) => response.json()),
        await Promise.all(
          accountNFTs.map((nft) =>
            fetch(nftMetadataFetchURL(nft.id), requestOptions).then(
      ...
      if (accountNFTs.length) {
        for (let i = 0; i < accountNFTs.length; i += 1) {
          ...
          if (
            !nftHashes.includes(accountNFTs[i].imageIpfsHash) &&
            isNotMinted
          ) {
