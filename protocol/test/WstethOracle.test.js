...
async function checkPriceWithError(price, lookback = '0', error = '100') {
    const oraclePrice = lookback == '0' ?
        await season.getWstethEthPrice() :
        await season.getWstethEthTwap(lookback)
...
describe('wStEth Oracle', function () {
    before(async function () {
        ...
        chainlinkAggregator = await ethers.getContractAt('MockChainlinkAggregator', STETH_ETH_CHAINLINK_PRICE_AGGREGATOR);
...
testIfRpcSet('wStEth Oracle with Forking', function () {
    it("Returns correct value when forking", async function () {
        ...
        expect(await usdOracle.getWstethUsdTwap('500000')).to.be.equal(to6('2744.261803'))
        expect(await usdOracle.getUsdPrice(WSTETH)).to.be.equal(to18('0.000387533493638216'))
    })
})