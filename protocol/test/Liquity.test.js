...
describe('Liquity', function () {
  ...
  describe('Sanity Checks', async function () {
    it('reverts when not supplied collateral', async function () {
      await expect(this.liquity.connect(user).manageTroveWithApproxHint(minFee, '2000000000000000000000', 2, Math.floor(Math.random() * 10), ethers.utils.parseEther('1'), {value: ethers.utils.parseEther('1')}))
    ...
    it('reverts when withdrawing excess debt', async function () {
      ...
      await this.liquity.connect(user).manageTroveWithApproxHint(minFee, '2000000000000000000000', 2, Math.floor(Math.random() * 10), ethers.utils.parseEther('4'), {value: ethers.utils.parseEther('4')});
      expect((await this.lusd.balanceOf(this.silo.address))/1e18).to.eq(2000);
      await expect(this.liquity.connect(user).manageTroveWithApproxHint(minFee, '2000000000000000000000', 2, Math.floor(Math.random() * 10), ethers.utils.parseEther('0'), {value: ethers.utils.parseEther('0')}))
        .to.be.revertedWith('BorrowerOps: An operation that would result in ICR < MCR is not permitted');
  ...
  describe('Borrower Operations', async function () {
    it('opens a trove for a new user', async function () {
      const lusdBefore = await this.lusd.totalSupply();
      await this.liquity.connect(user2).manageTroveWithApproxHint(minFee, '2000000000000000000000', 2, Math.floor(Math.random() * 10), ethers.utils.parseEther('7'), {value: ethers.utils.parseEther('7')});
	    const lusdAfter = await this.lusd.totalSupply();
    ...
    it('withdraws funds from an existing trove', async function () {
      await this.liquity.connect(user2).manageTroveWithApproxHint(minFee, '2000000000000000000000', 2, Math.floor(Math.random() * 10), ethers.utils.parseEther('0'), {value: ethers.utils.parseEther('0')});
