...
/**
 * @author Beasley
 * @title Users call specific functions on Uniswap
**/
...
contract UniswapRouter {
    ...
    function swapTokensForExactETH(uint amountOut, uint amountInMax, address[] memory path, address to, uint deadline, Storage.Settings memory set)
        internal
        ensure(deadline)
        returns (uint[] memory amounts)
    {
    ...
    function swapExactTokensForETH(uint amountIn, uint amountOutMin, address[] memory path, address to, uint deadline, Storage.Settings memory set)
        internal
        ensure(deadline)
        returns (uint[] memory amounts)
    {
   ...
   // returns sorted token addresses, used to handle return values from pairs sorted in this order
	...
	if ((token0 == s.c.bean && token1 == s.c.weth) || (token0 == s.c.weth && token1 == s.c.bean)) return s.c.pair;
