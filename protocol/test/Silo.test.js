...
describe('newSilo', function () {
  ...
  describe('Silo Balances After Withdrawal', function () {
    beforeEach(async function () {
      await beanstalk.connect(user).withdrawDeposit(
        BEAN, 
        to6('0'), 
        to6('500'), 
        EXTERNAL
  ...
  describe("ERC1155 Deposits", async function () {
    ...
    it('transfers an ERC1155 deposit', async function () {
      ...
      this.result = await beanstalk.connect(user).safeTransferFrom(
        user.address,
        user3.address,
        depositID,
        to6('1000'),
    ...
    it('batch transfers an ERC1155 deposit', async function () {
      ...
      this.result = await beanstalk.connect(user).deposit(
        BEAN,
        to6('1000'),
        EXTERNAL
      )
      ...
      this.result = await beanstalk.connect(user).safeBatchTransferFrom(
        user.address,
        user3.address,
    ...
    it('properly gives the correct batch balances', async function () {
      ...
      let b = await beanstalk.balanceOfBatch(
  ...
  describe("germination", async function () {
    ...
    describe("deposits", async function () {
      ...
      it('emit events', async function () {
        expect(this.result).to.emit(beanstalk, 'FarmerGerminatingStalkBalanceChanged').withArgs(
          user4.address,
          toStalk('1000')
        )
        expect(this.result).to.emit(beanstalk, 'TotalGerminatingBalanceChanged')
        .withArgs(
          '3',
          BEAN, 
          to6('1000'), 
    ...
    describe("withdraw", async function () {
      beforeEach(async function () {
        this.result = await beanstalk.connect(user4).withdrawDeposit(
          BEAN,
          to6('4'),
          to6('1000'),
          EXTERNAL
      ...
      it('a single farmer germination', async function () {
        ...
        expect((
          await beanstalk.getDeposit(
            user.address, 
            BEAN, 
            await beanstalk.stemTipForToken(BEAN))
          )[0]
        ).to.eq(49999999)
