...
describe("Ownership", function () {
  ...
  describe("ownership", async function () {
    describe("transfer", async function () {
      it("reverts if not owner", async function () {
        await expect(
          this.ownership.connect(user2).transferOwnership(user2Address)
    ...
    describe("claim", async function () {
      it("reverts if not candidate", async function () {
        await expect(this.ownership.connect(user2).claimOwnership()).to.be.revertedWith(
      ...
      it("claims ownership", async function () {
        ...
        await expect(this.result)
          .to.emit(this.ownership, "OwnershipTransferred")
          .withArgs(ownerAddress, user2Address);
      });
    });
  });
  ...
  describe("pause", async function () {
    it("reverts if not owner", async function () {
      await expect(this.pause.connect(user2).pause()).to.be.revertedWith(
  ...
  describe("unpause", async function () {
    it("reverts if not owner", async function () {
      await expect(this.pause.connect(user2).unpause()).to.be.revertedWith(
        "LibDiamond: Must be contract or owner"
      );
    });
