...
contract ReseedAccountStatus {
    ...
    function init(
        AccountStatus[] calldata accountStatuses
    ) external {
        // for each account
        for (uint i = 0; i < accountStatuses.length; i++) {
            ...
            for (uint j = 0; j < accountStatuses[i].tokens.length; j++) {
                ...
                s.accts[accountStatuses[i].account].mowStatuses[accountStatuses[i].tokens[j]].bdv =
                    accountStatuses[i].mowStatuses[j].bdv;
                s.accts[accountStatuses[i].account].mowStatuses[accountStatuses[i].tokens[j]].lastStem =
