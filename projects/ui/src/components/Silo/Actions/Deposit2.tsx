  ...
  const onSubmit = useCallback(
    async (
      ...
      try {
        ...
        txnBuilder.setMain(
          new txnBuilder.strategies.Silo.Deposit(sdk, {
            target: whitelistedToken,
            account,
            tokenIn: tokenIn,
            amountIn: amountIn,
            claimedBeansUsed,
        ...
        farmActions?.primary?.forEach((txnItem) => {
          txnBuilder.addBefore(getStrategy(txnItem));
        });

        farmActions?.secondary?.forEach((txnItem) => {
          txnBuilder.addAfter(getStrategy(txnItem));
        });

        const transferSteps = getTransferBeanSteps(
          totalClaimAmount,
          claimedBeansUsed,
          farmActions.transferToMode || FarmToMode.INTERNAL
        );

        transferSteps && txnBuilder.addAfter(transferSteps);
        ...
        const { execute } = await txnBuilder.build(
          finalAmountIn,
        ...
        await refetch(
          getActionsPerformed([farmActions?.primary, farmActions?.secondary]),
          {
            beanstalkSilo: true,
            farmerSilo: true,
            farmerBalances: true,
