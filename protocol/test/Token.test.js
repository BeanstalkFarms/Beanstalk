...
describe("Token", function () {
  ...
  describe("Balance functions", async function () {
    ...
    it("Tracks internal balance", async function () {
      expect(
        await this.tokenFacet.getInternalBalance(this.user.address, this.token.address)
      ).to.equal("100");
      expect(
        await this.tokenFacet.getInternalBalance(this.user2.address, this.token.address)
      ).to.equal("200");
      const internalBalances = await this.tokenFacet.getInternalBalances(this.user.address, [
        this.token.address,
        this.token2.address
      ]);
    ...
    it("Tracks external balance", async function () {
      expect(
        await this.tokenFacet.getExternalBalance(this.user.address, this.token.address)
      ).to.equal("700");
      expect(
        await this.tokenFacet.getExternalBalance(this.user2.address, this.token.address)
      ).to.equal("1000");
      const externalBalances = await this.tokenFacet.getExternalBalances(this.user.address, [
        this.token.address,
        this.token2.address
      ]);
    ...
    it("Tracks balance", async function () {
      expect(await this.tokenFacet.getBalance(this.user.address, this.token.address)).to.equal(
        "800"
      );
      expect(await this.tokenFacet.getBalance(this.user2.address, this.token.address)).to.equal(
        "1200"
      );
      const balances = await this.tokenFacet.getBalances(this.user.address, [
        this.token.address,
        this.token2.address
      ]);
    ...
    it("Tracks all balances", async function () {
      checkAllBalance(
        await this.tokenFacet.getAllBalance(this.user.address, this.token.address),
        "100",
        "700",
        "800"
      );
      checkAllBalance(
        await this.tokenFacet.getAllBalance(this.user2.address, this.token.address),
        "200",
        "1000",
        "1200"
      );
      const allBalances = await this.tokenFacet.getAllBalances(this.user.address, [
        this.token.address,
        this.token2.address
      ]);
  ...
  describe("transfer", async function () {
    describe("External to external", async function () {
      it("basic", async function () {
        ...
        checkAllBalance(
          await this.tokenFacet.getAllBalance(this.recipient.address, this.token.address),
          "0",
          "200",
          "200"
        );
    ...
    describe("External to internal", async function () {
      it("basic", async function () {
        this.result = await this.tokenFacet
          .connect(this.user)
        ...
        await expect(this.result)
          .to.emit(this.tokenFacet, "InternalBalanceChanged")
  ...
  describe("weth", async function () {
    ...
    it("deposit WETH to external, not all, farm", async function () {
      const ethBefore = await ethers.provider.getBalance(this.user.address);
      const wrapEth = await this.tokenFacet.interface.encodeFunctionData("wrapEth", [
        to18("1"),
        EXTERNAL
      ]);
  ...
  describe("Token Approval", async function () {
    describe("approve allowance", async function () {
      ...
      it("properly updates users token allowance", async function () {
        expect(
          await this.tokenFacet.tokenAllowance(
            this.user.address,
            this.user2.address,
            this.token.address
          )
      ...
      it("emits TokenApproval event", async function () {
        await expect(
          ...
          .to.emit(this.tokenFacet, "TokenApproval")
    ...
    describe("increase and decrease allowance", async function () {
      ...
      it("emits TokenApproval event on increase", async function () {
        const result = await this.tokenFacet
          .connect(this.user)
    ...
    describe("Approve Token Permit", async function () {
      describe("reverts", function () {
        it("reverts if tokenPermitDomainSeparator is invalid", async function () {
          expect(await this.tokenFacet.connect(this.user).tokenPermitDomainSeparator()).to.be.equal(
        ...
        describe("reverts", function () {
          it("reverts if permit expired", async function () {
            const nonce = await this.tokenFacet
              .connect(this.user)
              .tokenPermitNonces(this.user.address);
