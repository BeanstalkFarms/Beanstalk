...
describe('Curve', function () {
  before(async function () {
    ...
    this.threecurve = await ethers.getContractAt('Mock3Curve', THREE_CRV);
    this.uniswap = await ethers.getContractAt("MockUniswapFacet", this.diamond.address);

    await this.season.siloSunrise(0)
    await this.bean.connect(user).approve(this.silo.address, ethers.utils.parseUnits('100000000000000000000', 50))
    await this.lusd.connect(user).approve(this.silo.address, ethers.utils.parseUnits('10000000000000000000000', 50));
    await this.threecurve.connect(user).approve(this.silo.address, ethers.utils.parseUnits('10000000000000000000', 50));
    ...
    await this.lusd.mintE(userAddress, ethers.utils.parseUnits('20000000000000', 18));
    await this.threecurve.mint(userAddress, ethers.utils.parseUnits('200000000000000', 18));
    await this.bean.mint(userAddress, ethers.utils.parseUnits('2000000000000', 6));

    await this.uniswap.connect(user).toInternalBalance(ethers.utils.parseUnits('10000000000000', 18), this.lusd.address);
    await this.uniswap.connect(user).toInternalBalance(ethers.utils.parseUnits('100000000000000', 18), this.threecurve.address);
    await this.uniswap.connect(user).toInternalBalance(ethers.utils.parseUnits('1000000000000', 6), this.bean.address);
...
describe("External Balances", function () {
  describe("Add Liquidity -> BEAN:LUSD", async function () {
    it("adds liquidity to BEAN:LUSD two tokens", async function () {
      ...
      expect((await this.beanLusd.balanceOf(userAddress)).sub(this.preLP)).to.eq('20000000000000000000000000000');
    ...
    it("adds liquidity to BEAN:LUSD LUSD token only", async function () {
      ...
      expect((await this.beanLusd.balanceOf(userAddress)).sub(this.preLP)).to.eq('7489107900835358977153342642');  // Number determined from current balance of pool
      expect((await this.lusd.balanceOf(userAddress)).sub(this.preLusd)).to.eq('-10000000000000000000000000000')
    ...
    it("adds liquidity to BEAN:LUSD BEAN token only", async function () {
      ...
      expect((await this.bean.balanceOf(userAddress)).sub(this.preBean)).to.eq('-10000000000000000');
      expect((await this.beanLusd.balanceOf(userAddress)).sub(this.preLP)).to.eq('6817685035118681698639275588'); // Number determined from current balance of pool
  ...
  describe("Add Liquidity -> BEAN:3CRV", async function () {
    it("adds liquidity to BEAN:3CRV two tokens", async function () {
      ...
      await this.curve.connect(user).addLiquidityCurve([ethers.utils.parseUnits('10000000000', 6), ethers.utils.parseUnits('10000000000', 18)], 1, BEAN_3CRV, false, false);
      expect((await this.bean.balanceOf(userAddress)).sub(this.preBean)).to.eq('-10000000000000000');
      expect((await this.bean3crv.balanceOf(userAddress)).sub(this.preLP)).to.eq('20000000000000000000000000000');
    ...
    it("adds liquidity to BEAN:3CRV 3CRV token only", async function () {
      ...
      expect((await this.bean3crv.balanceOf(userAddress)).sub(this.preLP)).to.eq('7489107900835358977153342642');
      expect((await this.bean.balanceOf(userAddress)).sub(this.preBean)).to.eq('0');
      expect((await this.threecurve.balanceOf(userAddress)).sub(this.pre3Crv)).to.eq('-10000000000000000000000000000');
    });
    it("adds liquidity to BEAN:3CRV BEAN token only", async function () {
      ...
      expect((await this.bean.balanceOf(userAddress)).sub(this.preBean)).to.eq('-10000000000000000');
      expect((await this.bean3crv.balanceOf(userAddress)).sub(this.preLP)).to.eq('6817685035118681698639275588');
  ...
  describe("Swap -> BEAN:LUSD", async function () {
    it("swaps from BEAN -> LUSD", async function () {
      ...
      expect((await this.lusd.balanceOf(userAddress)).sub(this.preLusd)).to.eq('499999500964052703077');
    });
    it("swaps from LUSD -> BEAN", async function () {
      ...
      expect((await this.bean.balanceOf(userAddress)).sub(this.preBean)).to.eq('500000498');
    });
  });
  describe("Swap -> BEAN:3CRV", async function () {
    it("swaps from BEAN -> 3CRV", async function () {
      ...
      expect((await this.bean.balanceOf(userAddress)).sub(this.preBean)).to.eq('-1000000000');
      expect((await this.threecurve.balanceOf(userAddress)).sub(this.pre3CRV)).to.eq('499999500964052703077');
    });
  ...
  describe("Remove Liquidity -> BEAN:LUSD", async function () {
    it("removes liquidity to BEAN:LUSD two tokens", async function () {
      ...
      expect((await this.bean.balanceOf(userAddress)).sub(this.preBean)).to.eq('5270627954075016'); // Beans post-swap
      expect((await this.beanLusd.balanceOf(userAddress)).sub(this.preLP)).to.eq('-10000000000000000000000000000');
      expect((await this.lusd.balanceOf(userAddress)).sub(this.preLusd)).to.eq('5270096857696636290080580266')
    });
  ...
  describe("Remove Liquidity -> BEAN:3CRV", async function () {
    it("removes liquidity to BEAN:3CRV both tokens", async function () {
      ...
      expect((await this.bean.balanceOf(userAddress)).sub(this.preBean)).to.eq('5270627954075016');
      expect((await this.bean3crv.balanceOf(userAddress)).sub(this.preLP)).to.eq('-10000000000000000000000000000');
      expect((await this.threecurve.balanceOf(userAddress)).sub(this.pre3Crv)).to.eq('5270096857696636290080580266');
    });
...
describe("Internal Balances", function () {
  describe("Add Liquidity -> BEAN:LUSD", async function () {
    it("adds liquidity to BEAN:LUSD two tokens", async function () {
      // First check to make sure no mess up happened above

      expect(await this.lusd.balanceOf(this.silo.address)).to.eq(ethers.utils.parseUnits('10000000000000', 18));
      expect(await this.bean.balanceOf(this.silo.address)).to.eq(ethers.utils.parseUnits('1000000000000', 6));
      expect(await this.threecurve.balanceOf(this.silo.address)).to.eq(ethers.utils.parseUnits('100000000000000', 18));

      this.preBean = await this.uniswap.internalBalance(userAddress, this.bean.address);
      this.preLP = await this.uniswap.internalBalance(userAddress, this.beanLusd.address);
      this.preLusd = await this.uniswap.internalBalance(userAddress, this.lusd.address);
      ...
      expect((await this.uniswap.internalBalance(userAddress, this.bean.address)).sub(this.preBean)).to.eq('-10000000000000000');
      expect((await this.uniswap.internalBalance(userAddress, this.beanLusd.address)).sub(this.preLP)).to.eq('18869704694900623384022390013'); // We're getting a whole lot of LP
      expect((await this.uniswap.internalBalance(userAddress, this.lusd.address)).sub(this.preLusd)).to.eq('-10000000000000000000000000000');
    });
    it("adds liquidity to BEAN:LUSD LUSD token only", async function () {
      this.preBean = await this.uniswap.internalBalance(userAddress, this.bean.address);
      this.preLP = await this.uniswap.internalBalance(userAddress, this.beanLusd.address);
      ...
      expect((await this.uniswap.internalBalance(userAddress, this.beanLusd.address)).sub(this.preLP)).to.eq('7069520671066142975963862280');
      expect((await this.uniswap.internalBalance(userAddress, this.lusd.address)).sub(this.preLusd)).to.eq('-10000000000000000000000000000')
      expect((await this.uniswap.internalBalance(userAddress, this.bean.address)).sub(this.preBean)).to.eq('0');
    });
    it("adds liquidity to BEAN:LUSD BEAN token only", async function () {
      this.preBean = await this.uniswap.internalBalance(userAddress, this.bean.address);
      this.preLP = await this.uniswap.internalBalance(userAddress, this.beanLusd.address);
      ...
      expect((await this.uniswap.internalBalance(userAddress, this.bean.address)).sub(this.preBean)).to.eq('-10000000000000000');
      expect((await this.uniswap.internalBalance(userAddress, this.beanLusd.address)).sub(this.preLP)).to.eq('6644892284624257420706869437');
      expect((await this.uniswap.internalBalance(userAddress, this.lusd.address)).sub(this.preLusd)).to.eq('0');
    });
  });
  describe("Add Liquidity -> BEAN:3CRV", async function () {
    it("adds liquidity to BEAN:3CRV both tokens", async function () {
      this.preBean = await this.uniswap.internalBalance(userAddress, this.bean.address);
      this.preLP = await this.uniswap.internalBalance(userAddress, this.bean3crv.address);
      this.pre3Crv = await this.uniswap.internalBalance(userAddress, this.threecurve.address);
      ...
      expect((await this.uniswap.internalBalance(userAddress, this.bean.address)).sub(this.preBean)).to.eq('-10000000000000000');
      expect((await this.uniswap.internalBalance(userAddress, this.bean3crv.address)).sub(this.preLP)).to.eq('18869704694900623384022390013');
      expect((await this.uniswap.internalBalance(userAddress, this.threecurve.address)).sub(this.pre3Crv)).to.eq('-10000000000000000000000000000');
    });
    it("adds liquidity to BEAN:3CRV 3CRV token only", async function () {
      this.preBean = await this.uniswap.internalBalance(userAddress, this.bean.address);
      this.preLP = await this.uniswap.internalBalance(userAddress, this.bean3crv.address);
      ...
      expect((await this.uniswap.internalBalance(userAddress, this.bean3crv.address)).sub(this.preLP)).to.eq('7069520671066142975963862280');
      expect((await this.uniswap.internalBalance(userAddress, this.bean.address)).sub(this.preBean)).to.eq('0');
      expect((await this.uniswap.internalBalance(userAddress, this.threecurve.address)).sub(this.pre3Crv)).to.eq('-10000000000000000000000000000');
    });
    it("adds liquidity to BEAN:3CRV BEAN token only", async function () {
      ...
      expect((await this.uniswap.internalBalance(userAddress, this.bean.address)).sub(this.preBean)).to.eq('-10000000000000000');
      expect((await this.uniswap.internalBalance(userAddress, this.bean3crv.address)).sub(this.preLP)).to.eq('6644892284624257420706869437');
      expect((await this.uniswap.internalBalance(userAddress, this.threecurve.address)).sub(this.pre3Crv)).to.eq('0');
    });
  });
  describe("Swap -> BEAN:LUSD", async function () {
    it("swaps from BEAN -> LUSD", async function () {
      this.preBean = await this.uniswap.internalBalance(userAddress, this.bean.address);
      this.preLusd = await this.uniswap.internalBalance(userAddress, this.lusd.address);
      this.preLusdE = await this.lusd.balanceOf(userAddress);
      ...
      expect((await this.uniswap.internalBalance(userAddress, this.bean.address)).sub(this.preBean)).to.eq('-1000000000');
      expect((await this.uniswap.internalBalance(userAddress, this.lusd.address)).sub(this.preLusd)).to.eq('0');
      expect((await this.lusd.balanceOf(userAddress)).sub(this.preLusdE)).to.eq('499999497452212644656');
    });
    it("swaps from LUSD -> BEAN", async function () {
      this.preBean = await this.uniswap.internalBalance(userAddress, this.bean.address);
      this.preBeanE = await this.bean.balanceOf(userAddress);
      this.preLusd = await this.uniswap.internalBalance(userAddress, this.lusd.address);
      ...
      expect((await this.uniswap.internalBalance(userAddress, this.lusd.address)).sub(this.preLusd)).to.eq('-1000000000000000000000');
      expect((await this.uniswap.internalBalance(userAddress, this.bean.address)).sub(this.preBean)).to.eq('0');
      expect((await this.bean.balanceOf(userAddress)).sub(this.preBeanE)).to.eq('500000502');
    });
  });
  describe("Swap -> BEAN:3CRV", async function () {
    it("swaps from BEAN -> 3CRV", async function () {
      this.preBean = await this.bean.balanceOf(userAddress);
      this.pre3CrvE = await this.threecurve.balanceOf(userAddress);
      this.pre3Crv = await this.uniswap.internalBalance(userAddress, this.threecurve.address);
      ...
      expect((await this.bean.balanceOf(userAddress)).sub(this.preBean)).to.eq('-1000000000');
      expect((await this.threecurve.balanceOf(userAddress)).sub(this.pre3CrvE)).to.eq('0');
      expect((await this.uniswap.internalBalance(userAddress, this.threecurve.address)).sub(this.pre3Crv)).to.eq('499999497452212644656');
    });
    it("swaps from 3CRV -> BEAN", async function () {
      this.preBeanE = await this.bean.balanceOf(userAddress);
      this.preBean = await this.uniswap.internalBalance(userAddress, this.bean.address);
      ...
      expect((await this.threecurve.balanceOf(userAddress)).sub(this.pre3Crv)).to.eq('-1000000000000000000000');
      expect((await this.bean.balanceOf(userAddress)).sub(this.preBeanE)).to.eq('0');
      expect((await this.uniswap.internalBalance(userAddress, this.bean.address)).sub(this.preBean)).to.eq('500000502');
    });
  });
  describe("Remove Liquidity -> BEAN:LUSD", async function () {
    it("removes liquidity to BEAN:LUSD two tokens", async function () {
      // Perform the necessary conversions so that we can continue with the internal balance tests
    
      this.am1 = await this.beanLusd.balanceOf(userAddress);
      this.am2 = await this.bean3crv.balanceOf(userAddress);
      await this.uniswap.connect(user).toInternalBalance(this.am1, this.beanLusd.address);
      await this.uniswap.connect(user).toInternalBalance(this.am2, this.bean3crv.address);

      this.preBean = await this.uniswap.internalBalance(userAddress, this.bean.address);
      this.preLP = await this.uniswap.internalBalance(userAddress, this.beanLusd.address);
      this.preLusd = await this.uniswap.internalBalance(userAddress, this.lusd.address);
      await this.curve.connect(user).removeLiquidityCurve('10000000000000000000000000000', [ethers.utils.parseUnits('10000000000', 5), ethers.utils.parseUnits('10000000000', 17)], BEAN_LUSD, true, true);
      expect((await this.uniswap.internalBalance(userAddress, this.bean.address)).sub(this.preBean)).to.eq('5487125423106048');
      expect((await this.uniswap.internalBalance(userAddress, this.beanLusd.address)).sub(this.preLP)).to.eq('-10000000000000000000000000000');
      expect((await this.uniswap.internalBalance(userAddress, this.lusd.address)).sub(this.preLusd)).to.eq('5486568561642228243814536906')
    });
    it("removes liquidity to BEAN:LUSD LUSD token only", async function () { 
      this.preBean = await this.uniswap.internalBalance(userAddress, this.bean.address);
      this.preLP = await this.uniswap.internalBalance(userAddress, this.beanLusd.address);
      this.preLusd = await this.uniswap.internalBalance(userAddress, this.lusd.address);
      await this.curve.connect(user).removeLiquidityImbalanceCurve('10000000000000000000000000000', ['0', ethers.utils.parseUnits('10000000000', 17)], BEAN_LUSD, true, true);
      expect((await this.uniswap.internalBalance(userAddress, this.beanLusd.address)).sub(this.preLP)).to.eq('-10000000000000000000000000000');
      expect((await this.uniswap.internalBalance(userAddress, this.lusd.address)).sub(this.preLusd)).to.eq('1000000000000000000000000000')
      expect((await this.uniswap.internalBalance(userAddress, this.bean.address)).sub(this.preBean)).to.eq('0');
    });
    it("removes liquidity to BEAN:LUSD BEAN token only", async function () {
      this.preBean = await this.uniswap.internalBalance(userAddress, this.bean.address);
      this.preLP = await this.uniswap.internalBalance(userAddress, this.beanLusd.address);
      this.preLusd = await this.uniswap.internalBalance(userAddress, this.lusd.address);
      await this.curve.connect(user).removeLiquidityImbalanceCurve('10000000000000000000000000000', [ethers.utils.parseUnits('10000000000', 5), '0'], BEAN_LUSD, true, true);
      expect((await this.uniswap.internalBalance(userAddress, this.bean.address)).sub(this.preBean)).to.eq('1000000000000000');
      expect((await this.uniswap.internalBalance(userAddress, this.beanLusd.address)).sub(this.preLP)).to.eq('-10000000000000000000000000000');
      expect((await this.uniswap.internalBalance(userAddress, this.lusd.address)).sub(this.preLusd)).to.eq('0');
    });
  });
  describe("Remove Liquidity -> BEAN:3CRV", async function () {
    it("removes liquidity to BEAN:3CRV both tokens", async function () {
      ...
      expect((await this.bean.balanceOf(userAddress)).sub(this.preBeanE)).to.eq('5487125423106048');
      expect((await this.uniswap.internalBalance(userAddress, this.bean3crv.address)).sub(this.preLP)).to.eq('-10000000000000000000000000000');
      expect((await this.threecurve.balanceOf(userAddress)).sub(this.pre3CrvE)).to.eq('5486568561642228243814536906');
    });
    it("removes liquidity to BEAN:3CRV 3CRV token only", async function () { 
      this.preLP = await this.uniswap.internalBalance(userAddress, this.bean3crv.address);
      this.preBeanE = await this.bean.balanceOf(userAddress);
      ...
      expect((await this.uniswap.internalBalance(userAddress, this.bean3crv.address)).sub(this.preLP)).to.eq('-10000000000000000000000000000');
      expect((await this.bean.balanceOf(userAddress)).sub(this.preBeanE)).to.eq('0');
      expect((await this.threecurve.balanceOf(userAddress)).sub(this.pre3CrvE)).to.eq('1000000000000000000000000000');
    });
    it("removes liquidity to BEAN:3CRV BEAN token only", async function () { 
      this.preLP = await this.uniswap.internalBalance(userAddress, this.bean3crv.address);
      this.preBeanE = await this.bean.balanceOf(userAddress);
      ...
      expect((await this.bean.balanceOf(userAddress)).sub(this.preBeanE)).to.eq('1000000000000000');
      expect((await this.uniswap.internalBalance(userAddress, this.bean3crv.address)).sub(this.preLP)).to.eq('-10000000000000000000000000000');
      expect((await this.threecurve.balanceOf(userAddress)).sub(this.pre3CrvE)).to.eq('0');
    });
  });
