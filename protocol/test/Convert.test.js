...
describe('Convert', function () {
  ...
  describe('Withdraw For Convert', async function () {
    describe("Revert", async function () {
      it('diff lengths', async function () {
        await expect(mockBeanstalk.connect(user).withdrawForConvertE(this.siloToken.address, [to6('1'), to6('2')], ['100'], '100')).to.be.revertedWith('Convert: stems, amounts are diff lengths.')
      });
      ...
      it('crate balance too low', async function () {
        ...
        await expect(mockBeanstalk.connect(user).withdrawForConvertE(this.siloToken.address, [to6('2')], ['150'], '150')).to.be.revertedWith('Silo: Crate balance too low.')
      });

      it('not enough removed', async function () {
        await expect(mockBeanstalk.connect(user).withdrawForConvertE(this.siloToken.address, [to6('2')], ['100'], '150')).to.be.revertedWith('Convert: Not enough tokens removed.')
      });
    })
    ...
    describe("Withdraw 1 Crate", async function () {
      beforeEach(async function () {
        this.result = await mockBeanstalk.connect(user).withdrawForConvertE(this.siloToken.address, [to6('2')], ['100'], '100');
      })

      it('Emits event', async function () {
        await expect(this.result).to.emit(beanstalk, 'RemoveDeposits').withArgs(user.address, this.siloToken.address, [2000000], ['100'], '100', ['100']);
    ...
    describe("Withdraw 1 Crate 2 input", async function () {
      beforeEach(async function () {
        this.result = await mockBeanstalk.connect(user).withdrawForConvertE(this.siloToken.address, [to6('2'), to6('1')], ['100', '100'], '100');
      })

      it('Emits event', async function () {
        await expect(this.result).to.emit(beanstalk, 'RemoveDeposits').withArgs(user.address, this.siloToken.address, [to6('2'), to6('1')], ['100', '0'], '100',['100', '0'] );
        await expect(this.result).to.emit(mockBeanstalk, 'MockConvert').withArgs('200', '100');
      })
    ...
    describe("Withdraw 2 Crates exact", async function () {
      beforeEach(async function () {
        this.result = await mockBeanstalk.connect(user).withdrawForConvertE(this.siloToken.address, [to6('1'), to6('2')], ['100', '50'], '150');
      })

      it('Emits event', async function () { 
        await expect(this.result).to.emit(beanstalk, 'RemoveDeposits').withArgs(user.address, this.siloToken.address, [to6('1'),  to6('2')], ['100', '50'], '150', ['100', '50']);
    ...
    describe("Withdraw 2 Crates under", async function () {
      beforeEach(async function () {
        this.result = await mockBeanstalk.connect(user).withdrawForConvertE(this.siloToken.address, [to6('1'), to6('2')], ['100', '100'], '150');
      })
  ...
  describe('Deposit For Convert', async function () {
    describe("Revert", async function () {
      it("Reverts if BDV is 0", async function () {
        await expect(mockBeanstalk.connect(user).depositForConvertE(this.siloToken.address, '100', '0', '100')).to.be.revertedWith("Convert: BDV or amount is 0.")
      })

      it("Reverts if amount is 0", async function () {
        await expect(mockBeanstalk.connect(user).depositForConvertE(this.siloToken.address, '0', '100', '100')).to.be.revertedWith("Convert: BDV or amount is 0.")
      })
    })
    ...
    describe('Deposit not germinating', async function () {
      beforeEach(async function () {
        ...
        this.result = await mockBeanstalk.connect(user2).depositForConvertE(this.siloToken.address, '100', '100', '100');
      });
      ...
      it('Increment totals', async function () {
        ...
        expect(await beanstalk.getGerminatingTotalDeposited(this.siloToken.address)).to.equal('100');
        expect(await beanstalk.getGerminatingTotalDepositedBdv(this.siloToken.address)).to.eq('100');
    ...
    describe('Deposit Tokens with grown stalk', async function () {
      beforeEach(async function () {
        ...
        expect(await beanstalk.getGerminatingTotalDepositedBdv(this.siloToken.address)).to.eq('0');
        expect(await beanstalk.getTotalGerminatingStalk()).to.equal('0');
        this.result = await mockBeanstalk.connect(user2).depositForConvertE(this.siloToken.address, '100', '100', '300');
      });
  ...
  describe("lambda convert", async function () {
    ...
    beforeEach(async function () {
      this.result = await beanstalk.connect(user).callStatic.convert(
    ...
    it('Emits events', async function () {
      await expect(this.result).to.emit(beanstalk, 'RemoveDeposits').withArgs(user.address, this.siloToken.address, [to6('1'),  to6('2')], ['100', '100'], '200', ['100', '100']);
      await expect(this.result).to.emit(beanstalk, 'AddDeposit').withArgs(user.address, this.siloToken.address, to6('1.5'), '200', '200');
    })
  })
});
