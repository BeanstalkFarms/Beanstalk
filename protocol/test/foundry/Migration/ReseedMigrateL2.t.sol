...
contract reeseedMigrateL2 is TestHelper {
    ...
    function setUp() public {
        ...
        vm.label(LibConstant.BEAN, "BEAN");
    ...
    function test_bcm_transfer() public {
        uint256 beanEthBalance = IERC20(LibConstant.BEAN_ETH_WELL).balanceOf(LibConstant.BEANSTALK);
        uint256 bean3crvBalance = IERC20(CURVE_BEAN_METAPOOL).balanceOf(LibConstant.BEANSTALK);
        uint256 wstethBalance = IERC20(LibConstant.BEAN_WSTETH_WELL).balanceOf(
            LibConstant.BEANSTALK
        );
    ...
    function test_transferTokens() public {
        ...
        // transfer out.
        address token = address(LibConstant.BEAN);

        uint256 initialTokenBalance = IERC20(LibConstant.BEAN).balanceOf(LibConstant.BEANSTALK);
        uint256 initialUserBalance = IERC20(LibConstant.BEAN).balanceOf(user);
        uint256 initialInternalTokenBalance = bs.getInternalBalance(user, token);

        vm.prank(user);
        IERC20(LibConstant.BEAN).approve(LibConstant.BEANSTALK, type(uint256).max);
        ...
        assertEq(IERC20(token).balanceOf(LibConstant.BEANSTALK) - initialTokenBalance, 1e6);
    ...
    function test_bean_l2_migration_external() public {
        ...
        IERC20(LibConstant.BEAN).approve(LibConstant.BEANSTALK, 1e6);
        ...
        vm.startPrank(BS_FARMS);
        L2MigrationFacet(LibConstant.BEANSTALK).approveL2Receiver{value: 0.005 ether}(
            BS_FARMS,
            L2_BEANSTALK,
            2e14, // max submission cost = 200k gas * 10 gwei
            200000, // 200k gas to execute on L2
