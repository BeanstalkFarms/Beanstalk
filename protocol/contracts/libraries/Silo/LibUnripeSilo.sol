...
library LibUnripeSilo {
    ...
    function removeLegacyUnripeBeanDeposit(address account, uint32 season) internal {
        ...
        delete s.accounts[account].bean.deposits[season];
    }
    ...
    function unripeBeanDeposit(
        ...
        AppStorage storage s = LibAppStorage.diamondStorage();
        uint256 legacyAmount = s.accounts[account].bean.deposits[season];

        // Sum the `account` pre-exploit Silo V1 Bean Balance
        // and the Silo V2 Unripe Bean Balance
        amount = uint256(s.accounts[account].legacyV2Deposits[C.UNRIPE_BEAN][season].amount).add(
            legacyAmount
        );
        ...
        bdv = uint256(s.accounts[account].legacyV2Deposits[C.UNRIPE_BEAN][season].bdv).add(
    ...
    function removeLegacyUnripeLPDeposit(address account, uint32 season) internal {
        AppStorage storage s = LibAppStorage.diamondStorage();
        delete s.accounts[account].lp.depositSeeds[season];
        delete s.accounts[account].lp.deposits[season];
        delete s.accounts[account].legacyV2Deposits[C.unripeLPPool1()][season];
        delete s.accounts[account].legacyV2Deposits[C.unripeLPPool2()][season];
    }
    ...
    function unripeLPDeposit(
        ...
        amount = uint256(s.accounts[account].legacyV2Deposits[C.UNRIPE_LP][season].amount).add(
        ...
        bdv = uint256(s.accounts[account].legacyV2Deposits[C.UNRIPE_LP][season].bdv).add(legBdv);
    }
    ...
    function getBeanEthUnripeLP(
        ...
        AppStorage storage s = LibAppStorage.diamondStorage();
        bdv = s.accounts[account].lp.depositSeeds[season].div(4);
        ...
        amount = s.accounts[account].lp.deposits[season].mul(AMOUNT_TO_BDV_BEAN_ETH).div(1e18);
    }
    ...
    function getBeanLusdUnripeLP(
        ...
        AppStorage storage s = LibAppStorage.diamondStorage();
        bdv = uint256(s.accounts[account].legacyV2Deposits[C.unripeLPPool2()][season].bdv);

        // `amount` is equal to the pre-exploit BDV of the Deposited BEAN:LUSD
        // tokens. This is the equivalent amount of Unripe BEAN:3CRV LP.
        amount = uint256(s.accounts[account].legacyV2Deposits[C.unripeLPPool2()][season].amount)
    ...
    function getBean3CrvUnripeLP(
        ...
        AppStorage storage s = LibAppStorage.diamondStorage();
        bdv = uint256(s.accounts[account].legacyV2Deposits[C.unripeLPPool1()][season].bdv);

        // `amount` is equal to the pre-exploit BDV of the Deposited BEAN:3CRV
        // tokens. This is the equivalent amount of Unripe BEAN:3CRV LP.
        amount = uint256(s.accounts[account].legacyV2Deposits[C.unripeLPPool1()][season].amount)
